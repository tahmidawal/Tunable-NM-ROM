#!/usr/bin/env bash
# launch.sh <cell> : stage (built by fu_cells.sh) -> scp DIRECTLY into the paralab
# namespace pobj2/<cell> (never through login /tmp) -> verify checksums -> squeue
# before/after -> sbatch.  One job per cell dir.  Prints the job id.
set -euo pipefail
cell="$1"
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
STAGE="$HERE/stage/$cell"
REMOTE="/cluster/tufts/paralab/tawal01/pobj2/$cell"
