...
>
> **And the like-for-like comparison is worse than either of those.** Measured against an iso-accuracy FOM — CG run only as far as the ROM's own accuracy, which is the honest denominator — the coordinate ROM is **0.74× at N=64** (slower than simply under-converging CG) and **1.42× at N=128**. That 0.74× is an *upper bound*, not a point estimate: a coarse tolerance ladder overshoots the target, so a finer one makes the FOM cheaper still. **The claim cannot be "the ROM is faster than the FOM."** It is mesh-independence: ROM cost flat at 3.42 → 3.64 ms from N=64→128 while the iso-accuracy FOM roughly doubles per refinement, 2.51 → 5.18 ms. Everything of value lies to the right of the crossover, which sits between N=64 and N=128.
>
