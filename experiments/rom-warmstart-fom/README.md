...
> dropped. Cross-`N` wall clock: Poisson from Slurm job `2511369` on `cuda:0`, Burgers
> from job `2511371` on `cuda:0`.
...
**more BiCGStab iterations in all 12 configurations**, including the
4 of 12 where it *improves* the Newton count — a guess that is 1–2% wrong in a
...
47.62 / 85.56 / 228.1 /
449.9 ms at `N = 32/64/128/256` with an achieved residual band of about
