...
wsf_verify.py     independent re-derivation of the headline claims from the raw JSONs,
                  checked against the rendered README (130 checks)
...
exact to 2.13e-14 relative error and takes **0.166 ms at `N = 512`**,
against 81.58 ms for CG at `1e-10` — **494x faster than the
iterative FOM, and 0.0633–0.166 ms across the whole ladder.** On this
...
(0.663x → 0.920x at `tau_FOM = 1e-10`), so a crossover plausibly
exists beyond `N = 512`, but **we did not measure one and do not claim one.** Against an
iterative FOM that a direct solver already beats by 494x, that extrapolation
...
- **`err_final` at the tightest tolerance is limited by the reference solution's own
  accuracy** (relative residual 3.96e-12 at `N = 512`, an f64 floor of the FD
...
- **Panel/consolidated agreement is complete, not partial**: all eleven jobs finished, so
  261 hardware-independent quantities were compared between the fanned-out
...
15.145 ms, this cell at 14.86 ms, and the sister
`cost-to-tolerance` cell at 14.79 ms — a 2.3% spread.
