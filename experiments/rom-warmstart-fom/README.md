...
wsf_verify.py     independent re-derivation of the headline claims from the raw JSONs,
                  checked against the rendered README (146 checks)
...
> Every number in this section is **substituted by `wsf_render_readme.py`** from facts in
> `wsf_facts.py`; `README.md` is generated from `README.tmpl.md` and an unknown placeholder is
> a hard error. The tables live in `SUMMARY_TABLES.md`, generated by `wsf_summarize.py`.
> **One exception, flagged at every use:** three timings measured by the sister
> `cost-to-tolerance` cell are hard-coded constants relayed from it, not derived here or
> verifiable from this checkout. Everything else is derived from the run JSONs.
> Sources: 11 complete reports, 174 rows, 0 incomplete report(s)
> dropped. Cross-`N` wall clock: Poisson from Slurm job `2511369` on `NVIDIA A100 80GB PCIe`, Burgers
> from job `2511371` on `NVIDIA A100 80GB PCIe`.

### The answer

**Poisson.** The hybrid **never breaks even** anywhere in the ladder: its best result is 0.933x (N=512, rom_tau=0.01, tau_FOM=1e-06), i.e. it is always slower than simply running the FOM. Per tolerance the crossover mesh is N = none in the ladder at
`tau_FOM = 1e-6`, N = none in the ladder at `1e-8` and N = none in the ladder at `1e-10`.

**Burgers.** The ROM warm start **loses in every configuration measured**: it beat the previous-step start on Newton iterations in 4 of 12, on inner BiCGStab iterations in 0 of 12, and on wall clock in 0 of 12. The mechanism is the inner linear work: the ROM start costs
**more BiCGStab iterations in all 12 configurations**, including those
...
beats the previous-step start in 12 of 12 configurations on Newton iterations and
10 of 12 on BiCGStab iterations. **A one-line heuristic outperforms the learned
...
1 of 12 configurations — N=256, tau_FOM=1e-06 (434.7 vs 449.9 ms). It is the one place the effect
...
|---|---|---|---|---|---|---|
| 32 | 900 | 3.223 | 4.861 | 0.663x | 0.5 | 3.68% |
| 64 | 3844 | 6.549 | 8.453 | 0.775x | 0.5 | 2.84% |
| 128 | 15876 | 13.33 | 15.49 | 0.861x | 0.5 | 2.63% |
| 256 | 64516 | 26.26 | 30.17 | 0.870x | 0.01 | 2.39% |
| 512 | 260100 | 81.58 | 88.67 | 0.920x | 0.01 | 2.21% |
...
it is as accurate as the ROM, against 1772 for the full solve. (Different
sample populations: the diagnostic runs on 4 cases, the full-solve count
averages 16.) Yet
...
decode of the latent onto the FOM's grid is not — it grows from 0.118 ms at
`N = 32` to 4.56 ms at `N = 512`. Any hybrid that hands a *field* to a
...
exact to 2.13e-14 relative error. Taking both operands from one row
(`N = 512`, `tau_FOM = 1e-10`, `rom_tau = mean of 5 ROM rungs`): 0.1663 ms for
the direct solve against 81.93 ms for CG — **493x**. Across the
whole ladder the direct solve costs 0.0633–0.166 ms. On this
...
Variant `lspg:eq256:weak64` (m = 256 EQ nodes), 50 backward-Euler steps, at
`tau_FOM = 1e-8`:

| N | Newton (prev) | Newton (extrap) | Newton (ROM) | BiCGStab (prev) | BiCGStab (extrap) | BiCGStab (ROM) |
|---|---|---|---|---|---|---|
| 32 | 100.2 | 83.5 | 100.2 | 789 | 699 | 1074 |
| 64 | 101.5 | 87.2 | 103.8 | 1440 | 1313 | 2105 |
| 128 | 101.8 | 89.0 | 104.0 | 2839 | 2642 | 4111 |
| 256 | 101.8 | 89.5 | 103.8 | 5577 | 5139 | 7695 |

Wall clock at `tau_FOM = 1e-8`: the pure FOM takes 489.5 ms at `N = 256`
against a hybrid total of 1124 ms (0.436x), of which
273.2 ms is the ROM stage and 206.4 ms the decode. The ROM's
own trajectory error is 0.0198, comfortably good enough to be a plausible
...
simply running the FOM. Its best result anywhere is 0.933x
(N=512, rom_tau=0.01, tau_FOM=1e-06) — still a loss. The trend is monotone in `N`
(0.663x → 0.920x at `tau_FOM = 1e-10`). **We did not measure a
crossover and do not claim one exists.** Extrapolating would require assuming the ROM stage
stays flat and the saving fraction holds, and neither is true in the measured range — the
decode grows from 0.118 to 4.56 ms and the saving falls with tighter
solves. The break-even meshes in `SUMMARY_TABLES.md` P4b freeze both at their `N = 512` values
and are therefore a **conditional toy calculation, not a prediction.** Against an
iterative FOM that a direct solver already beats by 493x, that extrapolation
is of limited interest anyway. Burgers loses outright, to a baseline the FOM already had for free.
...
assumed — 1.55x at `1e-6` down to 1.16x at `1e-10`; the
...
- **Measured, not assumed, and both stated risks landed.** Risk (a) — the ROM costs more than
  ...
  none in the ladder. Risk (b) — the Burgers FOM already warm-starts — is why that arm loses.
- **`dt` is not varied.** The Burgers testbed hard-codes `dt = 0.005`, where the previous
  state is a very good guess. The ROM's advantage may grow with `dt` — untested here — but changing `dt`
...
- **The Poisson CG is unpreconditioned**, as in the testbed. A preconditioner would cut iterations in
  both arms; whether that shrinks the absolute saving a warm start buys was not measured.
...
- **`err_final` at the tightest tolerance is limited by the reference solution's own
  accuracy.** The reference CG is *requested* at `1e-12` and *achieves* a true
  relative residual of 8.788e-13 at `N = 128`, 9.832e-13 at `N = 256` and
  3.962e-12 at `N = 512` — an f64 floor of the FD operator that grows with `N`. The correctness gate is reference-free: every row asserts the
  delivered iterate's true relative residual is at most `tau`, and the largest ratio observed
  was 1x `tau` on Poisson and 0.998x on Burgers.
- **Solver health.** 0 BiCGStab breakdowns, 0 non-zero Newton flags
  and 0 health warnings across every Burgers configuration; the known NaN
  landmine did not fire, and had it fired it would have been reported, not dropped.
- **The best-per-`N` hybrid is post-selected, and these runs cannot bound the resulting
  bias.** It minimises over the ROM tolerance ladder using the same timing samples it reports.
  Worse, **the per-measurement repetition arrays were not persisted in these runs** (only the
  medians survived), so the best-of-noise bias cannot be reconstructed from them. Using the
  only retained proxy — the spread among the repeated paired zero-start baselines — the
  results audit found the best/second-best gap no larger than that proxy in **6 of 15**
  `(N, tau_FOM)` groups. **Treat the full ladder (P1) as the primary result and the exact
  identity of the "best" rung as unresolved.** The headline is unaffected: the best measured
  speedup is 0.933x, so breaking even would need a ~6.7% improvement, far outside
  that proxy. `time_fn`'s repetition arrays are now persisted so a future run can settle it.
- **Panel/consolidated agreement is complete, and all but one quantity agree**: all eleven
  jobs finished, so 261 hardware-independent quantities were compared between the fanned-out
  per-mesh jobs and the single-GPU consolidation runs. 1 disagreed, the
  largest by 0.0625 (iters_from_rom at (512, 0.5, 1e-10)) — one CG iteration on
...
- **`N = 1024` was not run.** The question as posed covers `N` up to 512, and over that range
  the answer is unambiguous: none in the ladder. An extrapolated break-even mesh is given in
...
(`oc_*_any_matched_rung` = no / no), and an
...
`N = 32` the archived Poisson solve genuinely reaches 8.86e-14, i.e. it delivers
...
1e-13. At `N = 512` the archived solve reaches only 3.94e-12 despite the same
...
**400 Newton steps and 400 inner BiCGStab solves,
regardless of need.** The accuracy it lands on is far past anything asked for: its worst
per-step relative Newton residual is 8.85e-13 at `N = 256`.
...
per step is then whatever convergence requires (capped at 25). The sister
...
47.62 / 85.56 / 228.1 /
449.9 ms at `N = 32/64/128/256`, with the per-step Newton counts and the
...
1. At `tau = 1e-6` this cell converges in 1.97–1.99 Newton steps per
   ...
   **more** work. (Across the full tolerance ladder the count runs
   1.97–2.40; the cross-check is pinned at `1e-6`.) The
...
**A third candidate asymmetry — per-step Python overhead — was reported eliminated.** The
sister cell states that its Burgers rungs call a `@jax.jit` rollout whose body is a single
`lax.scan` over all 50 steps, timed as one invocation with one
`block_until_ready`, as here. **That is its report of its own code and is not auditable from
this checkout**; if it holds, effect 2 is the only asymmetry left and the expected sign stands
without widening the band.

**What is averaged over what, precisely.** In this cell the iteration counts and errors
average 4 held-out trajectories, but **every Burgers wall clock times trajectory 0
only** — the timed and the averaged populations are not the same, and the pre-registered
comparison is against the trajectory-0 timing. The sister cell's trajectory count, burn-in
length and verdict band are **relayed, not auditable from this checkout**; they are recorded
here as its description of its own method, not as verified fact.
...
| 256 | 2159 | 400 | 8.85e-13 | 496.8 | 120 | 9.48e-11 | 4.35x |

`tau = 1e-10` is the **tightest rung measured here**, not an accuracy match: the fixed-8 scan
reaches 8.85e-13 at `N = 256` while this rung lands at 9.48e-11,
about 107x looser. Treat the factor as an engineering one. At the more
realistic `tau = 1e-8` the tolerance-based FOM costs 489.5 ms at `N = 256`
(4.41x).
...
Measured here against the archived value, the two agree to 0.21% at
`N = 32`, 6.8% at `N = 64`, 0.13% at `N = 128` and
0.53% at `N = 256`. The correction is therefore not an extrapolation from a
...
- The **iteration** multiplier (400 fixed Newton steps against the measured
  tolerance-based count) is a pure work-count proxy needing no hardware assumption. It is
  **not** systematically larger or smaller than the time multiplier — on Burgers it is larger
  at three of four meshes and smaller at one — so it is not a bound in either direction, only
  an independent estimate. **Use it wherever the anchor is loose.**
...
|---|---|---|---|---|---|---|
| 32 | 5.591 | 3.548 | 3.66 | 3.1% | **archive-stale** | time (instruments agree) |
| 64 | 7.786 | 7.125 | 7.144 | 0.27% | **archive-stale** | time (instruments agree) |
| 128 | 15.14 | 14.86 | 14.79 | 0.44% | tight | time |

Every mesh with a second instrument comes out **archive-stale or tight, and none is
unmeasurable**. At `N = 64` the two instruments agree to 0.27% while both sit
~8.5% below the archive **in the same direction**; at `N = 32` they agree to
3.1% while both sit far below an archive value of 5.591 ms.
...
> there — it records 5.591 ms for the same function this cell measures at
> 3.548 ms, a 37% gap, and a coarse solve is dominated by
> per-iteration kernel launch overhead that does not transfer between environments. But a
> *second independent instrument* lands within 3.1% of this cell, so the
...
> solve genuinely reaches 8.86e-14 there, so by that definition its over-convergence
> factor is essentially unity and there is little to correct. The engineering factor
> (1.16x) is the one that applies, and a reader should know which question
> they are answering before quoting it.
>
> This cell's `N = 32` figure **was** burn-in protected — 1216 CG solves immediately precede
> it in the log — so the 37% gap to the archive is a real difference in
> the archived environment, not a clock-ramp artefact in ours. It is independently
> corroborated: the sister cell re-times the same function at 3.66 ms,
> 3.1% from this cell's 3.548 ms, and reports that adding
...
> percent (0.865 vs 0.862); what licenses the choice
...
tolerance difference (the two agree to 2.6e-16 on a representative linear solve, and the re-timed archived
function agrees with the archived JSON to 0.13% at `N = 128` — a WALL-TIME
anchor, not a trajectory-state comparison — but the outer loops are not the same code). Old values read from `../burgers2d-rom-latent-stepping/runs/followup/bt_n/timing_n.json`:
...
0.267 across the ladder, a 28% spread. Applying a single
...
> this section headlined the Poisson over-convergence factor as **~1.16x**
...
> `1e-6` it is **1.53-1.57x**. The sister cell measured
> 1.55x independently at a `1e-6` target; **the two cells agree once the
...
|---|---|---|---|
| `1e-6` | 1.53-1.57x | 1.55x | 0.65 |
| `1e-8` | 1.31-1.32x | 1.31x | 0.76 |
| `1e-10` | 1.15-1.16x | 1.16x | 0.86 |

It is near-constant **across meshes** at every tolerance (hence a single scalar per assumed
tolerance is defensible), but it varies by a factor of 1.34 **across
...
the recomputed true residual, so the `1e-6` solve delivers 9.93e-07–9.99e-07,
i.e. within 1.01x of its target. A *rung-based* ladder that must pick the cheapest
...
`CG_TOL = 1e-13` — the tolerance used to **manufacture the truth data**, not a
tolerance any consumer of the solution would request. A fixed, very tight tolerance inflates the
baseline the same way a fixed iteration count does, though the mechanisms differ: one forces
work regardless of convergence, the other forces convergence far past what is asked.

The comparison below is **like-for-like in the solver**: both columns are
`jax.scipy.sparse.linalg.cg`, differing only in tolerance. (This cell's own instrumented CG is in fact
1.3–7.3% *slower* per solve than `jax.scipy`'s at the same
tolerance, so comparing the testbed's `jax.scipy` baseline against the instrumented one would
still conflate "tighter tolerance" with "different implementation", just in the other
direction. That mixed ratio is recorded as `oc_p_factor_mixed_*` and is **not** what is
quoted here.)
...
| 256 | 29.61 | 1034 | 4.56e-13 | 25.56 | 876 | 9.99e-11 | 1.16x |
| 512 | 93.07 | 2244 | 3.94e-12 | 80.24 | 1772 | 9.99e-11 | 1.16x |
...
agreement of 37% at `N = 32`, 8.5% at `N = 64`,
1.9% at `N = 128`, 4.9% at `N = 256` and
3.1% at `N = 512`.
...
15.14 ms, this cell at 14.86 ms, and the sister
`cost-to-tolerance` cell at 14.79 ms — a 2.4% spread.
...
|---|---|---|---|---|
| 32 | 0.28x | 0.18x | 0.21x | 0.24x |
| 64 | 0.39x | 0.25x | 0.29x | 0.34x |
| 128 | 0.74x | 0.47x | 0.57x | 0.64x |
| 256 | 1.40x | 0.91x | 1.07x | 1.20x |
| 512 | 3.56x | 2.33x | 2.73x | 3.08x |

**At a `1e-6` consumer tolerance the archived Poisson ROM no longer beats the FOM at
`N = 256`** (0.91x, against the 1.40x published and the
1.20x this section previously reported) — a sign change, not just a
rescaling.
...
|---|---|---|---|---|---|---|---|---|---|
| 32 | 5.591 | 0.28x | 0.28x | 37% **archive-stale** | time (instruments agree) | 0.865 | 0.862 | 0.24x | 0.24x |
| 64 | 7.786 | 0.39x | 0.39x | 8.5% **archive-stale** | time (instruments agree) | 0.865 | 0.860 | 0.34x | 0.34x |
| 128 | 15.14 | 0.76x | 0.74x | 1.9% tight | time | 0.869 | 0.856 | 0.66x | 0.64x |
| 256 | 31.13 | 1.56x | 1.40x | 4.9% tight | time | 0.861 | 0.848 | 1.34x | 1.20x |
| 512 | 96.01 | 4.87x | 3.56x | 3.1% tight | time | 0.863 | 0.790 | 4.20x | 3.08x |

### What to do with this

1. Restate every Burgers and Poisson speedup against a **tolerance-based** FOM, with the
   tolerance named. `tau = 1e-10` is the **tightest available engineering rung** — it is not
   accuracy-matched to what the old baselines actually reached (they are 25–1300x tighter on
   Poisson and ~100x tighter on Burgers); `1e-8` is the more defensible engineering choice and
   makes the correction larger still.
2. On **Burgers**, do not apply a single scalar: the multiplier ranges 0.209 to
   0.267 (28% spread) and changes the ladder's slope as well as
   its level. On **Poisson** the multiplier is close to constant across meshes
   (0.861–0.869 as per-mesh means, 0.93% spread; the
   raw spread over every repeated rung is 2.7% but that mixes in
   rung-to-rung timing noise), so a single scalar there is a fair approximation — the blanket "never use a scalar" rule holds for Burgers,
...
Every result row additionally carries a **sha256 of every `wsf_*.py`** (so a dirty tree
cannot publish two different codes under one commit hash), plus `gpu`, `gpu_kind`,
`jax_backend`, `slurm_job_id`, `seed`, and `run_role`.

> **Known provenance defect in these runs, found by the results audit.** The `commit` field
> *inside* the eleven reports reads `72b44d40c86c…`, which is **not an object in
> this repository**. `wsf_util.provenance` called `git -C <staged code dir>`; on the cluster
> that directory is not a git repository, so discovery walked up into an unrelated ancestor
...
|---|---|---|---|---|---|---|---|
| `wsb_cons` | consolidated | `2511371` | NVIDIA A100 80GB PCIe | gpu | `63de48bdd5dd` | highest | 12 |
| `wsb_n128` | panel | `2511381` | NVIDIA A100 80GB PCIe | gpu | `63de48bdd5dd` | highest | 3 |
| `wsb_n256` | panel | `2511377` | NVIDIA A100 80GB PCIe | gpu | `63de48bdd5dd` | highest | 3 |
| `wsb_n32` | panel | `2511390` | NVIDIA A100 80GB PCIe | gpu | `63de48bdd5dd` | highest | 3 |
| `wsb_n64` | panel | `2511388` | NVIDIA A100 80GB PCIe | gpu | `63de48bdd5dd` | highest | 3 |
| `wsp_cons` | consolidated | `2511369` | NVIDIA A100 80GB PCIe | gpu | `63de48bdd5dd` | highest | 75 |
| `wsp_n128` | panel | `2511382` | NVIDIA A100 80GB PCIe | gpu | `63de48bdd5dd` | highest | 15 |
| `wsp_n256` | panel | `2511379` | NVIDIA A100 80GB PCIe | gpu | `63de48bdd5dd` | highest | 15 |
| `wsp_n32` | panel | `2511392` | NVIDIA A100 80GB PCIe | gpu | `63de48bdd5dd` | highest | 15 |
| `wsp_n512` | panel | `2511373` | NVIDIA A100 80GB PCIe | gpu | `63de48bdd5dd` | highest | 15 |
| `wsp_n64` | panel | `2511389` | NVIDIA A100 80GB PCIe | gpu | `63de48bdd5dd` | highest | 15 |
