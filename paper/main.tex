...
% =====================================================================
\begin{abstract}
Neural operators, including Fourier Neural Operators and DeepONets,
offer one accuracy--speed setting per trained
model; changing it requires retraining. We present a
nonlinear-manifold reduced-order model (NM-ROM) for Poisson, heat and
viscous Burgers equations with an accuracy--speed
tradeoff from one decoder, set after training by
...
evaluation; and non-negative least-squares (NNLS) empirical quadrature for Burgers advection.
The $4096^2$ rule passes checks on states reached in held-out trajectories but is not confirmed on independent re-draws. Across Poisson and heat in two and
...
tight bar $\nEqtopTightBar$. We repeat the check with independent trajectory
samples (re-draws): a rule is \emph{confirmed} only if every re-draw passes
...
quadrature labels are in Appendix~\ref{app:cg-current}. Dashes mark
...
confirmed on independent re-draws (Appendix~\ref{app:cg-current}).
...
Table~\ref{tab:headline} shows slower query-time growth with mesh size
...
fixed tight setting wherever one was timed (Table~\ref{tab:headline-times}).
...
Table~\ref{tab:ladder-main}), $q>0$ with the lattice rule marked
$^{w}$ in Table~\ref{tab:headline}. Each block uses one FOM time for all
speedups, so its $q=0$ ratio differs from Table~\ref{tab:headline}'s fast column.
...
On Burgers at $4096^2$, changing the correction rank trades accuracy for cost. Table~\ref{tab:tunability} compares every setting with one
shared Newton--BiCGStab setting
($\nTuneBurgDevSpeedMin$--$\nTuneBurgDevSpeedMax\times$ on development
cases). Table~\ref{tab:headline}'s fast column is a different comparison:
...
Table~\ref{tab:headline}. Settings with unconfirmed quadrature reach
$\nBhSubErrLo$--$\nBhSubErrHi\,\%$ error at
...
Nested corrections give one trained NM-ROM an accuracy--cost tradeoff
after training. Accuracy persists under mesh refinement on
two-dimensional Poisson, heat and Burgers, while speedup over the named
iterative solvers grows. At $4096^2$ the accurate setting
reaches $\nHeadPoissonAccErr\,\%$ at $\nHiresPoissonAccSFortyNinetySix\times$
on Poisson, $\nHeatWideAccErr\,\%$ at $\nHeatWideAccS\times$ on held-out
heat and $\nBurgHoldAccErrFortyNinetySix\,\%$ at
$\nBurgHoldAccSFortyNinetySix\times$ on held-out Burgers. It is more
accurate than a reproduced NM-ROM and POD-LSPG. The Burgers rule passes held-out checks but fails independent
confirmation. The confirmed variant uses the exact first-step residual
and is slower than the FOM. The named solvers remain
...
Future work includes a better three-dimensional bank, a cheaper high-rank
