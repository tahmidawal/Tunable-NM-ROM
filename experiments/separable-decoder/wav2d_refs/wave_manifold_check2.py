...
OMF=float(sys.argv[4]) if len(sys.argv)>4 else 0.0
p=16
Aw=Vt_[K:K+p].T                                  # next POD modes, orthogonal to V
zs=Z.std(axis=0)
rw=np.random.default_rng(3)
Om=rw.normal(size=(p,K))*OMF/zs[None,:]
phi=rw.uniform(0,2*np.pi,size=p)
amp=EPS*np.sqrt(np.mean(np.sum(X**2,axis=1)))/np.sqrt(p)
def g(z): return V@z + W@q(z) + amp*(Aw@np.sin(Om@z+phi))
def Jg(z): return V + W@dq(z) + amp*(Aw@(np.cos(Om@z+phi)[:,None]*Om))
print(f"wrinkle eps={EPS} freq={OMF}: rel amplitude {amp*np.sqrt(p)/np.sqrt(np.mean(np.sum(X**2,axis=1))):.3f}")
...
# ---- tangent-space velocity residual at oracle-projected held-out states ------------
# v_true from the FOM substep (central difference on a fine CN re-run); tangent residual ||(I-P_T) v||/||v||
tr=[]; pr=[]
for i in range(ntest):
    Sf=fom(Ute[i,0], subs=80)   # snapshots only; velocity via CN at snapshot times -> reconstruct with fine FOM
    # rerun to get v at snapshots
    h=dt_snap/80; a2=(c*h/2)**2; A=np.eye(n)-a2*L; B=np.eye(n)+a2*L; Ainv=np.linalg.inv(A)
    u=Ute[i,0].copy(); v=np.zeros(n)
    for k in range(nsnap):
        for _ in range(80):
            u1=Ainv@(B@u+h*v); v1=v+(h*c**2/2)*(L@(u+u1)); u,v=u1,v1
        if k%10==9:
            z=proj(u,u@V); J=Jg(z); Qj,_=np.linalg.qr(J)
            tr.append(np.linalg.norm(v-Qj@(Qj.T@v))/np.linalg.norm(v))
            pr.append(np.linalg.norm(v-V@(V.T@v))/np.linalg.norm(v))
print(f"tangent-space velocity residual (held-out, oracle points): manifold {np.mean(tr):.3f}   POD-K {np.mean(pr):.3f}")
