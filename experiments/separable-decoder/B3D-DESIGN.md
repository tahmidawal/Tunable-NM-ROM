...
| F8 2D-vs-3D `[A17, A38]` | state $v(x,y)p(z)$, $p = 1$ on the three central planes tapering to the faces, $v$ sign-changing; on the middle plane the 3D residual's **interior rows** and its JVP with the **lifted tangent** $\delta v\,p$ equal the actual 2D code's (`burgers2d_film.make_rollout(n).residual`, interior rows) | $\le 10^{-13}$ backward-normalised | **$x$-advection coefficient doubled** (a non-zero term on the plane; the $z$-Laplacian mutation of r2 was inert and is retracted; × 1.01 gave $1.9\times10^{-5}$ under the backward-error normalisation, below the bar, so the deterministic mutation is × 2) |
