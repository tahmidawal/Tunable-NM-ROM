...
def run_and_benchmark():
    ...
    peak_lr        = 2e-3
    weight_decay   = 5e-4
    # Pareto-champion config (Run8): max_iters=2, gn_tol=1e-2 → val 7.85e-2 / spd 269x
    # See results.tsv for the full 4-point frontier (Run0/Run5/Run6/Run8). All
    # strictly Pareto-dominate N32_fixed (val 9.30e-2 / spd 176x) on BOTH axes.
    max_iters      = 2
    gn_tol         = 1e-2
    cg_tol         = 1e-6
    cg_iters       = 1000
    eq_mode        = 'nnls'
    n_eq_samples   = 32
    outdir         = Path('/cluster/tufts/paralab/tawal01/NMROM-Apr8/20260423-NEURIPS/Autoresearch/Heat-3D/runs/N64_fixed_run3')
    seed           = 0
    # ───────────────────────────────────────────────────────────────────────
    ...
    finetune_mode = (not ckpt_path.exists()) and ('finetune_init_ckpt' in dir()) and finetune_init_ckpt.exists()
    if ckpt_path.exists():
        print(f"[ckpt-skip] Found checkpoint at {ckpt_path} — loading and skipping training")
        with open(ckpt_path, 'rb') as f:
            _ckpt = pickle.load(f)
        params = _ckpt['params']
        best_val = float(_ckpt.get('train_meta', {}).get('best_val', float('nan')))
        training_seconds = 0.0
        print(f"[ckpt-skip] Loaded params. best_val from meta: {best_val:.4e}")
        # recompute val loss to confirm
        REL_EPS = 1e-6
        v_sum = 0.0
        ...
        # Fine-tune branch: load init params from finetune_init_ckpt, then train num_epochs
        # at peak_lr (typically lower). Saves to outdir/checkpoint_vitcp.pkl on completion.
        print(f"[finetune] Loading init from {finetune_init_ckpt}")
        with open(finetune_init_ckpt, 'rb') as f:
            _ckpt = pickle.load(f)
        params = _ckpt['params']
        REL_EPS = 1e-6
        ...
        print(f"[finetune] Training {num_epochs} epochs batch={batch_size} peak_lr={peak_lr}")
        t_train_start = time.perf_counter()
        jkey = jax.random.PRNGKey(2 + seed); np_rng = np.random.default_rng(2 + seed)
        # Initialise best_val from current params (Run0 baseline)
        v_sum = 0.0
        for s in range(0, n_val_data, 256):
            vb = jnp.asarray(U_val[s:s+256])
            preds = jax.vmap(lambda u: model.apply({'params': params}, u, training=False))(vb)
        ...
        best_val = v_sum / n_val_data
        best_params = params
        print(f"[finetune] Init val_rel_l2 = {best_val:.4e}")
    ...
    if eq_mode == 'full':
        ...
        # CP-structured EQ: avoids full N^3 x k Jacobian (OOM at N=64).
        # J_D[n,m] = sum_r Jh[r,m] * V_all[r,n]  where Jh=(rank,k) is tiny.
        # Restrict to interior nodes only; G shape = (n_pairs*k, N_int).
        _int_idx_np = np.where(np.array(mask) > 0)[0]
        _int_idx = jnp.array(_int_idx_np)
        N2_ = N * N
        _ix = _int_idx_np // N2_; _iy = (_int_idx_np // N) % N; _iz = _int_idx_np % N
        _Wx = params['decoder']['W_x']
        _Wy = params['decoder']['W_y']
        _Wz = params['decoder']['W_z']
        # V_int: (rank, N_int) as numpy — kept off-GPU to avoid XLA constant folding hang
        V_int_np = np.array(_Wx[:, _ix] * _Wy[:, _iy] * _Wz[:, _iz])  # (rank, N_int)
        print(f"V_int: {V_int_np.shape}  (rank x N_interior)")

        # Pull decoder weights to numpy once — fully avoids JAX JIT for Jh
        _dec = params['decoder']
        _W1  = np.array(_dec['W1']['kernel']);  _b1  = np.array(_dec['W1']['bias'])
        _W2  = np.array(_dec['W2']['kernel']);  _b2  = np.array(_dec['W2']['bias'])
        _Wr  = np.array(_dec['W_rank']['kernel']); _br = np.array(_dec['W_rank']['bias'])
        _Wd  = np.array(_dec['W_direct']['kernel'])
        ...
        def _jh_numpy(z_np):
            # z_np: (k,); W1:(k,h), W2:(h,h), Wr:(h,rank), Wd:(k,rank)
            a1 = z_np @ _W1 + _b1          # (h,)
            s1 = _swish_np(a1)
            a2 = s1 @ _W2 + _b2            # (h,)
            ds1 = _dswish_np(a1)            # (h,)
            ds2 = _dswish_np(a2)            # (h,)
            # Jh = Wd.T + Wr.T @ diag(ds2) @ W2.T @ diag(ds1) @ W1.T
            # A @ diag(v) == A * v[None,:]; shapes: (rank,k)
            Jh = _Wd.T + (_Wr.T * ds2[None, :]) @ (_W2.T * ds1[None, :]) @ _W1.T
            return Jh   # (rank, k)

        # Encode EQ snapshots using JAX eager mode (jax.disable_jit) — GPU dispatch without
        # XLA compilation. Fast on GPU, no minutes-long compile hang.
        eq_u_next_np = np.stack([p[0] for p in eq_pairs])             # (n_eq, N)
        eq_u_prev_np = np.stack([p[1] for p in eq_pairs])             # (n_eq, N)
        eq_kappas    = np.array([p[2] for p in eq_pairs], dtype=np.float32)

        _Wx_np = np.array(_dec['W_x']); _Wy_np = np.array(_dec['W_y']); _Wz_np = np.array(_dec['W_z'])
        _bias_sc = float(_dec['bias'])
        ...
        with jax.disable_jit():
            for i, u_flat in enumerate(eq_u_next_np):
                ...
                z_i, scale_i = model.apply({'params': params}, u_jnp,
                                           training=False, method=model.encode)
                u_pred_i = model.apply({'params': params}, z_i,
                                       method=model.decode_normalised)
                z_np_all.append(np.array(z_i))
                u_pred_np.append(np.array(u_pred_i))
                if i % 8 == 0:
                    print(f"  EQ encode {i+1}/{len(eq_u_next_np)}")
        z_np_all  = np.stack(z_np_all)    # (n_eq, k)
        u_pred_np = np.stack(u_pred_np)   # (n_eq, N)
        # normalise u_prev in numpy
        u_prev_abs = np.max(np.abs(eq_u_prev_np), axis=1, keepdims=True) + AMP_EPS
        u_prev_np  = eq_u_prev_np / u_prev_abs                        # (n_eq, N)

        # Compute residuals and Jh entirely in numpy
        Jh_list = []
        R_list  = []
        mask_np = np.array(mask)
        u_g_np  = np.array(u_g)
        for i in range(len(eq_pairs)):
            u_p = mask_np * u_pred_np[i] + u_g_np                    # constrained decode
            kap = eq_kappas[i]
            # K_op_3d in numpy
            u3 = u_p.reshape(N, N, N)
            lap = np.zeros_like(u3)
            lap[1:-1,1:-1,1:-1] = (
                6*u3[1:-1,1:-1,1:-1]
                - u3[:-2,1:-1,1:-1] - u3[2:,1:-1,1:-1]
                - u3[1:-1,:-2,1:-1] - u3[1:-1,2:,1:-1]
                - u3[1:-1,1:-1,:-2] - u3[1:-1,1:-1,2:]
            ) / dx**2
            R = u_p - u_prev_np[i] + dt * kap * lap.flatten()
            R_list.append(R[_int_idx_np].astype(np.float32))         # (N_int,)
            Jh_list.append(_jh_numpy(z_np_all[i]))                   # (rank, k)

        # One batched BLAS call: (n_pairs*k, rank) @ (rank, N_int) = (n_pairs*k, N_int)
        Jh_stack = np.concatenate([Jh.T for Jh in Jh_list], axis=0)  # (n_pairs*k, rank)
        JD_stack = Jh_stack @ V_int_np                                 # (n_pairs*k, N_int)
        R_stack  = np.repeat(np.stack(R_list), k_dim, axis=0)         # (n_pairs*k, N_int)
        G_train  = (JD_stack * R_stack).astype(np.float32)            # (n_pairs*k, N_int)
        G_np = np.array(G_train)
        b_np = np.sum(G_np, axis=1)
        w_int, _ = nnls(G_np, b_np, maxiter=3 * G_np.shape[0])
