...
VARIANTS = os.environ.get("VARIANTS", "inc,adv,path,grad").split(",")
...
def main():
    ...
    OUT = f"{OUT_PREFIX}sep_eq_gradfit_{tag}.json"
    ...
    report = dict(config=dict(
        pde="burgers2d", kind="eq_gradfit_3way", N=N, k=K, r=R,
        ...
        grad_w=GRAD_W, step_tol=STEP_TOL, stall=STALL,
    ...
    for (zr, zpr, nur, _, _, _) in row_states:
        ...
        _, J_f, _, _ = fo["rJ"](G_int, fo["Phi"], jnp.asarray(zr, dtype=F64),
                                pf, nur)
        wt_r = (1.0 + bc.DT * nur * lam_np) ** (-bc.WEAK_ALPHA)
        grad_blocks.append(xc.build_grad_row_block(
            np.asarray(J_f), wt_r, bc.DT, state_blocks[-1][0],
            state_blocks[-1][1]))
    sc.log(f"  built {len(state_blocks)} state row blocks "
           f"(+{len(grad_blocks)} grad blocks) [{time.time()-t_rows:.0f}s]")
    ...
    if "grad" in VARIANTS:
        ...
        build_variant("grad", keep, wq_np, eq_info)
