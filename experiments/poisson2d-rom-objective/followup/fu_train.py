"""Follow-up variant of pro_train.py: (re)train a stage-0 FiLM AUTO-DECODER (same recipe as
...
# TRAINING seed (net init, latent init, batch sampling).  Default = the data seed ->
# bit-identical to pro_train.py.  The DATA draw always uses mp.SEED.
TRAIN_SEED = int(os.environ.get("TRAIN_SEED", str(mp.SEED)))
OUTDIR = sys.argv[1]
...
def main():
    ...
    report = dict(config=cfg, fom_max_rel_residual=fom_res, eps0=eps0, n_freq=n_freq,
                  ...
                  complete=True)
    tag = f"K{K_LAT}_N{N}" + ("_hbc" if HARD_BC else "") + (f"_S{TRAIN_SEED}" if TRAIN_SEED != mp.SEED else "")  # cfg['hard_bc'] is authoritative for consumers
