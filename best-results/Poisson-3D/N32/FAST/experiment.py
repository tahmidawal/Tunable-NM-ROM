...
def run_and_benchmark():
    ...
    cg_tol        = 1e-3
    cg_iters      = 8
    lm_factor     = 5e-2   # bisect: lm=1e-2 diverges, lm=1e-1 over-damps; case1 hit 4.1e-3 at 1e-1
    eq_mode       = 'nnls'   # 'nnls' or 'full'
    min_eq_points = 2000
    n_eq_snaps    = 200
    outdir        = Path('/cluster/tufts/paralab/tawal01/NMROM-Apr8/20260423-NEURIPS/Autoresearch/Poisson-3D/runs/N32_run21')
    seed          = 42
    # reuse Run14 k=32 checkpoint (val=1.67e-3); EQ-only
    load_checkpoint_from = Path('/cluster/tufts/paralab/tawal01/NMROM-Apr8/20260423-NEURIPS/Autoresearch/Poisson-3D/runs/N32_run14/checkpoint.pkl')
    # ───────────────────────────────────────────────────────────────────────
    ...
    # ── Training (or checkpoint reuse) ────────────────────────────────────
    if load_checkpoint_from is not None and load_checkpoint_from.exists():
        print(f"Loading checkpoint from {load_checkpoint_from} — skipping AE training")
        with open(load_checkpoint_from, 'rb') as f:
        ...
        best_val = chunked_rec_loss(params, U_val)
        print(f"Loaded checkpoint val: {best_val:.4e}")
    else:
        schedule = optax.warmup_cosine_decay_schedule(
    ...
    if len(eq_indices) < min_eq_points:
        # Restrict fallback to interior nodes only — boundary nodes break the stencil
        w_int_ranked = np.argsort(w_eq_int)[::-1][:min_eq_points]
        top_idx = interior_indices[w_int_ranked]
        eq_indices = np.unique(np.concatenate([eq_indices, top_idx]))
    eq_weights = w_eq_full[eq_indices]
    eq_weights = eq_weights / eq_weights.sum()  # normalise to sum=1
    n_eq = len(eq_indices)
    ...
    def make_rom_solver_eq(_max_iters, _gn_tol):
        ...
        def rom_solve(z_init, F_eq):
            ...
            def body_fn(state):
                ...
                lam = jnp.maximum(lm_factor * jnp.trace(JtWJ) / k_dim, 1e-8)
