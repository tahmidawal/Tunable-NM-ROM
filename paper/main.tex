...
\paragraph{Head.} $\head(\latent)=\varphi_\theta(\latent)+W\T\latent$, with
...
(Tables~\ref{tab:problems} and~\ref{tab:config3d}). The skip is a design
choice, not ablated here.

\paragraph{Bank.} Every Dirichlet bank is a coordinate network times a
...
\paragraph{Corrections.} $\corr$ holds leading singular directions of
...
Each row of Table~\ref{tab:headline} names one frozen model (two-dimensional
...
step, and (arm by arm, Table~\ref{tab:headline-times}) solve the normal
...
Table~\ref{tab:knobs-main} summarises the deployment-time controls.
Correction rank is the accuracy control (with a smaller fixed test space
the same ranks give only $\nQxmTwoFiftySixErrSpan\times$, so test count is
