...
EXLIN = int(os.environ.get("EXLIN", "0"))
EQ_ADV_ONLY = int(os.environ.get("EQ_ADV_ONLY", "0"))
...
def main():
    ...
    report = dict(config=dict(
        ...
        eq_tail_rounds=EQ_TAIL_ROUNDS, exlin=bool(EXLIN),
        eq_adv_only=bool(EQ_ADV_ONLY), step_tol=STEP_TOL, stall=STALL,
    ...
    for Mi in EQ_MS:
        ...
        # A = Phi^T G_int, the (M, R) exact-linear matrix (EXLIN only).
        A_j = (jnp.asarray(np.asarray(Phi)).T @ G_int) if EXLIN else None
        del Phi
        ...
        if EXLIN:
            # -------- exact-linear sampled ops (exp/2026-08-26-eq-learned) --
            # lin = A(h(z) - h(z_prev)) + DT*nu*lam*A h(z) EXACTLY; advection
            # stays on the m nodes via G_st/Phi_q, byte-identical code to the
            # incumbent closure above.  prev_c becomes the M-vector A h(zn).
            ...
            # gate L: exlin linear part == full-grid linear part (exactness)
            # gate A: exlin advection part == incumbent advection part
            gL, gA = [], []
            for _ in range(5):
                zt = jnp.asarray(Z_tr[grng.integers(len(Z_tr))]
                                 + 0.05 * grng.standard_normal(K))
