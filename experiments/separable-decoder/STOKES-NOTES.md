...
**It was predicted to win, and it did.** Across the whole mesh ladder the direct $G$-span solve is more accurate than the nonlinear arm by 6.9e+09x to 6.5e+11x (worst-mesh aggregate held-out error 2.071e-12 at N=256) and cheaper by 5241x to 8663x per query. Both comparisons hold at every mesh. **That is the expected outcome — not a failure of the machinery and not a result.**
<!-- END GENERATED -->
...
<!-- generated by stk2d_rom_tables.py from stk2d_recon_gate_recon_nu1.json (commit 48413c6c863d) and stk2d_rom_gates_rom_nu1.json (commit 48413c6c863d) -- do not edit by hand -->
...
|---|---|---|---|---|
| FOM back-substitution (as this cell's solver reports it) | 1.374 | 11.459 | 90.139 | 408.693 |
| FOM back-substitution, bare (no backward-error bookkeeping) | 1.380 | 11.044 | 81.359 | 404.008 |
| ROM, quadrature-free residual | 24.751 | 31.566 | 31.743 | 51.804 |
| ROM, full-grid residual | 111.043 | 255.221 | 3568.183 | 16224.363 |
| ROM, EQ residual | 52.167 | 50.829 | 64.960 | 72.156 |
| direct G-span solve (S7c) | 0.005 | 0.005 | 0.005 | 0.006 |
| POD-K direct solve (S7a) | 0.004 | 0.004 | 0.004 | 0.005 |
| POD-R Galerkin (S7b) | 0.005 | 0.005 | 0.005 | 0.006 |
| ROM, quadrature-free, NON-AFFINE force (separate arm) | 34.193 | 52.551 | 62.010 | 59.753 |
...
|---|---|---|---|---|---|---|
| 32 | 0.01 | 2.8 | 0.1 | 2.117e-17 | 7.148e-17 | 1.035e+00 |
| 64 | 0.02 | 8.6 | 0.7 | 3.250e-17 | 1.258e-16 | 1.033e+00 |
| 128 | 0.06 | 7.2 | 6.9 | 2.990e-17 | 2.825e-16 | 1.025e+00 |
| 256 | 0.27 | 9.4 | 80.1 | 2.699e-16 | 5.391e-16 | 1.018e+00 |
...
runs: `stk2d_recon_gate_recon_nu1.json` (rev 1, complete=True certified=True, 3258 s) and `stk2d_rom_gates_rom_nu1.json` (rev 1, complete=True certified=True, 1634 s) | commit `48413c6c863d` | host `spark-d69e` | numpy 2.4.4 scipy 1.17.1 | jax backend `gpu` x64=True matmul `highest`
...
29. **PROCESS: I edited `stk2d_recon_gate.py` while a certified run of it was in
    flight.** The run was killed, the artifact and the heads deleted, the code
    committed, and the run restarted from the committed source; nothing from
    the aborted run is reported anywhere. It is recorded because the
    `git_commit` field this project relies on for provenance **would not have
    caught it** — uncommitted edits do not change `HEAD`, so the artifact would
    have carried a hash that did not describe the code that produced it.
...
without a certified, passing stop-gate artifact. The FOM snapshots are cached
