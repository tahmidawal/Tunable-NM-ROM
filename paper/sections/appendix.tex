...
once and gives every wide-bank row of Table~\ref{tab:headline}; the
development row ran on another GPU (A100) and is shown for reference.
Wide bank: $R=128$, $k=8$; earlier: the $R=32$ checkpoint of the
Heat rows; 3D: the Heat 3D model of Table~\ref{tab:failures}. The 3D rows
...
is job \provQxmWithinJob, each with its own measured error); the last column is dense
