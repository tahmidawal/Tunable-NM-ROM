...
One row = one frozen model evaluated at two deployment settings:
  fast      the fastest retained NM-ROM setting that meets its stopping rule
  accurate  the retained NM-ROM setting with the smallest worst-case error
...
for m, lab_ in (('head_q0', '$q=0$'), ('nested_q32', '$q=32$')):
    ...
    F.append(dict(problem='Reflective wave 2D, $1024^2$ (dev.)', setting=lab_, error_pct=100 * W[m]['worst_energy_state'], fom_error_pct=100 * W[wf]['worst_energy_state'],
...
fl = [r'% GENERATED by paper/gen_headline.py -- do not edit.', r'\small', r'\begin{tabular}{@{}llrrrl@{}}', r'\toprule',
      r'Problem & Setting & NM-ROM err.\ (\%) & FOM err.\ (\%) & Speedup & FOM \\', r'\midrule']
fmd = []; seen = None
for r in F:
...
(HERE / 'tables/headline-provenance.json').write_text(json.dumps(dict(
    rule='One frozen model per row. fast = fastest retained setting meeting its stopping rule; accurate = smallest worst-case error. '
