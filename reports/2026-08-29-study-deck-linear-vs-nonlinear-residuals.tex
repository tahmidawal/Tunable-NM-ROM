...
\begin{frame}{Seeing it (2/2): where the arms differ --- pointwise errors and error over time}
  \centering
  \includegraphics[height=0.72\textheight,trim=0 0 0 40,clip]{figs/b1d-tensor-errors-n512.png}
  \vspace{0.5mm}\footnotesize
  Worst trajectory. Top: $|\text{ROM}-\text{truth}|$ --- tensor and oracle are one curve; NNLS-32 spikes higher at the front. Middle: $|\text{tensor}-\text{oracle}|$ on its own scale, $10^{-8}$--$10^{-7}$. Bottom: relative error per time step (two trajectories and the 8-trajectory mean) --- tensor $\equiv$ oracle; NNLS-32 drifts above after step $\sim$10.
\end{frame}
