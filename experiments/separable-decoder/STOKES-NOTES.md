> **This file holds three reports.** Phase 1 (the MAC full-order solver and its
...
> manifold-richness verdict) starts at *"Stokes 2D — phase 2a"*; **phase 2b**
...
# Stokes 2D — phase 2b: the vector-valued decoder, the quadrature-free residual, and the reduced solve

Phase 2b of the 2026-08-30 Stokes cell (`exp/2026-08-30-stokes-vector`), covering
the decoder $\mathbf{u}=\bar{\mathbf{u}}+G\,h(z)$, the quadrature-free residual,
and gates **S4**, **S6**, **S7**, **S8**, **S9** — plus the **reconstruction stop
gate** that had to clear before any of them could run. It builds on the phase-1
FOM and the phase-2a bank without modifying a single operator, solver or
bank-construction routine in either.

**Status of the numbers below: final.** Every one is generated from
`runs/stk2d/stk2d_recon_gate_recon_nu1.json` and
`runs/stk2d/stk2d_rom_gates_rom_nu1.json` by `stk2d_rom_tables.py`; none is
typed by hand. Every gate is recorded as a **number** and enforced by an
assertion, and every gate has a **negative control that was run and made to
fire**.

## Read this before the numbers: what this cell is for

`STOKES-DESIGN.md`, unchanged since revision 1: *"This cell is a de-risking
rehearsal for 2D incompressible Navier–Stokes. It is not expected to produce a
positive result, and it must not be written up as one."*

The reason is structural, not empirical. Steady Stokes is **linear** and, after
pressure elimination, the velocity is linear in the force amplitudes. $G$ is
itself a POD basis of the response, so a **direct linear reduced solve in the
$G$ span is a one-shot projection**: assemble $b(\mu)$ in $O(MQ)$ and apply a
precomputed $R\times M$ pseudo-inverse. That control is gate **S7c**.
<!-- BEGIN GENERATED (stk2d_rom_tables.py s7c) -->
**It was predicted to win, and it did.** Across the whole mesh ladder the direct $G$-span solve is more accurate than the nonlinear arm by 6.9e+09x to 6.5e+11x (worst-mesh aggregate held-out error 2.071e-12 at N=256) and cheaper by 5387x to 8940x per query. Both comparisons hold at every mesh. **That is the expected outcome — not a failure of the machinery and not a result.**
<!-- END GENERATED -->

What phase 2b delivers is the *machinery*, gated: a vector-valued
divergence-free decoder with no `bc(x)` mask, a residual with the pressure
eliminated exactly and analytically, an independent full-grid implementation to
gate it against, a strong-form empirical-quadrature arm defined from scratch,
and honest cost numbers for all of them. Everything except the (absent)
convection tensor carries to Navier–Stokes.

## The stop gate, first, because it is what phase 2b was gated on

<!-- BEGIN GENERATED PHASE2B VERDICT (stk2d_rom_tables.py verdict) -->
<!-- generated by stk2d_rom_tables.py verdict from stk2d_recon_gate_recon_nu1.json (commit 48413c6c863d) -- do not edit by hand -->

**VERDICT: PASS.** At the predeclared primary rung (N=64, R=32, K=8, training form `auto`, fit cohort 16384), the held-out nonlinear reconstruction oracle reads **9.8687e-03** in aggregate against POD-8's 3.8486e-02 -- a **3.90x** gain against a required 3x -- and **7.9965e-03** in the per-case median against POD-8's 3.7301e-02, a **4.66x** gain.  The per-case maximum is 5.3431e-02.

**Where the head actually stops.** This is the question phase 2a could not answer, and the answer is a number: the finite MLP head stops at 9.869e-03 aggregate, which is ABOVE the same bank's linear POD-16 error of 3.953e-03 (by a factor of 2.50) and 3.90x better than POD-8 at 3.849e-02.  It is NOT anywhere near POD-32's bank-span ceiling, and phase 2a's remark that the head had 'ten orders of headroom' is exactly the overclaim the verification flagged: the headroom is real, the head does not reach it.

<!-- END GENERATED PHASE2B VERDICT -->

## What was built

- **`stk2d_rom.py`** — the mesh cell (phase-2a snapshots plus the phase-2a
  reorthogonalised bank plus the exact affine coefficient map), the numpy head
  and its analytic Jacobian, the precomputed quadrature-free reduced problem,
  the **independent** matrix-free full-grid residual and Jacobian, the damped
  LM solver, the three linear control arms, the strong-form empirical-quadrature
  machinery, the physical non-affine force family, and the balanced-order
  timing harness.
- **`stk2d_head.py`** — the JAX head, both training forms, save/load with
  provenance, and the batched multi-start reconstruction oracle.
- **`stk2d_recon_gate.py`** — the stop-gate driver. Reconstruction only: no
  residual, no ROM, no timing.
- **`stk2d_rom_gates.py`** — the ROM driver. **It refuses to run** unless the
  stop-gate artifact exists, is certified, and records `verdict.passed`.
- **`stk2d_rom_tables.py`** — generates every table below from those two
  JSONs, and the stop-gate verdict paragraph too, so no number in this report
  is hand-typed.

The FOM snapshots and the trained heads are **caches with recorded
provenance**, not results: each is keyed by its entire configuration, deleting
them reproduces everything from the seed, and every head carries the flags
(`smoke`, `producer`, mesh, architecture, steps, fit cohort, seed) that the ROM
driver asserts before using it. The artifact records, per rung, whether a head
was trained in that run or loaded.

```mermaid
flowchart TB
  subgraph offline["OFFLINE, once per mesh"]
    FOM["48 dictionary FOM solves<br/>+ 256 train + 64 held-out"]:::solved
    BANK["phase-2a psi-route bank G<br/>(metric-reorthogonalised)"]:::frozen
    CD["Cd = G^T M_u U_dict, cbar<br/>c(mu) = Cd theta(mu) - cbar EXACT"]:::param
    TRAIN["head h: R^8 -> R^R<br/>trained in COEFFICIENT space"]:::head
    PRE["A = Phi^T M_u L G   (M x R)<br/>b_q = Phi^T M_u f_q  (M x Q)<br/>Phi^T M_u L ubar     (M)"]:::param
  end
  subgraph online["ONLINE, per query mu"]
    B["b(mu) = sum_q theta_q(mu) b_q   O(MQ)"]:::param
    LM["LM over z:<br/>r(z) = -nu(Phi^T M_u L ubar + A h(z)) - b(mu)"]:::head
    U["u = ubar + G h(z*)"]:::solved
  end
  FOM --> BANK --> CD --> TRAIN
  BANK --> PRE
  PRE --> B --> LM --> U
  ...
  classDef frozen fill:#5b5b5b,stroke:#2b2b2b,color:#fff
  classDef param fill:#1f5f8b,stroke:#0d2f45,color:#fff
  classDef head fill:#6a4c93,stroke:#33254a,color:#fff
```

### The decoder, and the one identity everything rests on

$$\mathbf{u}(z)=\bar{\mathbf{u}}+G\,h(z),\qquad G\in\mathbb{R}^{n_u\times R},
\qquad h:\mathbb{R}^{K}\to\mathbb{R}^{R},\quad K=8 .$$

**There is no `bc(x)` mask**, as the design requires: multiplying a
divergence-free field by a scalar mask destroys divergence-freeness, since
$\nabla\!\cdot(\mathrm{bc}\cdot Gh)=\nabla\mathrm{bc}\cdot(Gh)+\mathrm{bc}\,
\nabla\!\cdot(Gh)$ and the first term is not zero. The bank already carries both
properties: every column is $C\psi$ with $\psi$ vanishing on the boundary
vertices, and the boundary-normal velocities are eliminated degrees of freedom,
so no-slip is structural; and phase 2a gated $\lVert Dg_i\rVert\sim10^{-19}$ per
mode.

Because $G$ is $M_u$-**orthonormal** (phase 2a reorthogonalised it to
$1.3\times10^{-15}$), the whole decoder problem collapses to coefficient space:

$$\lVert \mathbf{u}-\bar{\mathbf{u}}-G h(z)\rVert_M^2
= \lVert c-h(z)\rVert_2^2 + \lVert \mathrm{perp}\rVert_M^2,\qquad
c = G^\top M_u(\mathbf{u}-\bar{\mathbf{u}}).$$

Three consequences, all of which shape how the tables must be read.

1. The head is trained and evaluated on 32-dimensional coefficient vectors, not
   on fields. **Gate S-METRIC asserts that identity against a direct field
   computation** rather than assuming it.
2. $\lVert\mathrm{perp}\rVert_M$ is the **POD-$R$ truncation floor** and is
   completely independent of the head. The decoder's reconstruction error is
   therefore **bounded below by POD-$R$ at its own $R$, exactly**. At $R=K=8$
   the head *cannot* beat POD-8, which is why the $R$ ladder is not a
   like-for-like comparison and why the stop gate is stated at $R=32$.
3. The truncation floor must be formed **explicitly**, as
   $X-G(G^\top M_u X)$. Computing it as $\lVert x\rVert^2-\lVert c\rVert^2$
   costs twelve orders of magnitude and returns a plausible-looking wrong
   number — see retraction 24.

### The training data is free, and exactly so

Steady Stokes is linear and the dictionary is affine, so phase 2a's gated
superposition identity $\mathbf{u}(\mu)=U_{\text{dict}}\theta(\mu)$ carries into
coefficient space **exactly**:

$$c(\mu)=C_d\,\theta(\mu)-\bar c,\qquad C_d=G^\top M_u U_{\text{dict}},
\qquad \bar c = G^\top M_u\bar{\mathbf{u}} .$$

Once the 48 dictionary responses are solved, any number of training coefficient
vectors costs one 48-column matvec each. The head's fit cohort is therefore
16384 parameter samples at **no additional FOM cost**, while the **bank is still
built from the frozen phase-2a 256-snapshot training set and is bit-for-bit the
phase-2a bank** — so carried-forward condition 3 (rerun S1 if the bank changes)
is not triggered. Gate **S-AFFINE** checks $c(\mu)$ against coefficients taken
from direct FOM solves at phase 2a's own $10^{-14}N^2$ budget, and gate
**S-REGR** checks that this cell's bank reproduces phase 2a's certified
held-out POD errors.

### The quadrature-free residual

$$r(z) = -\nu\big(\Phi^\top M_u L\bar{\mathbf{u}} + A\,h(z)\big) - b(\mu),
\qquad A=\Phi^\top M_u L G,\qquad b(\mu)=\sum_q\theta_q(\mu)\,b_q .$$

$A$ is $M\times R$ and every $b_q=\Phi^\top M_u\mathbf{f}_q$ is precomputed once.
**Pressure never appears in the ROM**, exactly and analytically: $\Phi$ is
divergence-free, so $\Phi^\top M_u\mathrm{Grad}_h p=-(D\Phi)^\top M_p p = 0$.

The honest per-query cost is $O(MQ)$ to assemble $b(\mu)$, plus, per LM
iteration, one head evaluation, one head Jacobian, and $O(MR+MK)$. Nothing
scales with $n_u$. "Cost per residual is $MR$" omits the head and the
query-time assembly and is not quoted anywhere here.

### Two implementations of the head, on purpose

The head is written twice: in JAX (training, autodiff) and in numpy (**every
timed path**), so that no JAX dispatch overhead enters the S6 cost comparison
and every arm — FOM back-substitution, the linear controls, the nonlinear
arms — is timed in the same language on the same CPU. Gate **S-HEAD** asserts
the two agree in value and analytic Jacobian, and that the analytic Jacobian
...
<!-- BEGIN GENERATED PHASE2B (stk2d_rom_tables.py) -->
<!-- generated by stk2d_rom_tables.py from stk2d_recon_gate_recon_nu1.json (commit 48413c6c863d) and stk2d_rom_gates_rom_nu1.json (commit cccf5ee24b5f) -- do not edit by hand -->

### S-RECON -- THE STOP GATE, and its verdict

| quantity | value |
|---|---|
| configuration | N=64, R=32, K=8, training form `auto`, fit cohort 16384 |
| held-out oracle, AGGREGATE | 9.8687e-03 |
| POD-8, aggregate | 3.8486e-02 |
| required (POD-K / 3), aggregate | 1.2829e-02 |
| gain, aggregate | 3.90x |
| held-out oracle, PER-CASE MEDIAN | 7.9965e-03 |
| POD-8, per-case median | 3.7301e-02 |
| required (POD-K median / 3) | 1.2434e-02 |
| gain, per-case median | 4.66x |
| held-out oracle, per-case max | 5.3431e-02 |
| **VERDICT** | **PASS** |

Per mesh and per R, with both negative controls (all on the identical held-out cohort in the identical mass-weighted norm):

| N | R | K | head params | oracle agg | oracle med | oracle max | oracle (train cohort) | POD-K | POD-R | POD-R truncation floor | gain agg | gain med | NEG CTL linear head | NEG CTL untrained head | head |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 32 | 8 | 8 | 35272 | 3.868e-02 | 3.752e-02 | 1.848e-01 | 3.551e-02 | 3.868e-02 | 3.868e-02 | 3.868e-02 | 1.00x | 1.00x | 3.868e-02 | 2.019e-01 | cache |
| 32 | 16 | 8 | 36368 | 1.278e-02 | 1.103e-02 | 7.150e-02 | 1.430e-02 | 3.868e-02 | 4.366e-03 | 4.366e-03 | 3.03x | 3.40x | 3.763e-02 | 7.471e-01 | cache |
| 32 | 32 | 8 | 38560 | 1.135e-02 | 9.548e-03 | 8.863e-02 | 1.163e-02 | 3.868e-02 | 2.670e-14 | 2.670e-14 | 3.41x | 3.93x | 3.763e-02 | 8.660e-01 | cache |
| 64 | 8 | 8 | 35272 | 3.849e-02 | 3.730e-02 | 1.946e-01 | 3.512e-02 | 3.849e-02 | 3.849e-02 | 3.849e-02 | 1.00x | 1.00x | 3.849e-02 | 1.775e-01 | cache |
| 64 | 16 | 8 | 36368 | 1.165e-02 | 1.025e-02 | 5.976e-02 | 1.170e-02 | 3.849e-02 | 3.953e-03 | 3.953e-03 | 3.30x | 3.64x | 3.738e-02 | 7.463e-01 | cache |
| 64 | 32 | 8 | 38560 | 9.869e-03 | 7.996e-03 | 5.343e-02 | 1.097e-02 | 3.849e-02 | 4.015e-14 | 4.015e-14 | 3.90x | 4.66x | 3.738e-02 | 8.677e-01 | cache |
| 128 | 8 | 8 | 35272 | 3.849e-02 | 3.913e-02 | 2.039e-01 | 3.489e-02 | 3.849e-02 | 3.849e-02 | 3.849e-02 | 1.00x | 1.00x | 3.849e-02 | 1.656e-01 | trained |
| 128 | 16 | 8 | 36368 | 1.043e-02 | 9.194e-03 | 5.064e-02 | 1.155e-02 | 3.849e-02 | 3.800e-03 | 3.800e-03 | 3.69x | 4.26x | 3.731e-02 | 7.458e-01 | trained |
| 128 | 32 | 8 | 38560 | 9.746e-03 | 8.038e-03 | 4.064e-02 | 1.245e-02 | 3.849e-02 | 2.794e-13 | 2.794e-13 | 3.95x | 4.87x | 3.731e-02 | 8.696e-01 | trained |
| 256 | 8 | 8 | 35272 | 3.846e-02 | 3.910e-02 | 2.108e-01 | 3.464e-02 | 3.841e-02 | 3.841e-02 | 3.841e-02 | 1.00x | 1.00x | 3.841e-02 | 1.676e-01 | trained |
| 256 | 16 | 8 | 36368 | 1.298e-02 | 1.055e-02 | 6.751e-02 | 1.640e-02 | 3.841e-02 | 3.707e-03 | 3.707e-03 | 2.96x | 3.71x | 3.714e-02 | 7.704e-01 | trained |
| 256 | 32 | 8 | 38560 | 6.403e-03 | 6.474e-03 | 3.389e-02 | 8.334e-03 | 3.841e-02 | 1.318e-12 | 1.318e-12 | 6.00x | 6.04x | 3.714e-02 | 8.736e-01 | trained |

### S-SELECT -- which training form, chosen on a VALIDATION cohort

| training form | N | R | head params | final rel-MSE | validation oracle (rel) | validation oracle (median) | train s |
|---|---|---|---|---|---|---|---|
| sup | 64 | 32 | 38560 | 2.611e-05 | 1.222e-02 | 3.515e-03 | 371 |
| auto | 64 | 32 | 38560 | 7.718e-05 | 1.011e-02 | 9.207e-03 | 325 |

selected: `auto`

### S-METRIC / S-REGR / S-AFFINE -- the three supporting gates

| N | S-METRIC identity dev | \|\|perp\|\|/\|\|x\|\| at R=32 | S-REGR worst dev (R<32) | S-REGR dev at R=32 (recorded) | POD-8 here | POD-8 certified (phase 2a) | S-AFFINE coeff dev | its 1e-14 N^2 budget |
|---|---|---|---|---|---|---|---|---|
| 32 | 2.000e-16 | 2.670e-14 | 0.000e+00 | 0.000e+00 | 3.8680e-02 | 3.8680e-02 | 3.637e-15 | 1.024e-11 |
| 64 | 6.612e-16 | 4.015e-14 | 0.000e+00 | 0.000e+00 | 3.8486e-02 | 3.8486e-02 | 2.102e-15 | 4.096e-11 |
| 128 | 7.424e-16 | 2.794e-13 | 0.000e+00 | 0.000e+00 | 3.8489e-02 | 3.8489e-02 | 1.715e-14 | 1.638e-10 |
| 256 | 6.923e-16 | 1.318e-12 | 0.000e+00 | 0.000e+00 | 3.8410e-02 | 3.8410e-02 | 2.009e-13 | 6.554e-10 |

### S4 -- the quadrature-free residual vs an INDEPENDENT full-grid path

| N | R | M | states | resid rel | resid cancellation-aware | Jacobian cancellation-aware | pressure-elimination rung | Phi vs matrix-free / its 1e-15 N budget | NEG CTL even ghosts | NEG CTL dropped mean | NEG CTL A + 1e-10 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 32 | 32 | 64 | 160 | 8.369e-02 | 5.992e-16 | 4.739e-16 | 3.400e-16 | 5.594e-15 / 3.2e-14 | 1.279e-01 | 3.525e-02 | 1.466e-09 |
| 64 | 32 | 64 | 160 | 4.539e-02 | 1.758e-15 | 1.135e-15 | 4.926e-16 | 1.855e-14 / 6.4e-14 | 1.699e-01 | 2.780e-02 | 1.255e-09 |
| 128 | 32 | 64 | 160 | 6.825e-03 | 2.329e-15 | 1.641e-15 | 1.366e-15 | 1.955e-14 / 1.3e-13 | 1.215e-01 | 2.188e-02 | 1.451e-09 |
| 256 | 32 | 64 | 160 | 1.112e-02 | 7.384e-15 | 4.765e-15 | 1.518e-14 | 1.075e-13 / 2.6e-13 | 9.365e-02 | 2.057e-02 | 1.085e-09 |

### S7 -- the three controls and the nonlinear arm (held-out error)

| N | R | M | nonlinear QF | nonlinear QF (median) | (a) POD-K | (b) POD-R Galerkin | (c) direct G-span | nonlinear QF, same 8 cases & same 2 starts | nonlinear, full-grid residual | nonlinear, EQ residual | LM iters (median, all starts) | rank A | cond A | decoder reconstruction ceiling (oracle) |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 32 | 32 | 64 | 1.890e-02 | 1.678e-02 | 5.013e-02 | 2.781e-14 | 2.908e-14 | 2.946e-02 | 2.946e-02 | 2.951e-02 | 227 | 32 | 26.69 | 1.135e-02 |
| 64 | 32 | 64 | 1.893e-02 | 1.807e-02 | 4.981e-02 | 4.178e-14 | 4.565e-14 | 2.237e-02 | 2.237e-02 | 2.265e-02 | 238 | 32 | 36.33 | 9.869e-03 |
| 128 | 32 | 64 | 1.866e-02 | 1.631e-02 | 4.833e-02 | 2.847e-13 | 3.335e-13 | 1.168e-02 | 1.168e-02 | 1.337e-02 | 224 | 32 | 45.18 | 9.746e-03 |
| 256 | 32 | 64 | 1.423e-02 | 1.355e-02 | 4.618e-02 | 1.359e-12 | 2.071e-12 | 1.431e-02 | 1.431e-02 | 4.071e-02 | 238 | 32 | 50.93 | 6.403e-03 |

The full-grid and EQ nonlinear arms run on the FIRST 8 held-out cases with 2 LM starts, because each of their iterations is O(n_u); the quadrature-free arm is repeated on exactly those 8 cases with exactly those 2 starts for a like-for-like column.  The three arms agree on the RESIDUAL to 1e-15 (gate S4), so any remaining difference between them is LM path dependence, not a difference in the residual definition.

### S6 -- online cost per query, balanced-order timing

| ms per query | N=32 | N=64 | N=128 | N=256 |
|---|---|---|---|---|
| FOM back-substitution (as this cell's solver reports it) | 1.489 | 11.388 | 90.382 | 412.014 |
| FOM back-substitution, bare (no backward-error bookkeeping) | 1.442 | 12.102 | 81.786 | 400.314 |
| ROM, quadrature-free residual | 25.622 | 34.163 | 35.132 | 51.725 |
| ROM, full-grid residual | 115.357 | 282.873 | 3065.286 | 16420.693 |
| ROM, EQ residual | 53.331 | 55.493 | 62.944 | 75.313 |
| direct G-span solve (S7c) | 0.005 | 0.005 | 0.005 | 0.006 |
| POD-K direct solve (S7a) | 0.004 | 0.005 | 0.004 | 0.005 |
| POD-R Galerkin (S7b) | 0.005 | 0.006 | 0.006 | 0.006 |
| ROM, quadrature-free, NON-AFFINE force (separate arm) | 34.973 | 52.462 | 62.066 | 62.434 |

Offline cost, and the EQ arm's setup, timed separately:

| N | QF precompute (A, b_q, Phi^T M_u L ubar) s | EQ setup (design + greedy + NNLS) s | FOM factorisation s | EQ machinery exactness (all faces, unit weights) | Phi^T M_u Grad p on FOM pressures (S3 field path) | affine-vs-physical force discrepancy |
|---|---|---|---|---|---|---|
| 32 | 0.00 | 2.7 | 0.1 | 2.117e-17 | 7.148e-17 | 1.035e+00 |
| 64 | 0.03 | 7.0 | 0.7 | 3.250e-17 | 1.258e-16 | 1.033e+00 |
| 128 | 0.09 | 9.4 | 7.5 | 2.990e-17 | 2.825e-16 | 1.025e+00 |
| 256 | 0.39 | 8.0 | 81.2 | 2.699e-16 | 5.391e-16 | 1.018e+00 |

The EQ arm against node budget (the fitted rule is a MEASUREMENT, not a gate):

| N | nodes m | stencil rows touched | x/y lattice split | NNLS fit residual | held-out residual err (max) | (median) | \|\|A_EQ - A\|\|/\|\|A\|\| | w min | w max |
|---|---|---|---|---|---|---|---|---|---|
| 32 | 64 | 284 | 32/32 | 3.104e-01 | 3.208e-02 | 1.354e-02 | 1.883e+00 | 1.294e+00 | 2.341e+01 |
| 32 | 128 | 593 | 67/61 | 1.494e-01 | 1.737e-02 | 9.769e-03 | 1.962e+00 | 0.000e+00 | 2.354e+01 |
| 32 | 256 | 1127 | 131/125 | 3.773e-02 | 5.008e-03 | 2.963e-03 | 1.994e+00 | 0.000e+00 | 1.567e+01 |
| 32 | 512 | 1773 | 253/259 | 1.200e-03 | 1.864e-04 | 1.013e-04 | 2.000e+00 | 0.000e+00 | 9.895e+00 |
| 64 | 64 | 283 | 30/34 | 2.733e-01 | 2.997e-02 | 1.468e-02 | 1.930e+00 | 0.000e+00 | 1.015e+02 |
| 64 | 128 | 595 | 63/65 | 1.411e-01 | 1.332e-02 | 8.788e-03 | 1.980e+00 | 0.000e+00 | 9.907e+01 |
| 64 | 256 | 1220 | 129/127 | 3.723e-02 | 4.402e-03 | 2.548e-03 | 1.999e+00 | 0.000e+00 | 8.680e+01 |
| 64 | 512 | 2457 | 256/256 | 4.383e-03 | 9.604e-04 | 3.374e-04 | 2.000e+00 | 0.000e+00 | 5.396e+01 |
| 128 | 64 | 284 | 30/34 | 2.587e-01 | 2.774e-02 | 1.360e-02 | 1.936e+00 | 0.000e+00 | 4.044e+02 |
| 128 | 128 | 598 | 63/65 | 1.387e-01 | 1.519e-02 | 8.283e-03 | 1.979e+00 | 0.000e+00 | 4.027e+02 |
| 128 | 256 | 1233 | 124/132 | 3.953e-02 | 4.426e-03 | 2.480e-03 | 2.000e+00 | 0.000e+00 | 2.439e+02 |
| 128 | 512 | 2501 | 255/257 | 1.511e-02 | 2.545e-03 | 9.410e-04 | 1.999e+00 | 0.000e+00 | 4.636e+02 |
| 256 | 64 | 307 | 33/31 | 2.827e-01 | 2.869e-02 | 1.386e-02 | 1.930e+00 | 1.677e+00 | 1.849e+03 |
| 256 | 128 | 625 | 63/65 | 1.700e-01 | 2.125e-02 | 9.488e-03 | 1.971e+00 | 0.000e+00 | 1.597e+03 |
| 256 | 256 | 1265 | 127/129 | 1.164e-01 | 1.577e-02 | 7.030e-03 | 1.987e+00 | 0.000e+00 | 2.209e+03 |
| 256 | 512 | 2543 | 261/251 | 9.019e-02 | 1.103e-02 | 6.544e-03 | 1.991e+00 | 0.000e+00 | 1.063e+04 |

### S8 -- the M ladder and the conditioning of A J_h(z)

| N | R | M | M >= R | rank(A J_h) min | max | sigma_min/sigma_max (min) | (median) | states |
|---|---|---|---|---|---|---|---|---|
| 32 | 32 | 32 | True | 8 | 8 | 3.297e-02 | 9.710e-02 | 16 |
| 32 | 32 | 64 | True | 8 | 8 | 3.411e-02 | 9.710e-02 | 16 |
| 32 | 32 | 128 | True | 8 | 8 | 3.350e-02 | 9.650e-02 | 16 |
| 64 | 32 | 32 | True | 8 | 8 | 2.650e-02 | 7.737e-02 | 16 |
| 64 | 32 | 64 | True | 8 | 8 | 2.704e-02 | 7.823e-02 | 16 |
| 64 | 32 | 128 | True | 8 | 8 | 2.646e-02 | 7.672e-02 | 16 |
| 128 | 32 | 32 | True | 8 | 8 | 1.966e-02 | 7.866e-02 | 16 |
| 128 | 32 | 64 | True | 8 | 8 | 2.056e-02 | 7.989e-02 | 16 |
| 128 | 32 | 128 | True | 8 | 8 | 2.022e-02 | 7.880e-02 | 16 |
| 256 | 32 | 32 | True | 8 | 8 | 2.349e-02 | 9.463e-02 | 16 |
| 256 | 32 | 64 | True | 8 | 8 | 2.403e-02 | 9.649e-02 | 16 |
| 256 | 32 | 128 | True | 8 | 8 | 2.372e-02 | 9.544e-02 | 16 |

Near-collision conditioning -- the coincident-blob case is INCLUDED in the domain, not excluded (one M rung shown per mesh):

| N | M | blob separation | family Jacobian rank | family sigma_min/sigma_max | rank(A J_h) | A J_h sigma_min/sigma_max | nonlinear ROM coeff err | direct G-span coeff err |
|---|---|---|---|---|---|---|---|---|
| 32 | 64 | 0.1 | 8 | 1.067e-03 | 8 | 3.810e-02 | 1.067e-02 | 1.496e-14 |
| 32 | 64 | 0.01 | 8 | 9.555e-05 | 8 | 7.779e-02 | 3.665e-03 | 1.420e-14 |
| 32 | 64 | 0.001 | 8 | 9.417e-06 | 8 | 7.763e-02 | 3.764e-03 | 1.443e-14 |
| 32 | 64 | 0.0001 | 8 | 9.403e-07 | 8 | 7.762e-02 | 3.774e-03 | 1.422e-14 |
| 32 | 64 | 0 | 4 | 3.158e-12 | 8 | 7.762e-02 | 3.776e-03 | 1.442e-14 |
| 64 | 64 | 0.1 | 8 | 1.058e-03 | 8 | 4.608e-02 | 5.313e-03 | 2.123e-14 |
| 64 | 64 | 0.01 | 8 | 9.397e-05 | 8 | 3.278e-02 | 6.241e-03 | 2.108e-14 |
| 64 | 64 | 0.001 | 8 | 9.244e-06 | 8 | 3.200e-02 | 6.434e-03 | 2.103e-14 |
| 64 | 64 | 0.0001 | 8 | 9.229e-07 | 8 | 3.192e-02 | 6.450e-03 | 2.113e-14 |
| 64 | 64 | 0 | 4 | 4.950e-12 | 8 | 3.191e-02 | 6.452e-03 | 2.114e-14 |
| 128 | 64 | 0.1 | 8 | 1.083e-03 | 8 | 1.114e-01 | 5.877e-03 | 1.744e-13 |
| 128 | 64 | 0.01 | 8 | 9.470e-05 | 8 | 1.363e-01 | 6.814e-03 | 1.696e-13 |
| 128 | 64 | 0.001 | 8 | 9.293e-06 | 8 | 1.386e-01 | 6.848e-03 | 1.691e-13 |
| 128 | 64 | 0.0001 | 8 | 9.275e-07 | 8 | 1.388e-01 | 6.850e-03 | 1.693e-13 |
| 128 | 64 | 0 | 4 | 6.644e-12 | 8 | 1.388e-01 | 6.850e-03 | 1.698e-13 |
| 256 | 64 | 0.1 | 8 | 1.121e-03 | 8 | 3.531e-02 | 6.619e-03 | 1.089e-12 |
| 256 | 64 | 0.01 | 8 | 9.615e-05 | 8 | 3.184e-02 | 6.529e-03 | 1.088e-12 |
| 256 | 64 | 0.001 | 8 | 9.411e-06 | 8 | 4.572e-02 | 8.191e-03 | 1.092e-12 |
| 256 | 64 | 0.0001 | 8 | 9.391e-07 | 8 | 4.574e-02 | 8.204e-03 | 1.092e-12 |
| 256 | 64 | 0 | 4 | 3.376e-12 | 8 | 4.575e-02 | 8.205e-03 | 1.093e-12 |

### S9 -- the R frontier, K fixed at 8, against the M ladder

| N | R | M | head params | POD-R truncation floor | nonlinear QF (agg) | (median) | (a) POD-K | (c) direct G-span | (b) POD-R Galerkin | rank A / R | LM iters |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 32 | 8 | 32 | 35272 | 3.868e-02 | 5.054e-02 | 5.020e-02 | 5.019e-02 | 5.019e-02 | 3.943e-02 | 8/8 | 182 |
| 32 | 16 | 32 | 36368 | 4.366e-03 | 2.122e-02 | 1.859e-02 | 5.019e-02 | 1.239e-02 | 4.550e-03 | 16/16 | 230 |
| 32 | 32 | 32 | 38560 | 2.670e-14 | 1.882e-02 | 1.842e-02 | 5.019e-02 | 3.133e-01 | 2.781e-14 | 28/32  (direct arm INVALID) | 223 |
| 32 | 8 | 64 | 35272 | 3.868e-02 | 5.051e-02 | 5.013e-02 | 5.013e-02 | 5.013e-02 | 3.943e-02 | 8/8 | 184 |
| 32 | 16 | 64 | 36368 | 4.366e-03 | 2.310e-02 | 1.850e-02 | 5.013e-02 | 1.168e-02 | 4.550e-03 | 16/16 | 229 |
| 32 | 32 | 64 | 38560 | 2.670e-14 | 1.853e-02 | 1.821e-02 | 5.013e-02 | 2.908e-14 | 2.781e-14 | 32/32 | 224 |
| 32 | 8 | 128 | 35272 | 3.868e-02 | 5.131e-02 | 5.075e-02 | 5.114e-02 | 5.114e-02 | 3.943e-02 | 8/8 | 188 |
| 32 | 16 | 128 | 36368 | 4.366e-03 | 2.393e-02 | 1.929e-02 | 5.114e-02 | 1.283e-02 | 4.550e-03 | 16/16 | 229 |
| 32 | 32 | 128 | 38560 | 2.670e-14 | 1.839e-02 | 1.939e-02 | 5.114e-02 | 2.940e-14 | 2.781e-14 | 32/32 | 220 |
| 64 | 8 | 32 | 35272 | 3.849e-02 | 4.991e-02 | 4.991e-02 | 4.991e-02 | 4.991e-02 | 3.906e-02 | 8/8 | 186 |
| 64 | 16 | 32 | 36368 | 3.953e-03 | 1.905e-02 | 1.901e-02 | 4.991e-02 | 1.306e-02 | 4.007e-03 | 16/16 | 244 |
| 64 | 32 | 32 | 38560 | 4.015e-14 | 1.957e-02 | 1.951e-02 | 4.991e-02 | 6.334e-02 | 4.178e-14 | 28/32  (direct arm INVALID) | 229 |
| 64 | 8 | 64 | 35272 | 3.849e-02 | 4.993e-02 | 4.972e-02 | 4.981e-02 | 4.981e-02 | 3.906e-02 | 8/8 | 184 |
| 64 | 16 | 64 | 36368 | 3.953e-03 | 1.892e-02 | 1.835e-02 | 4.981e-02 | 1.238e-02 | 4.007e-03 | 16/16 | 242 |
| 64 | 32 | 64 | 38560 | 4.015e-14 | 1.975e-02 | 1.899e-02 | 4.981e-02 | 4.565e-14 | 4.178e-14 | 32/32 | 233 |
| 64 | 8 | 128 | 35272 | 3.849e-02 | 5.099e-02 | 5.062e-02 | 5.100e-02 | 5.100e-02 | 3.906e-02 | 8/8 | 190 |
| 64 | 16 | 128 | 36368 | 3.953e-03 | 2.004e-02 | 2.026e-02 | 5.100e-02 | 1.389e-02 | 4.007e-03 | 16/16 | 246 |
| 64 | 32 | 128 | 38560 | 4.015e-14 | 2.065e-02 | 1.999e-02 | 5.100e-02 | 4.641e-14 | 4.178e-14 | 32/32 | 239 |
| 128 | 8 | 32 | 35272 | 3.849e-02 | 4.848e-02 | 4.870e-02 | 4.847e-02 | 4.847e-02 | 3.894e-02 | 8/8 | 179 |
| 128 | 16 | 32 | 36368 | 3.800e-03 | 1.774e-02 | 1.842e-02 | 4.847e-02 | 1.244e-02 | 3.836e-03 | 16/16 | 249 |
| 128 | 32 | 32 | 38560 | 2.794e-13 | 1.816e-02 | 1.697e-02 | 4.847e-02 | 2.684e-02 | 2.847e-13 | 29/32  (direct arm INVALID) | 216 |
| 128 | 8 | 64 | 35272 | 3.849e-02 | 4.835e-02 | 4.827e-02 | 4.833e-02 | 4.833e-02 | 3.894e-02 | 8/8 | 184 |
| 128 | 16 | 64 | 36368 | 3.800e-03 | 1.875e-02 | 1.814e-02 | 4.833e-02 | 1.148e-02 | 3.836e-03 | 16/16 | 239 |
| 128 | 32 | 64 | 38560 | 2.794e-13 | 1.802e-02 | 1.651e-02 | 4.833e-02 | 3.335e-13 | 2.847e-13 | 32/32 | 216 |
| 128 | 8 | 128 | 35272 | 3.849e-02 | 4.942e-02 | 4.960e-02 | 4.938e-02 | 4.938e-02 | 3.894e-02 | 8/8 | 184 |
| 128 | 16 | 128 | 36368 | 3.800e-03 | 1.992e-02 | 1.958e-02 | 4.938e-02 | 1.295e-02 | 3.836e-03 | 16/16 | 242 |
| 128 | 32 | 128 | 38560 | 2.794e-13 | 1.901e-02 | 1.761e-02 | 4.938e-02 | 3.370e-13 | 2.847e-13 | 32/32 | 216 |
| 256 | 8 | 32 | 35272 | 3.841e-02 | 4.664e-02 | 4.462e-02 | 4.632e-02 | 4.632e-02 | 3.878e-02 | 8/8 | 196 |
| 256 | 16 | 32 | 36368 | 3.707e-03 | 2.241e-02 | 1.794e-02 | 4.632e-02 | 1.109e-02 | 3.757e-03 | 16/16 | 243 |
| 256 | 32 | 32 | 38560 | 1.318e-12 | 1.878e-02 | 1.263e-02 | 4.632e-02 | 1.119e-01 | 1.359e-12 | 30/32  (direct arm INVALID) | 232 |
| 256 | 8 | 64 | 35272 | 3.841e-02 | 4.648e-02 | 4.500e-02 | 4.618e-02 | 4.618e-02 | 3.878e-02 | 8/8 | 196 |
| 256 | 16 | 64 | 36368 | 3.707e-03 | 2.165e-02 | 1.739e-02 | 4.618e-02 | 9.847e-03 | 3.757e-03 | 16/16 | 242 |
| 256 | 32 | 64 | 38560 | 1.318e-12 | 1.882e-02 | 1.204e-02 | 4.618e-02 | 2.071e-12 | 1.359e-12 | 32/32 | 238 |
| 256 | 8 | 128 | 35272 | 3.841e-02 | 4.728e-02 | 4.523e-02 | 4.698e-02 | 4.698e-02 | 3.878e-02 | 8/8 | 198 |
| 256 | 16 | 128 | 36368 | 3.707e-03 | 2.237e-02 | 1.806e-02 | 4.698e-02 | 1.112e-02 | 3.757e-03 | 16/16 | 244 |
| 256 | 32 | 128 | 38560 | 1.318e-12 | 1.952e-02 | 1.242e-02 | 4.698e-02 | 2.123e-12 | 1.359e-12 | 32/32 | 238 |

`M >= R` is NECESSARY BUT NOT SUFFICIENT for the direct G-span arm.  Where `rank A < R` the least-squares solve is underdetermined and the design's own fallback -- a proper G-Galerkin reduced Stokes solve -- is the valid arm; it is computed and reported at every rung, so no rung is lost.

### Supporting gates

| gate | worst value | rule |
|---|---|---|
| S-HEAD numpy vs JAX head (value) | 1.564e-15 | <= 1e-12 |
| S-HEAD numpy vs JAX head (Jacobian) | 2.180e-15 | <= 1e-12 |
| S-HEAD Jacobian vs central difference | 3.265e-09 | <= 1e-06 |
| S4 residual (cancellation-aware) | 7.384e-15 | <= 1e-12 |
| S4 Jacobian (cancellation-aware) | 4.765e-15 | <= 1e-12 |
| S4 pressure-elimination rung | 1.518e-14 | <= 1e-12 |
| S4 weakest negative control | 1.085e-09 | >= 1e-10 |
| S6 EQ machinery exactness | 2.699e-16 | <= 1e-13 |
| S6 FOM backward error (worst over checked solves) | 8.891e-19 | <= 1e-13 |
| S6 FOM continuity (phase 2a's corrected form) | 1.599e-16 | <= 1e-12 |
| S6 FOM continuity, PHASE 1's normalisation | 2.664e-15 | DIAGNOSTIC ONLY -- collapses on gradient forcing |
| S6 FOM 1e-9 perturbation control | 1.523e-13 | > 1e-13 |
| S6/S3 Phi^T M_u Grad p on FOM pressures | 5.391e-16 | <= 1e-12 |
| S8 worst rank(A J_h) | 8 | == K = 8 |
| S-METRIC identity | 7.424e-16 | <= 1e-12 |
| S-REGR vs certified phase 2a | 0.000e+00 | <= 1e-09 |
| S-AFFINE (worst / its 1e-14 N^2 budget) | 3.552e-04 | <= 1 |
| STOPGATE | True | must be true or this driver refuses to run |
| PRECOND (asserts / mismatch / smoke), stop gate | True / 0 / 0 | asserts live, frozen, smoke=0 |
| PRECOND (asserts / mismatch / smoke), ROM | True / 0 / 0 | asserts live, frozen, smoke=0 |
| MANIFEST, stop gate (missing / rows / non-finite) | 0 / 0 / 0 | all zero |
| MANIFEST, ROM (missing / rows / non-finite) | 0 / 0 / 0 | all zero |
| S0 (dtype / x64 / matmul / backend), both drivers | float64 / True / highest / gpu | all asserted |

runs: `stk2d_recon_gate_recon_nu1.json` (rev 1, complete=True certified=True, 3258 s) and `stk2d_rom_gates_rom_nu1.json` (rev 1, complete=True certified=True, 1623 s) | commit `cccf5ee24b5f` | host `spark-d69e` | numpy 2.4.4 scipy 1.17.1 | jax backend `gpu` x64=True matmul `highest`
<!-- END GENERATED PHASE2B -->

---

## Reading the gates

### The six things these numbers say

1. **The head clears the predeclared bar, and it stops well short of the bank.**
   The reconstruction oracle reaches about $10^{-2}$ — roughly $4\times$ better
   than POD-8, and about $2.5\times$ **worse** than the same bank's linear
   POD-16. Phase 2a's "ten orders of headroom" was the overclaim its own
   verification flagged: the headroom is real and the head does not reach it.
2. **The ROM is worse than its own reconstruction oracle, by about $2\times$.**
   The oracle minimises $\lVert c-h(z)\rVert$; the ROM minimises
   $\lVert A(h(z)-c)\rVert$, which is the same thing weighted by $A$
   ($\operatorname{cond}A$ = 27–51 across the ladder). Gating reconstruction
   before building the ROM was therefore necessary and not sufficient.
3. **The direct $G$-span solve wins by seven to ten orders in accuracy and by
   three to four in cost.** Predicted, measured, and not a result.
4. **$M\ge R$ is necessary but not sufficient.** At $M=R=32$ the reduced
   operator $A$ is numerically rank 28–30, the direct least-squares arm
   degrades to $10^{-2}$, and only the design's own Galerkin fallback stays at
   $10^{-14}$. The design anticipated the fallback; this cell measures where it
   is actually needed.
5. **The quadrature-free ROM's cost is genuinely grid-independent and the
   full-grid one is not** — 24.8 ms to 51.8 ms across a 66$\times$ growth in
   $n_u$, against 111 ms to 16.2 s. The crossover against the FOM sits between
   $N=64$ and $N=128$.
6. **The coincident-blob degeneracy is a property of the parameterisation, not
   of the reduced solve.** The family's own Jacobian collapses from rank 8 to
   rank 4 at zero separation while $\operatorname{rank}(AJ_h)$ stays 8 and its
   conditioning does not move.


### The stop gate — what it measures and why it is stated the way it is

Phase 2a's verification was explicit that POD-32's $\sim10^{-12}$ is the
**bank-span** ceiling and not the neural decoder's, and that phase 2a contained
no trained decoder and so could not say whether a finite MLP head stops at
$4\times10^{-3}$, at $4\times10^{-2}$, or elsewhere. The stop gate answers
exactly that question and nothing else: **reconstruction only, no residual, no
ROM, no timing**, against POD-8/16/32 on the identical held-out cohort in the
identical mass-weighted norm.

The quantity gated is the **reconstruction oracle**, $\min_z\lVert c-h(z)\rVert$
per held-out case, by multi-start damped LM. It is the best the decoder can do
on that sample, so it **upper-bounds what any ROM built on this decoder can
achieve** — which is what makes it the right thing to gate before building the
ROM.

**The pass bar was fixed before any training run and was not moved:** the oracle
must beat POD-8 by at least $3\times$ in the aggregate held-out error **and** in
the per-case median. It is recorded in the artifact as a `verdict`, not as an
assertion, so the negative would have been reported rather than crashing the
run; `stk2d_rom_gates.py` reads that verdict and refuses to run without it.

**Two negative controls, both asserted, both fired in development.**

- A **linear head** (`layers=0`, so the whole map is affine in $z$) has as its
  image a $K$-dimensional affine subspace. It therefore *cannot* materially
  beat POD-$K$, and the gate asserts two things that follow from that geometry
  and nothing else: it never reads **below** the POD-$K$ floor, and it never
  **clears the stop bar**. In a development run at $R=8$ it landed on the POD-8
  floor to every digit printed — which is the control behaving exactly as the geometry
  says it must.
- An **untrained head** must be no better than POD-$K$.

Without those two the gate could not distinguish "the head works" from "the
metric or the cohort is wrong".

### S4 — the independent full-grid path, and why phase 2a's S-SOLVE could not substitute

Carried-forward condition 6 is specific: *"S4's full-grid path must genuinely
reassemble, apply, and project the MAC residual independently. Phase 2a's
S-SOLVE reads exactly 0.0 and is an assembly regression check, not independent
validation."*

The full-grid arm here decodes $\mathbf{u}=\bar{\mathbf{u}}+Gh(z)$, reassembles
$\mathbf{f}(\mu)$ from the dictionary, applies the MAC stencil through
`stk.apply_laplacian` — the **pad-and-slice matrix-free** operator, written in
phase 1 independently of the sparse assembly and gated against it there at
$1.1\times10^{-16}$ — and projects through a test space rebuilt column by column
by `stk.apply_curl`. It shares **no matrix** with the quadrature-free path; it
shares only the $(k,\ell)$ mode list, which is a specification rather than a
computation.

**The normalisation is cancellation-aware and it has to be.** At a converged
solve, and at every exact-FOM-coefficient state, $\lVert r\rVert$ is itself
roundoff, so a relative form $\lVert r_{qf}-r_{full}\rVert/\lVert r_{full}\rVert$
measures nothing — it is recorded, and it is *large*, precisely because its
denominator has collapsed. The gated form divides by the **term magnitude**
$\nu(\lVert\Phi^\top M_u L\bar{\mathbf{u}}\rVert+\lVert A\rVert_F\lVert h\rVert)
+\lVert b\rVert$, which cannot collapse. The same treatment is applied to the
Jacobian, with denominator $\nu\lVert A\rVert_F\lVert J_h\rVert$.

**The pressure-elimination rung** phase 2a's forward note asked for is included:
$\Phi^\top M_u(-\nu L\mathbf{u}_{\text{FOM}}-\mathbf{f})$ on every held-out FOM
solution, along a path the solver never takes.

**Three negative controls**, each required to exceed 100× the gate tolerance:
even (free-slip) ghosts in the full-grid path; the affine-mean constant term
dropped, which is exactly the revision-1 design's omission; and a relative
perturbation of $A$. The third one's margin is set by the perturbation and not
by the gate, which is why the perturbation size is quoted with it and a second,
smaller perturbation is recorded beside it (retraction 27).

### S-SELECT — the two training forms disagree about *which* error they minimise

This was not anticipated and it is worth stating. On the validation cohort the
**autodecoder** (free per-sample codes) wins the **aggregate**, energy-weighted
error while the **supervised** form (latent pinned to $2\mu-1$) wins the
**per-case median** by a factor of about 2.6. The two numbers are in the
S-SELECT table above.

The mechanism is straightforward once seen: the aggregate
$\lVert E\rVert_F/\lVert X\rVert_F$ is dominated by the largest-norm cases, and
the autodecoder is free to move its latent coordinates so that capacity lands
where the energy is. Pinning the latent to the parameter removes that freedom
and produces a more uniform *relative* fit, which is what the median sees.

The predeclared selection rule was the validation **aggregate**, so the
autodecoder was chosen and every head in this report is autodecoder-trained.
That rule was fixed before the numbers existed and is not revisited here. But
the stop gate requires **both** metrics, so the choice is not cosmetic, and a
future cell should either select on the metric it is actually gated on or
predeclare a combined criterion. Recorded as a limitation of this cell's
protocol rather than as a finding about decoders.

### S3's bank-side field path, closed here

Phase 1 gated the pressure-elimination control (S-PRESS) against
*constructed* pressures, and phase 2a left the bank-side field path open. It is
closed here, at the cost of one matvec: the **actual FOM pressure from a saddle
solve** is tested against the curl-sine space,
$\lVert\Phi^\top M_u\mathrm{Grad}_h p_{\text{FOM}}\rVert$, normalised by
$\sqrt M\,\lVert\mathrm{Grad}_h p\rVert_M$ so it cannot be read as an
absolute number. It is asserted at $10^{-12}$ alongside the S4 rung. Together
the two say the same thing from opposite directions: the test space annihilates
the pressure that a real solve produces, and the residual it leaves on a real
solution is roundoff.

### S6 — the cost gate, and the finding that empirical quadrature has no story here

Three arms, all evaluating the *same* strong-form projected residual:

- **quadrature-free**: $A h(z)$ with $A$ precomputed, $O(MR)$;
- **full-grid**: decode, apply, project, $O(n_u R + n_u + M n_u)$;
- **empirical quadrature (EQ)**: a weighted sum over $m$ sampled faces.

The EQ arm is defined from scratch, as the design requires, because the
incumbent Poisson EQ cannot be inherited: candidates are drawn **separately on
the two face lattices**; sampling is applied **only after analytic pressure
elimination**, so the sampled quantity is the momentum residual and the pressure
is already gone; the fit target is the **full strong MAC projection** at
perturbed states; the support is chosen by a non-negative greedy and the weights
by a genuine NNLS, because a quadrature rule with negative weights is not a
quadrature rule; and the setup is timed separately.

**Its exactness gate is a bookkeeping check and is labelled as one**: with all
faces and unit weights the EQ formula must reproduce the full projection. That
tests the stencil slicing and the index remapping, not the fitted rule. The
fitted rule's accuracy is **reported against node budget** — it is a
measurement, not a gate.

**And there is a finding, which is stated where it is implemented rather than
buried in a table.** Because this operator is linear,
$\Phi_S^\top W(-\nu L_S G_{\text{ext}})$ is a *constant* $M\times R$ matrix. The
EQ arm therefore **collapses to exactly the quadrature-free form, with an
approximate matrix in place of the exact one**. On a linear PDE empirical
quadrature has no cost story at all; it can only lose accuracy. The general
pointwise form is the one timed here, because it is the one that carries to
Navier–Stokes, and $\lVert A_{EQ}-A\rVert/\lVert A\rVert$ is reported so the
collapse is visible as a number.

**And a second finding, which the $\lVert A_{EQ}-A\rVert/\lVert A\rVert$
column makes visible.** That ratio is $\approx 2.0$ at *every* node budget,
including the budgets where the fitted rule reproduces the full projection to
$10^{-4}$. The fitted quadrature is accurate **only in the directions its fit
states span**; the matrix it collapses to is not close to $A$ in any norm. An
EQ rule certified by its fit residual alone would therefore certify almost
nothing about the operator it stands for — which is the same lesson as the lab
log's "never certify EQ by NNLS rel-fit", arriving here from a different
direction.

**The affine and non-affine force arms are timed separately and never blended.**
The non-affine arm uses the *physical* moving-blob force the affine dictionary
stands in for, built on the grid at query time, whose projection cannot be
precomputed and costs $O(Mn_u)$ inside the pipe. The measured discrepancy
between the two families is **$O(1)$** (about 1.02–1.04 relative), and that is
the honest statement: the affine dictionary is a *stand-in* for the physical
family with the same eight parameters and the same kernel geometry, **not an
approximation of it in any error-bound sense**. The design's wording implies
the latter; nothing here supports it, and the number is reported so it cannot
be read as one.

### S7 — the three controls, and the one that was expected to win

(a) POD-$K$ at matched **online** dimension, (b) POD-$R$ Galerkin at matched
trial span, (c) the direct reduced solve in the $G$ span. All four arms are
evaluated on the identical held-out cohort in the identical centred
mass-weighted norm.

(c) is valid only when $M\ge R$ and $\operatorname{rank}A=R$; **both are
asserted**, not assumed, and the design's fallback to a $G$-Galerkin solve is
implemented as arm (b) and reported regardless.

Two things worth stating before the numbers.

- **(c) is a one-shot projection**: one $O(MQ)$ assembly and one $R\times M$
  matvec. There is no iteration to compete with.
- The linear ROM arms minimise the **residual**, not the error, so arm (a) is
  *not* the POD-$K$ projection: it is the Petrov least-squares solve in the
  POD-$K$ span, and it sits above the POD-$K$ projection floor. That gap is a
  property of the Petrov formulation, not a defect.

### S8 — conditioning, including the near-collision case rather than excluding it

Carried-forward conditions 1 and 8 offered a choice: exclude coincident blobs
from the parameter domain with a stated minimum separation, **or** include
near-collision conditioning in S8. **This cell includes it.** The two-blob
family is unchanged and the domain is unrestricted; S8 sweeps the blob
separation to zero and reports, at each separation, the **family's own**
Jacobian rank (which degenerates from 8 to 4 at coincidence, exactly as the
verification found), the rank and conditioning of the reduced Jacobian
$A\,J_h(z)$, and both arms' errors.

The distinction the table makes, and it is the point of the gate: the
degeneracy is a property of the **parameterisation**, not of the reduced solve.
$J_h(z)$ is the *decoder's* Jacobian at a latent point, and it stays full rank
$K$ regardless of where $\mu$ sits, because the head is a fixed map and the LM
system is posed in $z$. A collision in $\mu$ does not make the reduced solve
ill-posed; it makes two different $\mu$'s name the same state.

### S7 — what actually happened, and the gap the oracle did not predict

The nonlinear arm lands at $1.4$–$1.9\times10^{-2}$ across the ladder while its
own reconstruction oracle sits at $6.4\times10^{-3}$–$1.1\times10^{-2}$: the ROM
is about **twice** its decoder's ceiling. That gap is not slack in the LM — the
LM converges — it is the difference between minimising
$\lVert c-h(z)\rVert$ (what the oracle does) and minimising
$\lVert A(h(z)-c)\rVert$ (what the ROM does), with
$\operatorname{cond}A$ running 27 to 51 over the ladder. It is worth stating
because the stop gate is deliberately phrased in the oracle, which
*upper-bounds* what the ROM can do — and the measured factor of two is how
loose that bound is here.

The three controls behave exactly as the design predicted. (a) POD-$K$ sits at
$4.6$–$5.0\times10^{-2}$, slightly **above** the POD-8 projection floor
because it minimises the residual rather than the error. (b) and (c) both reach
$10^{-14}$–$10^{-12}$, i.e. the bank-span floor, in a single matvec.

### S9 — the R frontier, deconfounded

Nested banks from one factorisation ($R=8,16$ are exact prefixes of the $R=32$
bank, because the metric Cholesky-QR's triangular factor makes column $j$ depend
only on columns $1..j$), $K$ fixed at 8, parameter count reported per $R$, run
against the $M$ ladder. **$R=64$ is algebraically unavailable at $Q=48$**
(phase-2a retraction 14) and is not attempted.

The frontier must be read with the structural bound from the decoder identity:
**the head cannot beat the POD-$R$ truncation floor at its own $R$**. At $R=8$
that floor *is* POD-8, so the $R=8$ rung cannot show a gain by construction; it
is included because leaving it out would hide the fact, and the measurement
confirms it exactly — the $R=8$ reconstruction oracle equals POD-8 to every
digit printed, at every mesh.

Two further things the frontier shows.

- **The head's error is limited by the FIT, not by the span.** Going from
  $R=16$ to $R=32$ moves the truncation floor by eleven orders
  ($4\times10^{-3}\to4\times10^{-14}$) and moves the oracle by about 15%
  ($1.17\times10^{-2}\to9.9\times10^{-3}$ at $N=64$). Everything below
  $10^{-2}$ is the head, not the bank.
- **The rank of $A$, not $M\ge R$, is what makes the direct arm valid.** At
  $M=R=32$ the direct arm degrades to $2.7\times10^{-2}$–$3.1\times10^{-1}$
  while the Galerkin arm stays at $10^{-14}$–$10^{-12}$. Both are reported at
  every rung and the rank is printed beside them.

### The eight carried-forward conditions, and where each is discharged

| # | condition (from `STOKES-PHASE2A-VERIFY-codex.md`) | where it is discharged here |
|---|---|---|
| 1 | two-blob family frozen; the coincident-blob rank drop handled by **either** excluding near-collisions **or** including them in S8 | **INCLUDED, not excluded.** The family and the parameter box are untouched; S8 sweeps the blob separation to zero and reports the family's rank collapse (8 -> 4) beside the reduced Jacobian's conditioning |
| 2 | $R\in\{8,16,32\}$; no $R=64$ | the $R$ ladder is `{8,16,32}` in `FROZEN_CONFIG`; $R=64$ is not attempted and the reason (phase-2a retraction 14) is in S9's rule string |
| 3 | use the **metric-reorthogonalised psi-route bank**; any change to family, snapshots or $R$ ladder requires rerunning S1 | `RomCell` builds the bank with `bank.build_bank` + `bank.reorth_psi` on the frozen 256-snapshot training set; **gate S-REGR asserts it reproduces phase 2a's certified held-out POD errors**, so a silent change to any of the three would fail here rather than pass unnoticed. The head's larger fit cohort changes the *decoder's training data*, not the bank |
| 4 | reconstruction gated **before** residual and timing work, with per-case statistics; POD-32 is not the head's ceiling | the stop gate, run first and as a separate driver; the ROM driver refuses to start without its verdict. Per-case median and max are in the table, and the text never presents POD-32 as the head's ceiling |
| 5 | S5's frozen per-mesh rules stand; **dense $A$ is mandatory** | $A=\Phi^\top M_u LG$ is stored and applied **dense** everywhere; nothing in phase 2b diagonalises, sparsifies or approximates it. (The EQ arm's fitted matrix is a *separate, clearly labelled arm*, and its distance from the exact $A$ is reported.) |
| 6 | S4's full-grid path must genuinely reassemble, apply and project; S-SOLVE cannot substitute | the full-grid arm uses `stk.apply_laplacian` and `stk.apply_curl`, the matrix-free implementations, and shares no matrix with the QF path. Stated in S4's rule string, with the reason |
| 7 | carry forward the corrected continuity normalisation and the $10^{-9}$ perturbation control | the FOM solves this driver makes are checked with phase 2a's corrected blockwise continuity form, phase 1's form recorded beside it as a diagnostic, and a relative $10^{-9}$ perturbation control asserted to fire |
| 8 | S8 includes near-collision conditioning, or the domain excludes coincident blobs | same as 1: included |

## Falsifiability of every assertion

Assertions that have never fired are not evidence. Every gate below was made to
fail; the in-band controls run inside the certified artifact and are tabulated
above, the out-of-band ones were run on a scratch copy and are not committed.
...
24. **MINE: the POD-$R$ truncation floor must not be computed as
    ...
    about $10^{-28}$ of the total, i.e. **twelve orders below the f64
    resolution of that subtraction**, and the expression returned
    $2.68\times10^{-8}$ relative where the true value is
    $4.0\times10^{-14}$. The danger is that $2.68\times10^{-8}$ is a
    *plausible-looking* number — it is not a NaN, not a zero, and not obviously
    wrong. It was caught only because it disagreed with phase 2a's certified
    POD-32 value, i.e. by the regression gate and not by inspection.
    *Resolution:* the truncation residual is formed explicitly as
    $X-G(G^\top M_u X)$ everywhere, and the certified phase-2a POD numbers are
    a gated regression target (**S-REGR**) precisely so a repeat is caught.
25. **MINE: the direct $G$-span pseudo-inverse omitted its $-\nu$ factor.** The
    residual is $r=\text{const}-\nu A a-b$, so the least-squares solution is
    $a=(-\nu A)^{+}(b-\text{const})$; I precomputed $A^{+}$. The arm returned
    $-a$ and read a relative error against the FOM coefficients of **exactly
    2.000000**. Caught by the first end-to-end check against the FOM, not by
    reading the code. *Worth stating as a rule:* a relative error of exactly
    2.0 is the signature of a sign flip and should be read as one.
26. **MINE: the LM stopping rule was ABSOLUTE on a residual whose magnitude is
    not $O(1)$.** $\lVert r\rVert$ here is an unnormalised projected residual
    whose size is set by $\lVert b(\mu)\rVert$ (the term-magnitude scale is
    about $1.9\times10^{2}$ at $N=32$), so a
    tolerance of $10^{-13}$ could never be met and every query silently ran the
    full 100-iteration budget from every start. That would have corrupted **no
    accuracy number** — the LM still converges — but it would have corrupted
    the **S6 cost comparison**, which would have measured the iteration budget
    instead of the work. *Resolution:* the stop is relative to a
    term-magnitude scale computable before the solve, plus a step-size stop.
27. **A perturbation control's margin is set by the perturbation, not by the
    gate, and quoting one without the other says nothing.** S4's third control
    perturbs $A$ by a relative $\varepsilon$; the discrepancy it produces is
    proportional to $\varepsilon$. A relative $10^{-10}$ perturbation reads
    $1.3\times10^{-11}$, only ten times the $10^{-12}$ gate — so a
    "controls must fail by three orders" rule would have rejected a **correct**
    implementation. *Resolution:* the gated control uses a relative $10^{-8}$
    perturbation against a floor of $100\times$ the gate tolerance, and the
    $10^{-10}$ result is recorded beside it as the gate's measured sensitivity.
    This is the same class of error as phase-2a retraction 19 (flat floors for
    $O(h^2)$ controls): a control floor has to be derived from what the control
    actually does.
28. **MINE: a `SMOKE=1` run trains and SAVES decoder heads to the same paths a
    certified run reads.** Found by seeing smoke-trained heads in
    `runs/stk2d/heads/` after the smoke test. A certified ROM run could have
    silently consumed a 400-step smoke decoder and reported its numbers as
    certified. `SMOKE=1` correctly refused to certify its **own** artifact, but
    that guarantee did not extend to the side files it wrote. *Resolution:* the
    producing run's smoke flag, artifact name, mesh, and entire training
    configuration travel **with** each head, and the ROM driver asserts all of
    them against the stop-gate artifact's own configuration.
30. **MINE, and it cost a two-hour run: the linear control's floor was
    mathematically wrong on held-out data.** I gated the linear control at or
    ABOVE the POD-$K$ floor, reasoning that its image is a $K$-dimensional
    affine subspace and therefore cannot beat POD-$K$. That is true **on the
    cohort POD-$K$ was fitted to** and false on a held-out one: POD-$K$ here is
    the first $K$ columns of a bank built from **256** training snapshots,
    while the linear control fits an *arbitrary* $K$-dimensional affine
    subspace to **16384** samples, so it can generalise slightly better — and
    it does, reading $3.738\times10^{-2}$ against POD-8's
    $3.849\times10^{-2}$ at $N=64$, i.e. 2.9% below the "floor". The assertion
    sits *after* the whole training loop, so it would have aborted the run at
    the end and lost every number in it. *Resolution:* the gated statements
    are the two that actually follow from the geometry — the control stays
    within a factor of 1.5 of the floor, and it can never CLEAR the stop bar
    (it reads $1.03\times$ POD-8 against a $3\times$ requirement) — and the
    ratio is recorded either way. *Second resolution, and the more useful one:*
    trained heads are now **cached with their full recorded configuration**, so
    a gate defect at the end of a long run costs minutes instead of the run.
    The artifact records per rung whether each head was trained or loaded.
31. **The FIFTH mesh-scaling absolute tolerance to be wrong in this cell.** S4
    compares the sparse test space against the matrix-free one and I gated it
    at a flat $10^{-13}$. Both constructions difference sines scaled by $1/h$,
    so the roundoff of their *difference* grows with the mesh: measured
    $5.59\times10^{-15}$, $1.86\times10^{-14}$, $1.96\times10^{-14}$,
    $1.07\times10^{-13}$ at $N=32/64/128/256$. The flat tolerance passed three
    meshes and failed the fourth **on refinement alone**, after every
    expensive computation in the run had already been done. That is the same
    failure as phase-2a retractions 19 and 20, in a place I had not looked.
    *Resolution:* the budget is $10^{-15}N$, which the measured values sit at
    0.15–0.42 of, and the budget is recorded beside the value in the table so
    the scaling is visible rather than folklore.
29. **PROCESS, and it happened TWICE: a run launched from a dirty working
    tree records a `git_commit` that does not describe the code that produced
    it.** The first time I edited `stk2d_recon_gate.py` while a certified run
    of it was in flight; the second time I launched the ROM driver in the same
    breath as an uncommitted threshold fix, so its artifact claimed commit
    `48413c6` while running `48413c6` plus a diff. Both runs were discarded and
    redone from committed source, and nothing from either is reported anywhere.
    The `git_commit` field this project relies on for provenance **cannot
    catch this** — uncommitted edits do not change `HEAD`.
    *Resolution, and it is a mechanism rather than a resolution not to do it
    again:* both drivers now record `git_dirty` and **PRECOND refuses to run
...
- **The stop-gate number is a lower bound on what a head can do, not the best
  achievable.** An exploratory screen over wider and deeper heads was started
  and **abandoned after two configurations for wall-clock reasons**; the frozen
  architecture is the first configuration screened. The reported oracle is
  therefore what *this* head achieves, and a better-tuned head would move it
  down, not up. Stated because "the head reaches X" and "the head cannot do
  better than X" are different claims and only the first is made here.
- **A prototype of the stop-gate quantity was run before the driver existed.**
  It was used to size the architecture and to find retraction 24. The
  predeclared $3\times$ bar was fixed by the task before any run and was not
  moved; the training form was chosen on a **validation** cohort disjoint from
  the held-out one (gate S-SELECT); but the honest statement is that the
  architecture was not chosen blind to the scale of the answer.
- **On a linear PDE the EQ arm cannot have a cost story**, because it collapses
  to a precomputed $M\times R$ matrix exactly as the quadrature-free form does.
  The design asked for the arm and the arm is built and gated, but its cost
  numbers should be read as a rehearsal of the *machinery* for Navier–Stokes,
  not as a comparison with anything to gain here.
- **The non-affine arm is a cost arm only.** It uses the physical moving-blob
  force the affine dictionary stands in for, and the discrepancy between the two
  families is reported as a **diagnostic**, not as an "approximation error of
  the affine dictionary": the affine amplitudes are Gaussian kernel weights, not
  interpolation coefficients, so the two families are related by construction
  but not by an interpolation bound. The design's wording implies the latter.
- **Only $\nu=1$ was run.** In steady Stokes $\nu$ rescales velocity by $1/\nu$
  and nothing else; phase 1 gate S-NU confirmed that to $1.3\times10^{-13}$, and
  $\nu$ enters the reduced problem only as a scalar multiplying $A$.
- **The ROM's LM is multi-start from a fixed deterministic set**, and every
  start is charged to the arm's cost, because the ROM has no way to know in
  advance which start converges. A warm start from a neighbouring query would be
  cheaper and is the obvious thing to do in a parameter sweep; it is not done
  here because it would make the per-query cost depend on the sweep order.

## What is not done, and what a Navier–Stokes cell would inherit

Out of scope and **not run**: the unsteady / time-block machinery (steady
Stokes has none), the skew-symmetric convection tensor, any $\nu$ sweep beyond
phase 1's, and the moving-centre family as an *accuracy* arm (it appears here
only as a separately-reported cost arm).

A Navier–Stokes cell inherits, gated:

- a **vector-valued, discretely divergence-free decoder** with no `bc(x)` mask,
  and the coefficient-space identity that makes its training and its error
  reporting exact;
- **exact analytic pressure elimination** — pressure appears nowhere in the
  reduced problem, and the rung that proves it on FOM solutions;
- the **quadrature-free residual in its general block form**, with $A$
  precomputed per affine operator component, gated against an independent
  matrix-free full-grid implementation for the residual *and* the Jacobian;
- a **strong-form EQ arm** with two-lattice sampling after pressure
  elimination, non-negative weights, and its own exactness bookkeeping;
- the **affine/non-affine cost separation**, with the $O(Mn_u)$ query-time
  projection timed inside the pipe where it belongs;
- a harness in which the reconstruction ceiling is gated **before** any ROM
  number is computed, and in which trained decoders carry their own provenance.

**What does not carry**, restated from the design so it cannot be forgotten: the
convection tensor. `b2d_tensor_common.py` is scalar, collocated, and
fixed-positive-upwind; MAC Navier–Stokes needs two staggered component lattices,
cross-component interpolation, a defined skew-symmetric convection operator, and
a new tensor $T_{mjk}=\langle\phi_m,\mathcal{B}(g_j,g_k)\rangle$ with its own
build-order and Jacobian gates. And the target is **unsteady**, which this cell
does not rehearse at all.

Three forward notes.

- **The head is bounded below by the POD-$R$ truncation floor at its own $R$.**
  Any future $R$ ladder must be read with that bound, and any claim that a
  nonlinear head "beats POD-$R$" is a category error unless the comparison is at
  matched *online* dimension.
- **On a linear PDE with affine parameter dependence the nonlinear head can
  never buy accuracy** — this is the design's own framing and this cell is
  consistent with it. The head's value is cost, and on steady Stokes there is no
  cost to buy, because the direct $G$-span solve is one matvec.
- **A perturbation control's margin is set by its perturbation** (retraction 27).
  Any future control of that shape must quote both.
...
JAX_ENABLE_X64=1 JAX_DEFAULT_MATMUL_PRECISION=highest JAXRUN_MAX=48G \
  jaxrun /home/tahmid/Dev/.venv/bin/python -u stk2d_recon_gate.py   # the STOP GATE
XLA_PYTHON_CLIENT_PREALLOCATE=false XLA_PYTHON_CLIENT_MEM_FRACTION=0.10 \
JAX_ENABLE_X64=1 JAX_DEFAULT_MATMUL_PRECISION=highest JAXRUN_MAX=48G \
  jaxrun /home/tahmid/Dev/.venv/bin/python -u stk2d_rom_gates.py    # S4/S6/S7/S8/S9
/home/tahmid/Dev/.venv/bin/python stk2d_rom_tables.py               # regenerates the tables
```

Two single local processes, no cluster job; the ROM driver **refuses to start**
without a certified, passing stop-gate artifact, and both drivers **refuse to
start from a dirty working tree** (retraction 29).

*One provenance note, stated because the mechanism that would now catch it was
added afterwards.* The stop-gate artifact records commit `48413c6`, which
predates the dirty-tree guard. Its tree **was** clean at launch, and the only
change to any file it imports between that commit and the one the ROM artifact
records is `_silu_np` in `stk2d_rom.py` — a function the stop-gate driver never
calls (it uses `RomCell`, `perp_energy` and the coefficient maps; the numpy
head belongs to the ROM driver). The ROM artifact records commit `cccf5ee` with
an empty `git_dirty`. The FOM snapshots are cached
under `runs/stk2d/cache/` and the trained heads under `runs/stk2d/heads/`;
neither is committed (both are regenerated from the seed, and the cache key
carries the entire configuration). The numerics are numpy/scipy f64 on CPU for
every operator and every **timed** path — a sparse direct solve is the right
tool and a single-language timed pipe is the fair comparison — while JAX on GPU
trains the head and is asserted by S0.

Environment knobs, stop gate: `RECON_NS`, `R_LADDER`, `R_STOP`, `K_LAT`,
`N_PRIMARY`, `N_FIT`, `N_VAL`, `BATCH`, `HID`, `LAYERS`, `STEPS`, `LR`,
`S_TRAIN`, `S_TEST`, `NU`, `SEED`, `OUT_TAG`, `OUT_PREFIX`, `CACHE`, `HEADS`,
`ALLOW_CPU`, `SMOKE`. ROM driver: `ROM_NS`, `R_LADDER`, `M_LADDER`, `M_MAIN`,
`R_MAIN`, `N_MAIN`, `K_LAT`, `REPS`, `BURN`, `EQ_BUDGETS`, `EQ_CAND`,
`EQ_FIT_STATES`, `N_S4_STATES`, `N_TIME_QUERIES`, `LM_STARTS`, plus the same
tail. Any of them differing from `FROZEN_CONFIG` aborts under PRECOND unless
`SMOKE=1`, and a `SMOKE=1` run sets `complete=false` by construction — and now
also stamps every head it writes as smoke-produced, so it cannot contaminate a
certified run through a side file (retraction 28).

## Glossary — phase 2b additions

The phase-1 and phase-2a glossaries still apply.

- **Decoder / head $h$** — the trained nonlinear map from the $K$-dimensional
  latent $z$ to the $R$ bank coefficients. The *decoder* is the whole map
  $z\mapsto\bar{\mathbf{u}}+Gh(z)$; the *head* is just $h$.
- **Latent $z$** — the reduced unknown the ROM actually solves for. It is not
  the physical parameter $\mu$: at query time the ROM sees $\mu$ only through
  $b(\mu)$ and recovers $z$ by minimising the residual.
- **Coefficient space** — $\mathbb{R}^R$, the coordinates of a field in the
  bank. Because the bank is orthonormal in the mass metric, distances there are
  exactly physical distances, which is why the head can be trained and scored
  entirely in coefficients.
- **Truncation floor / `perp`** — the part of a held-out field that lies
  outside the bank's span. It is what POD-$R$ leaves behind, it is independent
  of the head, and it is a hard lower bound on the decoder's error.
- **Reconstruction oracle** — the best a *fixed* decoder can do on a given
  field, $\min_z\lVert c-h(z)\rVert$, found by multi-start LM. It ignores
  whether the ROM's residual minimisation would actually find that $z$, which is
  exactly why it is the right thing to gate before building the ROM.
- **Autodecoder** — training a decoder with one free latent code per sample and
  no encoder, optimising codes and weights jointly.
- **Supervised (pinned-latent) form** — training the head as a regression from
  the family parameter to the coefficients, i.e. fixing $z=2\mu-1$. Legitimate
  because the ROM never uses $\mu$ to evaluate $h$; it only ever solves for $z$.
- **Levenberg–Marquardt (LM)** — a damped Gauss–Newton method for nonlinear
  least squares. The damping $\lambda$ is raised when a step fails and lowered
  when it succeeds, which is what makes it robust from a poor start.
- **Multi-start** — running the solver from several initial guesses and keeping
  the best. Charged in full to the arm's cost here, because the ROM cannot know
  in advance which start converges.
- **Petrov least squares vs projection** — the linear ROM arms minimise the
  *residual* over the trial span; the POD-$R$ *projection* minimises the error.
  They are different, and the first is always at least as large as the second,
  so a residual-minimising POD-$K$ arm sits above the POD-$K$ error floor.
- **Galerkin (arm b)** — testing with the trial basis itself, giving an
  $R\times R$ reduced system. Legitimate here only because the bank is
  divergence-free, which is what removes the pressure from the tested equation.
- **$A$, $b_q$** — the precomputed reduced operator $\Phi^\top M_u L G$ and the
  precomputed projected dictionary forces. Everything expensive is in them, and
  they are computed once.
- **Affine parameter dependence** — the property that the force is a fixed set
  of shapes with moving weights. It is what makes $b(\mu)$ an $O(MQ)$ sum of
  precomputed vectors instead of an $O(Mn_u)$ projection.
- **Non-affine arm** — the same physics with a genuinely moving blob, where that
  projection cannot be precomputed. Timed separately, never blended.
- **Empirical quadrature (EQ) / ECSW / hyper-reduction** — replacing a sum over
  all degrees of freedom by a weighted sum over a small chosen subset, with the
  weights fitted so the reduced sum reproduces the full one on training states.
  Weights must be non-negative for the rule to be a quadrature.
- **NNLS (non-negative least squares)** — least squares with the weights
  constrained to be $\ge0$. Used here for the EQ weights on a support chosen by
  a greedy.
- **Node budget $m$** — how many sampled faces the EQ rule is allowed. The
  accuracy/cost knob of that arm.
- **Stencil extension** — the set of degrees of freedom the sampled faces'
  differencing stencils touch, which is what the decoder must actually evaluate.
  It is several times larger than the node set.
- **Cancellation-aware normalisation** — dividing a discrepancy by a *term
  magnitude* that cannot collapse, instead of by a quantity that is itself
  roundoff at the point of interest. Necessary wherever a residual is compared
  at a converged solve.
- **Balanced-order timing** — sweeping the list of timed subjects forward on
  even repetitions and reversed on odd ones, so no subject is systematically
  first or last, and reporting all raw repetition times rather than only
  medians.
- **Provenance stamp** — configuration metadata written *into* a produced
  artifact (here, each trained head) and asserted by whatever consumes it, so a
  development run cannot contaminate a certified one through a side file.
- **Stop gate** — a gate whose failure ends the experiment rather than the run:
  it is recorded as a verdict in a complete artifact, and the next stage refuses
  to start without it.
