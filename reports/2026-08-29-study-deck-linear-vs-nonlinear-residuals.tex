  ...
  \begin{columns}[T]
    ...
    \begin{alertblock}{Where it stops}
      ...
      \begin{itemize}
        ...
        \item \textbf{2D as it stands:} the 2D Burgers bank is $R=512$, so $T$ would be $128\times512^2 \approx 270$ MB --- needs compression (project $h$ onto the $\sim K$ directions it spans) before a 2D port.
...
\begin{frame}{Linear vs nonlinear, side by side}
  ...
  \begin{tabular}{@{}p{0.17\textwidth}p{0.23\textwidth}p{0.28\textwidth}p{0.28\textwidth}@{}}
    ...
    Cost vs grid size & flat & flat (solve $128\to65\,536$) & flat\\
    \bottomrule
  \end{tabular}
  \vspace{2mm}
