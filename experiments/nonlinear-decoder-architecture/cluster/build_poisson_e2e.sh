...
tau_loose="${5:-0.01}"; tau_tight="${6:-0.001}"
...
[
  {"pde":"poisson2d","method":"coord","N":64,"k":16,"M":$selected_M,"m":$selected_m,"tau":$tau_loose,"arm":"architecture_e2e"},
  {"pde":"poisson2d","method":"coord","N":64,"k":16,"M":$selected_M,"m":$selected_m,"tau":$tau_tight,"arm":"architecture_e2e"}
]
EOF
/home/tahmid/Dev/.venv/bin/python - "$STAGE/configs.json" "$selected_M" "$selected_m" "$tau_loose" "$tau_tight" <<'PY'
import json
import sys

path, expected_M, expected_m = sys.argv[1], int(sys.argv[2]), int(sys.argv[3])
expected_tau = {float(sys.argv[4]), float(sys.argv[5])}
with open(path) as fp:
    rows = json.load(fp)
assert len(rows) == 2
assert {(row["M"], row["m"]) for row in rows} == {(expected_M, expected_m)}
assert {row["tau"] for row in rows} == expected_tau
PY
...
cd code
export KS=16 NS=64 TAUS=$tau_loose,$tau_tight M=$selected_M MQ=$selected_m M_BIG=$selected_M K_BIG=99 MQ_4M=$selected_m
