"""Train the WAVE-2D FiLM AUTO-DECODER u(x; z) (one latent per (trajectory,
time) snapshot) + POD basis for the linear control.  Port of blat_train_ad.

  data    : wc.build_data(N) from seed (TRAIN = first N_TRAIN traj)
  target  : U / eps, eps = global RMS of the training snapshots; per-TRAJECTORY
            inverse mean-square weights (mean 1) => relative-error training in the
            wave testbed's traj-RMS sense (per-snapshot norms pass near zero
            during kinetic/potential exchange, so per-row weights would spike)
  ...
  net     : ms_parametric FiLM net (f64), n_freq Nyquist-capped (N-1)//2,
            hard-BC factor b(x,y) (wlat_common.bc_factor)
...
Usage: N=64 K_LAT=8 [AD_STEPS=60000] [AD_BATCH=128] [P_SUB=0] [T_SMOOTH=1e-2]
       [LAT_REG=1e-4] [LAT_LR=5e-3] python wlat_train_ad.py <outdir>
"""
...
from wlat_common import mp, F64, log
...
# original runs.  The DATA draw always uses wc.SEED (multi-seed = training randomness only).
TRAIN_SEED = int(os.environ.get("TRAIN_SEED", str(wc.SEED)))
K = wc.K_LAT
N = wc.N
T1 = wc.NUM_STEPS + 1
...
def main():
    log(f"jax_backend={jax.default_backend()}  x64={jax.config.jax_enable_x64}  "
        ...
        f"t_smooth={T_SMOOTH} bc={wc.BC_MODE} hidden={wc.AD_HIDDEN}x{wc.AD_LAYERS}")
    d = wc.build_data(N)
    U = d["U"]
    U_tr = U[:wc.N_TRAIN]                          # TEST split (TEST_SEED) untouched here
    fp = wc.data_fingerprint(U)
    ...
    dec = wc.CoordDecoder(params, n_freq, eps, K)
    ...
    rms_tr = np.repeat(np.sqrt(np.mean(np.sum(U_tr ** 2, axis=2), axis=1)), T1)
    # traj-RMS metric version of the same reconstruction
    rels_traj = rels * np.linalg.norm(S, axis=1) / rms_tr
    Zn = np.asarray(Z).reshape(n_tr, T1, K)
    dz = np.linalg.norm(np.diff(Zn, axis=1), axis=2)
    log(f"  TRAIN recon at learned latents: snap-rel mean {rels.mean():.3e} "
        f"(median {np.median(rels):.3e}, max {rels.max():.3e}); traj-RMS metric "
        f"{rels_traj.mean():.3e}; latent RMS "
    ...
    ck = dict(params=jax.tree_util.tree_map(np.asarray, params), n_freq=n_freq,
              ...
              config=dict(wc.CONFIG, train_seed=TRAIN_SEED, ad_steps=AD_STEPS, ad_batch=AD_BATCH,
              ...
              train_rel_median=float(np.median(rels)),
              train_traj_rel_mean=float(rels_traj.mean()), train_secs=train_secs,
              hist=hist, backend=jax.default_backend())
    with open(os.path.join(OUTDIR, f"wlat_ad_{tag}.pkl"), "wb") as f:
        pickle.dump(ck, f)
    rep = {k: v for k, v in ck.items() if k not in ("params", "Z_train", "V")}
    rep["sv"] = [float(s) for s in sv[:32]]
    with open(os.path.join(OUTDIR, f"wlat_ad_{tag}_report.json"), "w") as f:
