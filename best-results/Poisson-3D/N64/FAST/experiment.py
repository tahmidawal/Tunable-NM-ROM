...
def run_and_benchmark():
    ...
    min_eq_points = 2000
    n_eq_snaps    = 600
    outdir        = Path('/cluster/tufts/paralab/tawal01/NMROM-Apr8/20260423-NEURIPS/Autoresearch/Poisson-3D/runs/N64_run39')
    seed          = 0
    # Set to a checkpoint path to skip AE retraining (EQ-only run)
    load_ckpt     = Path('/cluster/tufts/paralab/tawal01/NMROM-Apr8/20260423-NEURIPS/Autoresearch/Poisson-3D/runs/N64_run26/checkpoint.pkl')
    # ───────────────────────────────────────────────────────────────────────
    ...
    # ── Training (skipped if load_ckpt is set) ────────────────────────────
    ...
    # Safety: keep only strictly interior nodes (stencil neighbors must be in-bounds)
    ix_chk = eq_indices // N2; iy_chk = (eq_indices // N) % N; iz_chk = eq_indices % N
    ...
    for k1, k2, k3 in TEST_CASES:
        ...
        jax.effects_barrier(); eq_t = time.time() - t0
        t1 = time.time()
        u_pred = decode_full(z_f)
        u_pred.block_until_ready(); dec_t = time.time() - t1
        rom_t = eq_t + dec_t
        ...
        print(f"   case ({k1},{k2},{k3}) FOM {fom_t:.4f}s ROM {rom_t:.4f}s "
              f"(eq={eq_t:.4f}s dec={dec_t:.4f}s) "
              f"speedup {fom_t/rom_t:.2f}x relL2 {rel_l2:.4e} iters={iters}")
