"""Evaluate heat-2D FiLM nets (trained at N=16..256) against the N=512 reference.
...
heat2d_film.py computes its Nyquist-capped N_FREQ from the N env var at import
...
Usage:  python heat2d_convergence_eval.py <checkpoint-dir> [ref-npz] [out-json]
        checkpoint-dir contains heat2d_film_N{16,32,64,128,256}.pkl
"""
...
OUT = sys.argv[3] if len(sys.argv) > 3 else os.path.join(
    CKPT_DIR, "heat2d_convergence.json")
NS = [16, 32, 64, 128, 256]
N_REF = 512
...
results = {}
for N in NS:
    ck = os.path.join(CKPT_DIR, f"heat2d_film_N{N}.pkl")
    ...
    spec = importlib.util.spec_from_file_location(
        f"hf{N}", os.path.join(HERE, "heat2d_film.py"))
    hf = importlib.util.module_from_spec(spec)
    sys.modules[f"hf{N}"] = hf
    spec.loader.exec_module(hf)
    assert hf.EVAL_TIMES == eval_times, "eval-time mismatch vs refgen"
    ...
    z_va = z[sl]
    taus = np.asarray(eval_times, dtype=np.float64) / hf.NUM_STEPS

    with open(ck, "rb") as f:
        params = pickle.load(f)
    params = jax.tree_util.tree_map(jnp.asarray, params)
    net = jax.jit(lambda z_i, tau_i, pts: hf.film_apply(params, z_i, tau_i, pts))
    ...
    rollout, _ = hf.make_rollout(N)
    x_c = np.linspace(0.0, 1.0, N)
    Xc, Yc = np.meshgrid(x_c, x_c, indexing="ij")
    mask_c = np.asarray(hf.boundary_mask(N))
    ...
    for s in range(0, n_val, CHUNK):
        ...
        U0 = np.stack([
            ...
            for i in range(s, e)])
        snaps = np.asarray(rollout(jnp.asarray(U0),
                                   jnp.asarray(ref["kappa"][s:e])))
    ...
    print(f"N={N:4d}  data-floor={e_floor:.3e}  net-vs-ref512={e_net:.3e}  "
          f"(n_freq={hf.N_FREQ})  [{time.time()-t0:.0f}s]", flush=True)
