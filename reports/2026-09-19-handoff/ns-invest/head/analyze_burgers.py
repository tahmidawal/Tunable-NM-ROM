...
out['head_oracle_test_median_n0'] = float(np.median(e_or)); out['head_oracle_t0_median'] = float(np.median(e_or[t_te == 0]))
...
Pte = np.column_stack([mu_te[:, :4], np.log(mu_te[:, 4]), t_te / (T - 1)]); Pte_n = (Pte - P.mean(0)) / P.std(0)
folds = traj_te % 8
for deg in (1, 2):
    X = poly_features(Pte_n, deg); sse = 0.0
    for f in range(8):
        tr, te = folds != f, folds == f
...
sub2 = rng.choice(A_tr.shape[0], 6000, replace=False)
best = None
...
for n_use in (72, 144, 288, 576):
    ...
    idx = np.nonzero(m_)[0]; idx = idx[rng.choice(len(idx), min(6000, len(idx)), replace=False)]
...
json.dump(out, open('analyze_burgers.json', 'w'), indent=1)
