...
| Certified EQ twins of the dense rungs (rules refit on reachable states, certified by held-out $\rho\le0.116$) | **Fail on one leg.** Rungs $q\le64$ certify at $m{=}1024$ and match their dense twins to four decimals at $q{=}0$ and $128$; the $q{=}256$ rule is uncertifiable at $m\le2048$ (crossing extrapolated at $m\approx2522$), so the EQ ladder keeps one violation ($q{=}128\to256$: $0.89\to1.04$ %). Cost within $1.13\times$ of the old rules; the $q{=}256$ EQ error falls $6.07\to1.04$ %. | q-ridge (job 3768168) |

**EQ rules (q-ridge).** Every incumbent rule, fitted on static decoder outputs, fails the $\rho$ bar at every rung. Refitting on reachable states buys $2.7$–$15.7\times$ in $\rho$ at $q\le64$; NNLS fit residual is again anti-correlated with $\rho$. No $\lambda$ ridge and no test-count rule removes the EQ regression, and the ridge at $\lambda_{\rm rel}{=}1$ destroys the $q{=}256$ dense gain. The lane exceeded its 3-job cap by four (two preamble deaths with zero GPU time, two retracted fitter defects, one crash after a complete sweep); all recorded.
...
- Every namespace under `/cluster/tufts/paralab/tawal01/*_2026091{4,5,6}` is empty except `no_burgers_20260914/pilot-data01` (Burgers dataset cache) and `q_ridge_20260916` (now empty).
