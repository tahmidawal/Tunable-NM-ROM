#!/bin/bash
# rsync one staged job dir to the codesign-mm cluster namespace and submit it.
# Namespace is distinct from exlin/ and burgacc/ (one experiment = one
# namespace; shared account).  Usage: ./push_cdmm.sh <jobname> <sbatch-file>
set -euo pipefail
JOB=${1:?jobname}; SB=${2:?sbatch file}
HERE=$(cd "$(dirname "$0")" && pwd)
REM=/cluster/tufts/paralab/tawal01/codesign-mm/$JOB
