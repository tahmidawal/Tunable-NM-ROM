...
WT = os.path.join(HERE, "..", "Tunable-NM-ROM-Claude", "worktrees")
...
TAB = os.path.join(HERE, "COST_TABLES.md")
...
def table_speed_vs_accuracy(pde, tag, Ns, ladder_k=8):
    md(f"\n## 2. {tag}: the same trained model, tuned for speed or for accuracy\n")
...
if __name__ == "__main__":
    ...
    md("# Cost, tunability, and the hybrid — tables\n")
    md("Generated by `make_cost_figs.py` from the raw run JSONs. "
       "Cost and accuracy in every row come from the same solver invocation.\n")
    md("> **Provisional.** ROM and POD points within one mesh share a GPU and are "
       "directly comparable. The full-order ladder ran as a separate job, so ROM-vs-FOM "
       "ratios here are cross-GPU and will be superseded by the consolidation run.\n")
    ...
    for N in (64, 256):
        fig_pareto("poisson2d", "Poisson", N)
    fig_pareto("burgers2d", "Burgers", 64)
