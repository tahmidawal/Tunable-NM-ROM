...
def main():
    ...
    for n in NS:
            ...
            for trajectory in trajectory_order:
                for arm in order:
                    ...
                    if (
                        ...
                        raise SystemExit(f"N={n} {arm}: unhealthy timed solve {record}")
        ...
        cubic_pairs = {
            (r["trajectory_index"], r["repetition"]): r["elapsed_s"]
            for r in records["cubic"]
        }
        for arm_index, arm in enumerate(ARMS):
            elapsed = [r["elapsed_s"] for r in records[arm]]
            delta_linear = [
                linear_pairs[(r["trajectory_index"], r["repetition"])] - r["elapsed_s"]
                for r in records[arm]
            ]
            delta_cubic = [
                cubic_pairs[(r["trajectory_index"], r["repetition"])] - r["elapsed_s"]
                for r in records[arm]
            ]
            row = {
                "N": n,
                ...
                "paired_saving_vs_linear_median_ms": float(np.median(delta_linear) * 1e3),
                "paired_saving_vs_linear_median_95ci_ms": [
                    1e3 * v for v in bootstrap_ci(delta_linear, 20260819 + n + arm_index)
                ],
                "paired_saving_vs_cubic_s": delta_cubic,
                "paired_saving_vs_cubic_median_ms": float(np.median(delta_cubic) * 1e3),
                "paired_saving_vs_cubic_median_95ci_ms": [
                    1e3 * v for v in bootstrap_ci(delta_cubic, 20260829 + n + arm_index)
                ],
            ...
            bc.log(
                f"N={n} {arm:10s}: {row['timing_median_ms']:.2f}ms "
        ...
        report["equivalence"][str(n)] = bc.reference_equivalence(
            n,
            trajectories,
            chain,
            lin_tol=LINEAR_TOL,
