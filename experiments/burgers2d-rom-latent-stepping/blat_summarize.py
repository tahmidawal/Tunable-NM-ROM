"""Markdown tables from runs/*/blat_rom_*.json (+ stage-1 JSONs)."""
import glob, json, os, sys
import numpy as np

def f(x): return "—" if x is None or (isinstance(x, float) and not np.isfinite(x)) else f"{x:.2e}"
rows = []
cells = sorted(glob.glob("runs/ad_*/blat_rom_*.json"))
print("## Stage 2 — latent-stepping ROM (held-out TEST_SEED trajectories, 16 each)\n")
for c in cells:
    ...
    print(f"### N={N}, K={K}  ({gpu}; job {os.path.basename(glob.glob(os.path.dirname(c)+'/*.out')[0]).split('.')[0]})\n")
    print(f"auto-decoder TRAIN recon {f(r['train_rel_mean'])} · ORACLE inferred-latent floor (held-out) {f(r['oracle_inferred_latent_test']['traj_rel_mean'])} · IC-fit misfit (u0, cold start) {f(r['ic_fit']['rel_mean'])} · max FOM rel residual {r['max_fom_rel_residual']:.1e} · FOM rollout {fom*1e3:.0f} ms\n")
    print("| variant (solver:colloc:objective) | m | traj rel-L2 mean | median | max | blow-ups | iters cold / warm | step ms | rollout ms | speedup (rollout) | end-to-end* |")
    print("|---|---|---|---|---|---|---|---|---|---|---|")
    for v, s in r["rom"].items():
        t = r["timing"].get(v, {})
        print(f"| `{v}` | {s['m']} | {f(s['traj_rel_mean'])} | {f(s['traj_rel_median'])} | {f(s['traj_rel_max'])} | {s['n_blowup']}/{s['n_total']} | {s['iters_cold_step0']:.1f} / {s['iters_warm_mean']:.1f} | {s['step_time_ms_median']:.1f} | {t.get('rollout_s_median',float('nan'))*1e3:.0f} | {t.get('speedup_vs_fom_rollout_only',float('nan')):.2f}x | {t.get('speedup_vs_fom_end_to_end',float('nan')):.2f}x |")
    ...
    for kv, s in r["pod_rom"].items():
        ...
        print(f"| {k[1:]} | `{var}` | {f(s['traj_rel_mean'])} | {f(s['traj_rel_median'])} | {s['iters_warm_mean']:.1f} | {s['step_time_ms_median']:.1f} | {t.get('rollout_s_median',float('nan'))*1e3:.0f} | {t.get('speedup_vs_fom_rollout_only',float('nan')):.2f}x |")
    ...
    for v in ("lspg:full:fd", "lspg:full:weak64", "lspg:eq256:weak64", "lspg:full:weakc64"):
        if v in r["rom"]:
...
for arm, lab in (("icw_sqrt50", "sqrt(50)"), ("icw1", "1")):
    r = json.load(open(f"runs/s1_n64/{arm}/blat_stage1_N64.json"))
