"""2D Poisson decoder-architecture diagnostic (one resolution per run).

Problem:  -Lap(u) = a * exp(-((x-cx)^2+(y-cy)^2)/(2 w^2)),  u|_boundary = 0
on [0,1]^2.  cx,cy ~ U[0.15,0.85], w ~ logU[0.02,0.1], a ~ U[0.5,2].
Ground truth: 5-point FD + matrix-free CG in float64 (data matches operator),
regenerated from the seed at run start.

Arms (decoders conditioned on true normalized params z in R^4, no encoder):
  POD-r      : SVD floor for r in a sweep (yardsticks: r=4 equal-dim, r=24).
  grid-tied  : MLP(z)->h in R^24 ; u = h @ W, W in R^{24 x N^2} learned.
  coord-net  : u(x,y;z) = MLP([ff(x), ff(y), z]), params independent of N.

Both arms train on the same per-step random point subsets (P<=4096) so the
step cost is resolution-independent and identical between arms.
Final eval: full-grid mean rel-L2 in float64 on the val set.

Usage:  N=64 python poisson2d_diag.py [outdir]
"""
...
def neg_lap_interior(u_int, n):
    """-Laplacian on interior nodes (Dirichlet-0 boundary), u_int: (n-2, n-2)."""
    dx = 1.0 / (n - 1)
    u = jnp.pad(u_int, 1)
...
def fd_solve_batch(F_int_batch, n):
    """Solve -Lap u = F on interior via CG, float64. F_int_batch: (B, n-2, n-2)."""
...
def build_snapshots(n):
    """Return U (m, n*n) float64 fields (zero on boundary) and z (m,4) float32."""
    ...
    # residual check on a few samples
    res = []
...
def train_model(apply_at_idx, params, U_tr32, z_tr32, U_va32, z_va32, coords32,
                n_nodes, tag):
    """apply_at_idx(params, z, idx) -> predictions at the given flat node indices."""
    ...
    @jax.jit
    def val_loss(params):
        preds = jax.vmap(lambda z: apply_at_idx(params, z, all_idx))(z_va32)
        return jnp.mean(jax.vmap(rel_l2_sq)(preds, U_va32))
...
def eval_full(apply_at_idx, params, U_val, z_val, n_nodes):
    all_idx = jnp.arange(n_nodes)
    preds = jax.vmap(lambda z: apply_at_idx(params, z, all_idx))(jnp.asarray(z_val))
...
def main():
    ...
    # POD floors (f64 SVD on the training snapshots).
    t0 = time.time()
    ...
    U_tr32 = jnp.asarray(U_tr, dtype=jnp.float32)
    U_va32 = jnp.asarray(U_va, dtype=jnp.float32)
    z_tr32 = jnp.asarray(z_tr)
    z_va32 = jnp.asarray(z_va)
    x = np.linspace(0.0, 1.0, N)
    X, Y = np.meshgrid(x, x, indexing="ij")
    ...
    # Arm A: grid-tied.
    modelA = GridTiedDecoder(n_nodes=n_nodes, rank=RANK)
    pA = modelA.init(rng, jnp.zeros((4,), jnp.float32), jnp.arange(4))["params"]
    applyA = lambda p, zz, idx: modelA.apply({"params": p}, zz, idx)
    pA = train_model(applyA, pA, U_tr32, z_tr32, U_va32, z_va32, coords32,
                     n_nodes, "grid-tied")
    errA = eval_full(applyA, pA, U_va, z_va, n_nodes)
    nA = sum(int(np.prod(v.shape)) for v in jax.tree_util.tree_leaves(pA))

    # Arm B: coord-net.
    modelB = CoordDecoder()
    pB = modelB.init(rng, coords32[:4], jnp.zeros((4,), jnp.float32))["params"]
    applyB = lambda p, zz, idx: modelB.apply({"params": p}, coords32[idx], zz)
    pB = train_model(applyB, pB, U_tr32, z_tr32, U_va32, z_va32, coords32,
                     n_nodes, "coord-net")
    errB = eval_full(applyB, pB, U_va, z_va, n_nodes)
    nB = sum(int(np.prod(v.shape)) for v in jax.tree_util.tree_leaves(pB))

    print(f"RESULT N={N}  POD-4={pods[4]:.3e}  POD-24={pods[24]:.3e}  "
          f"grid-tied={errA:.3e} ({nA} params)  coord-net={errB:.3e} ({nB} params)",
          flush=True)
    ...
    with open(os.path.join(OUTDIR, f"results_2d_N{N}.json"), "w") as f:
        json.dump({"N": N, "backend": backend, "pod": pods,
                   ...
                   "params_grid": nA, "params_coord": nB,
