...
$^{w}$ in Table~\ref{tab:headline}. Speedups in a block share the FOM
row, so the $q=0$ ratio is not Table~\ref{tab:headline}'s fast column.
...
tradeoff. Table~\ref{tab:tunability} compares every setting with one
...
Table~\ref{tab:headline}, and the unconfirmed arms at
$\nBhSubErrLo$--$\nBhSubErrHi\,\%$ run at
