...
\label{tab:heat}
\input{tables/T11d_heat}
\end{table}

\begin{table}[h]
\centering
\caption{Neural operators on the shared Burgers data: capacity, epochs, whether
...
the \nNsCellArms{} settings, so the ladder and timing were never run for
...
held-out/training gap falls from \nNsDataBaseGap{} to
\nNsDataHeldoutOverTrain: the head no longer fits the training data either,
...
\item[FNO, U-Net, Transolver] Three neural-operator families. Each is fixed once trained; its evaluation grid is a cost--accuracy control of its own (Table~\ref{tab:resolution}), but one that does not change what the model can represent.
