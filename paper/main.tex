...
(\S\ref{sec:results:tunability} gives the cost of each rank). At $4096^2$ the accurate setting chosen on six development cases
...
Table~\ref{tab:headline-times} rise from $2048^2$ to $4096^2$ as well.

\begin{table}[t]
\centering
\caption{Other nonlinear-manifold ROMs on the shared Burgers 2D family:
...
On 32 held-out Burgers cases (Table~\ref{tab:nmrom-baselines}) our
...
\provPanelJob{} ($256^2$) and \provPanelTenTwentyFourJob{} ($1024^2$);
Table~\ref{tab:figure1-table} is a different allocation and gives a
different ratio.}
