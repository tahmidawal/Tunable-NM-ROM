#!/bin/bash
# rsync one staged job dir to the burgacc cluster namespace and submit it.
# Usage: ./push_burgacc.sh <jobname> <sbatch-file>
set -euo pipefail
JOB=${1:?jobname}; SB=${2:?sbatch file}
HERE=$(cd "$(dirname "$0")" && pwd)
REM=/cluster/tufts/paralab/tawal01/burgacc/$JOB
echo "--- squeue BEFORE ---"; ssh -o BatchMode=yes tufts-login "squeue -u tawal01 -h -o '%i %j %T'" || true
ssh -o BatchMode=yes tufts-login "mkdir -p $REM"
