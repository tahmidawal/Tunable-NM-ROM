...
def main():
    ...
    for bc,(_,run,entry) in boundaries.items():
        ...
        temporal_caveat=(f"The latter comparison is qualified by {unresolved} head/repeat runs failing the original timestep check; their temporal accuracy remains unresolved at that step." if unresolved else "All original head/repeat runs pass the timestep check.")
        lines.append(f"In `{bc}`, the MLP has {capacities['mlp']} head parameters and the quadratic head has {capacities['quadratic']}; this is not a parameter-matched architecture comparison. The {repeats} optimizer repeats share the same data. The velocity penalty improves mean tangent fitting in {sum(p[0] for p in pairs)} of {len(pairs)} matched comparisons, while median trajectory energy-state error worsens in {sum(p[1] for p in pairs)} at the original primary step. {temporal_caveat} Selected latent fits include {selected_nonstationary} nonstationary states; these remain failures of the declared fitting gate.")
    if refined:
