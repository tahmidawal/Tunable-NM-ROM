...
Figure-run provenance: `runs/b2dtensor/figs/sep_b2d_tensor_figs_n{64,256}.json` + `run_n{64,256}.log`
(local GB10, `jaxrun`, unpatched truth generator — fine at N<=256), `runs/b2dtensor/figs1024/`
(cluster job 3049945, A100-80GB pax007; the first attempt 3049917 failed on a stale `ROOT` path in the
sbatch, log kept).  The figure runs regenerate only the test trajectories (`SKIP_TRAIN_TRUTH=1`), use
1 timed repetition and skip the FOM ladder; their mean errors reproduce the main jobs' T-4 values to
the printed digits (the rollouts are deterministic given the checkpoint, seeds and node set).
<!-- FIGURES-END -->
