...
for c in cells:
    ...
    print("| variant (solver:colloc:objective) | m | traj rel-L2 mean | median | max | blow-ups | iters cold / warm | step ms | rollout ms | speedup (rollout) | end-to-end (py IC / jit IC) |")
    print("|---|---|---|---|---|---|---|---|---|---|---|")
    for v, s in r["rom"].items():
        t = r["timing"].get(v, {})
        print(f"| `{v}` | {s['m']} | {f(s['traj_rel_mean'])} | {f(s['traj_rel_median'])} | {f(s['traj_rel_max'])} | {s['n_blowup']}/{s['n_total']} | {s['iters_cold_step0']:.1f} / {s['iters_warm_mean']:.1f} | {s['step_time_ms_median']:.1f} | {t.get('rollout_s_median',float('nan'))*1e3:.0f} | {t.get('speedup_vs_fom_rollout_only',float('nan')):.2f}x | {t.get('speedup_vs_fom_end_to_end',float('nan')):.2f}x / {t.get('speedup_vs_fom_end_to_end_jit_ic',float('nan')):.2f}x |")
