...
| stopping rule | `||r|| <= max(GN_RTOL·||r(z_n)||, GN_TOL·rms(u0)·tol_scale)`, `GN_RTOL=1e-6`, `GN_TOL=1e-9`. The Burgers reference used the absolute term alone; heat **decays by >1e3 over the horizon**, so a tolerance pinned to `u0` is ~1/decay looser in relative terms at late times and is not comparable across `M` (an `M`-mode projection of an isotropic residual carries only `sqrt(M/(N-2)^2)` of its norm). The Galerkin root uses the matching relative criterion `||g|| <= GN_RTOL·||g_0||`. Both `||r||` and `||r||/||r(z_n)||` are reported. |
