...
\begin{frame}{Architecture with the tensor --- what changes, what does not}
  ...
  \begin{tikzpicture}[node distance=5mm and 9mm]
    ...
    \node[fz, below left=7mm and -8mm of $(H.south)!0.5!(G.south)$] (LIN) {\textbf{linear terms --- exact}\\$B=\Phi\T G$ precomputed once ($M\times R$)\\$\Phi\T u = B\,h(z)$};
    \node[fz, below right=7mm and -8mm of $(H.south)!0.5!(G.south)$] (TEN) {\textbf{quadratic term --- exact}\\$T=\Phi\T[G\otimes D^-G]$ precomputed once ($M\times R\times R$)\\$\Phi\T(u\,u_x) = h\T T\,h$};
