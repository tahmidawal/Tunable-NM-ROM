"""ROUND 5, Burgers -- CONFIRMATION of a coefficient-space-refined decoder.

This driver is `sep_burgers_r4.py` with exactly one capability added: when
`LOAD_CKPT` is set it SKIPS TRAINING and loads a decoder instead.  Everything
else -- data regeneration from the canonical seed with the incumbent generator
and its <=1e-8 truth gate, the span diagnostics, EQ construction (including the
tail-capped NNLS arm), gate 0, the IC encoder, the rollout, the standardised
(newton_tol, lin_tol) classical ladder, matched-accuracy pairing and the batch
sweep -- is byte-identical to the round-4 driver, so an r5 row and an r4 row
are directly comparable.

Why: round 5 refines h in the frozen bank's coefficient space (see
`sep_coeff_extract.py` and `sep_hfit.py`).  That produces a decoder with the
SAME g, the SAME bank, the SAME cached-collapse property and a new h, so the
only thing that has to be re-measured on the cluster is the part the identity
(*) cannot reach: the EQ quadrature, the actual weak solve, the rollout, and
the timing.  The rungs above the rollout (recon, span floor, oracle) are
computed exactly from the extraction npz and are reported alongside.

With LOAD_CKPT unset this file reproduces `sep_burgers_r4.py` exactly.

--- original round-4 header follows ---

ROUND 4, Burgers -- ACCURACY: attack h, not the span.

Round 3 (runs/push_r3{b,c,d}, jobs 2835789/90/2835794) settled where the error
lives.  `G_HIDDEN >= 2R` lifted the rank cap: the trained bank now has
numerical rank 512/512 and its own least-squares floor on fresh test states is
2.60e-4 at N=1024 (4.04e-4 at N=256, 1.15e-4 at R=1024) -- all BELOW the 1e-3
target.  Yet the K=16 per-state oracle only reaches 8.2e-3 and reconstruction
7.3e-3.  The span is not the constraint any more; the map h: R^K -> R^R is,
by a factor of ~32.

Since the Burgers family is intrinsically ~6-dimensional (5 sampled parameters
plus time) K=16 is not the shortage either, so this round varies h's capacity
and its optimisation, plus the two things the round-3 numbers flagged
directly: the error is concentrated in the sharp early-time states (oracle
3.5e-2 at t=0 against ~5.9e-3 at t=50), and the ctrl EQ set is too coarse for
a 1e-3 claim (rel fit 6.08e-3 at N=1024, worst row 9.8e2, single-step
weak-opt/oracle 1.119 mean / 4.02 max).

New knobs over `sep_burgers_r3.py`, all default-off so this file reproduces
round 3 exactly when they are unset:

  H_HIDDEN / H_LAYERS  wider and deeper h (already in `arch_from_env`)
  K                    24 / 32 arms -- the round-3 cost analysis says K is
                       nearly free online, so this should not cost speed
  W_EARLY              extra loss weight on states with t <= T_EARLY
  Z_POLISH             code-only refinement steps at frozen weights (the
                       per-snapshot codes are free variables; joint Adam may
                       leave them short of the best code for the FINAL span)
  EQ_M_FACTOR          quadrature points per test mode (default 4 = incumbent)
  EQ_TAIL              tail-capped NNLS (up-weight badly fitted rows) instead
                       of the reference fit, to attack the EQ worst-row tail

Everything else is unchanged and still asserted: incumbent discretization,
residual and Jacobian definitions; gate 0 (<=1e-12) per EQ set; no test truth
in any solve path; end-to-end timing including the IC fit and the full-grid
decode with all raw repetitions retained.

--- original round-3 header follows ---
...
W_EARLY = float(os.environ.get("W_EARLY", "1.0"))
Z_POLISH = int(os.environ.get("Z_POLISH", "0"))
Z_POLISH_LR = float(os.environ.get("Z_POLISH_LR", "3e-3"))
...
# ROUND 5: load an already-trained (or coefficient-space refined) decoder
# instead of training one.  Unset => identical behaviour to round 4.
...
def main():
    ...
    TAG = OUT_TAG or (f"N{N}_K{K}_R{R}_h{ARCH.get('h_hidden', 128)}"
                      f"x{ARCH.get('h_layers', 2)}")
    OUT = f"{OUT_PREFIX}sep_burgers_r5_{TAG}.json"
    CKPT = f"{OUT_PREFIX}sep_burgers_r5_{TAG}.pkl"
    ...
    report = dict(config=dict(
        pde="burgers2d", round=ROUND, load_ckpt=LOAD_CKPT, N=N, k=K, r=R, steps=STEPS, lr=LR,
        ...
        w_early=W_EARLY, z_polish=Z_POLISH, z_polish_lr=Z_POLISH_LR,
        eq_m_factor=EQ_M_FACTOR, eq_tail=bool(EQ_TAIL),
        eq_tail_cap=EQ_TAIL_CAP, eq_tail_rounds=EQ_TAIL_ROUNDS,
    ...
    # extra loss weight on the sharp early-time states (round-4 lever)
    w_extra = None
    if W_EARLY != 1.0:
        w_extra = np.where(tidx_of[pick] <= T_EARLY, W_EARLY, 1.0)
        sc.log(f"  early-time loss weight {W_EARLY} on "
               f"{int(np.sum(w_extra > 1.0))}/{pick.size} states")
    if LOAD_CKPT:
        params, Z_tr, cfg_in = sc.load_pkl(LOAD_CKPT)
        assert Z_tr.shape[0] == pick.size, (
            f"loaded codes {Z_tr.shape[0]} != state pick {pick.size}")
        assert Z_tr.shape[1] == K, f"loaded K={Z_tr.shape[1]} != K={K}"
        # exact per-snapshot reconstruction of the LOADED decoder on the same
        # training states, computed the same way the trainer reports it
        Gp = sc.features(params, jnp.asarray(coords_pool, dtype=F64))
        Hp = sc.head(params, jnp.asarray(Z_tr))
        per = []
        for s_ in range(0, pick.size, 2048):
            e_ = min(s_ + 2048, pick.size)
            Uh_ = Hp[s_:e_] @ Gp.T
            Ur_ = jnp.asarray(S_tr[s_:e_])
            per.append(np.asarray(jnp.linalg.norm(Uh_ - Ur_, axis=1)
                                  / jnp.linalg.norm(Ur_, axis=1)))
        per = np.concatenate(per)
        del Gp, Hp
        tinfo = dict(loaded=True, ckpt=os.path.basename(LOAD_CKPT),
                     ckpt_cfg=cfg_in, steps=0, seconds=0.0,
                     recon_rel_l2_mean=float(per.mean()),
                     recon_rel_l2_max=float(per.max()),
                     n_snapshots=int(pick.size),
                     n_points=int(coords_pool.shape[0]),
                     note="TRAINING SKIPPED: decoder loaded from LOAD_CKPT")
        sc.log(f"  LOADED decoder {os.path.basename(LOAD_CKPT)}: recon "
               f"rel-L2 mean {per.mean():.4e} max {per.max():.4e}")
    else:
        params, Z_tr, tinfo = ss.train_autodecoder_v2(
            jax.random.PRNGKey(SEED0), coords_pool, S_tr, K, R,
            steps=STEPS, lr=LR, lam_orth=LAM_ORTH, weight_decay=WD,
            p_sub=P_SUB, ema_decay=EMA_DECAY, full_last=FULL_LAST,
            time_cap=TIME_CAP, snap_norm=bool(SNAP_NORM), w_extra=w_extra,
            z_polish=Z_POLISH, z_polish_lr=Z_POLISH_LR,
            tag=f"burgers r5 N={N} k={K} r={R}", **ARCH)
    report["train"] = tinfo
    ...
    for Mi in EQ_MS:
        ...
        lbl = f"sep burgers r4 N={N} k={K} M={Mi} m={m_want}"
        if EQ_TAIL:
    ...
    save()
    sc.log(f"DONE burgers r4 N={N} [{time.time()-t_all:.0f}s] -> {OUT}")
