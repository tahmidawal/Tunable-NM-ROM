...
\begin{table}[h]
\centering
\caption{The blocks of Figure~\ref{fig:arch}. ``Fixed when'' distinguishes training, preparation and online work.}
...
quadrature rules & NNLS fit on reachable states, one rule per mesh and test space, validated by held-out $\rho$ & $m$ nodes & prepared offline \\
...
\begin{table}[htbp]
\centering
\caption{Meaningful deployment controls. Training parameters and model
...
\toprule
Control & Measured role \\
\midrule
Correction rank $q$ & Enlarges the trial space without retraining; can improve accuracy at greater solve cost. \\
Solver tolerance & Controls convergence; field-error gains plateau when representation error dominates. \\
Iteration limit & Bounds work; an early-stopped solve retains its numerical status. \\
EQ sample count & Selects a validated stored quadrature rule where EQ is used; chiefly changes residual cost. \\
\bottomrule
\end{tabular}
\end{table}
