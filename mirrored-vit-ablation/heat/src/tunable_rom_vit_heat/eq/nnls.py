...
def build_v_eq(
    ...
    """ViT-decoder ablation: no CP precomputation; only stencil indices.

    The symmetric ViT decoder couples all output tokens through self-
    attention, so there is no per-node CP factorisation. The solver
    evaluates the decoder on the full grid each GN iteration and gathers
    at stencil nodes via these indices. v_eq_st is None.

    Returns:
        None, (n_eq, 2d+1) stencil flat indices.
    """
    offsets = _stencil_offsets(N, spatial_dim)
    stencil_indices = eq_flat_indices[:, None] + offsets[None, :]
    return None, stencil_indices
