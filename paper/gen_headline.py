...
bl = [r'% GENERATED by paper/gen_headline.py from the nmrom-baselines lane (hash-pinned) -- do not edit.', r'\small',
      ...
      r'Method ($256^2$) & Unknowns & Worst (\%) & Median (\%) & Memory (MB) \\', r'\midrule']
bmd = []; appx_nb = []
if NBP:
    import re as _re
    fam, fa, gate, ga = NBP['fam256'], NBP['fam256-audit'], NBP['gate05'], NBP['gate05-audit']
    assert fam['backend'] == 'gpu' and fam['x64'] and fam['matmul'] == 'highest' and fam['intervals'] == 256
    ...
    assert fam['gate_binding']['gate']['nm_lspg_median'] == gate['gate']['nm_lspg_median']      # the family job is bound to this gate
    for f in ('kimae.py', 'lspg.py'):                                                                # admissible = same code as the passed gate ...
        assert fam['source_sha256']['experiments/nmrom-baselines/' + f] == gate['source_sha256'][f]
    act = gate['gate']['recipe']['act']                                                              # ... and the gate activation
    audited = {r['arm']: r for r in fa['rows']}
    A = fam['arms']
    plan = [('Kim et al. NM-LSPG', 'kim_final_K8'), ('Kim et al. NM-LSPG', 'kim_final_K16'), ('Kim et al. NM-LSPG', 'kim_final_K32'),
            ('Kim et al. NM-LSPG, data-matched', 'kim_final_K16_fit576'),
            ('POD-LSPG', 'pod_lspg_zero_K8'), ('POD-LSPG', 'pod_lspg_zero_K16'), ('POD-LSPG', 'pod_lspg_zero_K32'),
    ...
    for label, arm in plan:
        ...
        assert audited[arm]['audited'] and audited[arm]['agree'] and audited[arm]['recomputed'] == a['worst_evolved'], arm
        assert max(a['per_case_evolved']) == a['worst_evolved']
        k = a.get('solved_dimension', a['K'])
        T2.append(dict(label=label, arm=arm, family=a['family'], k=k, worst=100 * a['worst_evolved'], median=100 * a['median_evolved'],
                       mb=a['memory_analysis']['total'] / 1e6, ms=a['timing']['gpu_ms_median']))
    ours = [r for r in T2 if r['family'] == 'ours']; kim = [r for r in T2 if r['family'].startswith('kim')]; pod = [r for r in T2 if r['family'] == 'pod_lspg']
    assert max(r['worst'] for r in ours) < min(r['worst'] for r in kim + pod)       # the abstract's qualitative clause
    seen = None
    for r in T2:
        grp = r['family']
        if seen and grp != seen: bl.append(r'\addlinespace')
        seen = grp
        lab = r['label']
    ...
    for K in (8, 16, 32):
        a = A[f'kim_final_K{K}_hr']; assert a['finite'] and a['activation'] == act
        appx_nb.append(dict(label='Kim et al. NM-LSPG-HR (exploratory)', arm=f'kim_final_K{K}_hr', family='kim_hr', k=K, worst=100 * a['worst_evolved'],
    ...
    NB.update(nBaseOursFastWorst=e(A['ours_q0']['worst_evolved'] * 100), nBaseOursAccWorst=e(A['ours_q256']['worst_evolved'] * 100),
              ...
              nBaseEncoderNeedGB=f"{drop['need_gb']:.0f}", nBaseDeviceGB=f"{drop['device_limit_gb']:.0f}",
              nBaseDataMatched=str(fam['training']['kim_final_K16_fit576']['fit_trajectories']), nBaseGateAct=act)
    assert gate['gate']['attempt'] == 3 and 'sigmoid' == act
else:
...
write('TH_nmrom_baselines', bl, ['Method (256^2, 32 held-out cases)', 'Solved unknowns', 'Worst evolved (%)', 'Median evolved (%)', 'Query memory (MB)'], bmd)
if appx_nb:
    al = [r'% GENERATED by paper/gen_headline.py -- do not edit.', r'\small', r'\begin{tabular}{@{}llrrrrr@{}}', r'\toprule',
          r'Method & Residual path & $k$ & Worst (\%) & Median (\%) & GPU ms (this job) & MB \\', r'\midrule']
    amd = []
    for r in appx_nb:
        c = [r['label'], r['path'], str(r['k']), e(r['worst']), e(r['median']), f"{r['ms']:.0f}", f"{r['mb']:.0f}"]
        al.append(' & '.join(c) + r' \\'); amd.append([md(x) for x in c])
    al += [r'\bottomrule', r'\end{tabular}']
    write('TH_nmrom_baselines_appx', al, ['Method', 'Residual path', 'k', 'Worst (%)', 'Median (%)', 'GPU ms (this job)', 'MB'], amd)
