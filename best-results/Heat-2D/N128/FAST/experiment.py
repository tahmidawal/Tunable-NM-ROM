...
class Hyperparams:
    ...
    N: int = 128
    k_dim: int = 64
    rank: int = 256            # Run3 (Run9-original-recipe) best — rank=512 didn't help
    hidden_dim: int = 256
    patch_size: int = 8
    embed_dim: int = 96
    num_heads: int = 4
    ...
    # Pareto frontier explored across max_iters/gn_tol — see results.tsv. Default
    # to the SPEED ANCHOR (Run9: tol=2e-2 max=10) — best speedup at acceptable val.
    gn_max_iters: int = 10
    gn_tol: float = 2e-2
    cg_tol: float = 1e-6
...
def _load_training_snapshots(hp):
    ...
    Generates training_data_2d_128.pkl automatically on first call."""
    payload = fixed._load_or_generate(hp.N)
