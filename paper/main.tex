...
error $\nQxmErrSpan\times$ for $\nQxmCostSpan\times$ the runtime; the
...
\emph{accurate} the largest stored correction rank; no network is
...
(Table~\ref{tab:heat-hires}). The earlier Burgers model has one
...
higher and are not plotted (Table~\ref{tab:headline}).}
\label{fig:speedup}
\end{figure}
...
FOM up to $1024^2$ (\S\ref{sec:results:tunability} gives the cost of each
...
Figure~\ref{fig:speedup} shows the pattern common to all problems: the
...
over 32 held-out cases and compiled-query memory, one allocation. The shallow masked-autoencoder NM-LSPG of \citet{KimChoi2022NMROM} passed
...
\input{tables/TH_nmrom_baselines}
\end{table}

At matched latent dimension on 32 held-out Burgers cases
(Table~\ref{tab:nmrom-baselines}), our worst error is
...
Table~\ref{tab:headline}, as for its held-out Burgers rows.
...
test count fixed at $M=\nQxmFixedM$, dense residual. Errors are worst
...
\nSealedUnconvergedCells. Table~\ref{tab:sealed} retains every
