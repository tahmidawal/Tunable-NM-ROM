# Improving separable NM-ROM accuracy and complete-query cost
...
| --- | --- | --- | --- | --- |
| Heat | 1024 / 12 | 4.5555 | 27.733 / 21.411 | Current L2, full grid; Tested envelope |
| Burgers | 1024 / 4 | 4.6351 | 49.463 / 24.903 | Initial L2, full grid; Tested envelope |
| Poisson | 512 / 30 | 6.8016 | 4.018 / 2.267 | Steady L2, common grid; Tested envelope |
| Reflective wave | 512 / 2 | 6.2029 | 1217.242 / 61.584 | Initial wave state, common grid; Same grid only |
| Absorbing wave | 512 / 2 | 5.9132 | 1336.639 / 277.790 | Initial wave state, common grid; Same grid only |
...
The current method performs 20 sequential Crank–Nicolson steps with a nonlinear fit after each step. Instead, propagate the coefficients directly and fit the decoder only at the 5 requested later output times:
...
At 512 intervals on all 30 development cases, the selected `original_relative` checkpoint gives:

| Diagnostic | Worst relative error (%) |
| --- | --- |
| Unrestricted bank projection, full same-grid norm | 5.50243203 |
| Bank projection, common-reference physical diagnostic | 5.50013929 |
| Best recorded stationary head fit, common-reference norm | 6.80154998 |
| Deployed weak solve, common-reference norm | 6.80155740 |
...
The same-job finer/current-step controls at 1024 intervals are:

| Step | Worst full-grid error (%) | Complete query (ms) | Evolution / first-step budget exits |
| --- | --- | --- | --- |
| 0.005 | 3.9076 | 54.570 | 0 / 0 |
| 0.01 | 4.6351 | 49.463 | 6 / 6 |

These remain provisional development results. Budget-exit counts include every timing repetition; empirical physical qualification is not a stationarity certificate. A 9.36% reduction in the finer-step complete-query median would recover its better accuracy at the present coarser-step cost. That is a derived screening target, not a predicted acceleration or an FOM win.
...
For the latest first-seed larger head at 512 intervals, subtracting evolution from each recorded complete-query repetition gives:

| Boundary | Case | ROM query minus evolution, median (ms) | Same-job FOM query, median (ms) |
| --- | --- | --- | --- |
| Reflective | 0 | 86.195 | 61.794 |
| Reflective | 1 | 84.031 | 61.374 |
| Absorbing | 0 | 87.565 | 273.675 |
| Absorbing | 1 | 74.657 | 281.905 |
