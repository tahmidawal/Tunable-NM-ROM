...
representation \citep{LeeCarlberg2020,KimChoi2022NMROM}, although their
...
full-order solve grows with the mesh. We ask: \emph{can one trained
...
largely yes (Table~\ref{tab:headline}, Figure~\ref{fig:speedup}): the
...
(Table~\ref{tab:nmrom-baselines}). It is not yet so for
three-dimensional Burgers and heat, Navier--Stokes or the full wave
state (Table~\ref{tab:failures}).
...
\item \textbf{Resolution-robust speedups over named iterative solvers.}
...
step, and (in the settings listed in Table~\ref{tab:headline-times}) solve the normal
...
quadrature: $^{s}$\,stored rule of job \provEqtopLadderJob{} that passed the
held-out bar in its single draw and was not re-drawn (it is not the
$q=256$ rule of Table~\ref{tab:eqladder}); $^{\ell}$\,deterministic
...
separate run and are not the held-out cohort. Complete-query speedups: Table~\ref{tab:headline-times}. Dashes mark settings not
...
of Table~\ref{tab:headline}. The grey accurate Burgers point at $1024^2$ is dense (no validated
...
Table~\ref{tab:headline} are not plotted. The legend applies to both panels.}
\label{fig:speedup}
\end{figure}
...
Appendix~\ref{app:cg-current}). A certified quadrature rule at
...
Table~\ref{tab:failures} collects the four problems that miss their
...
(Table~\ref{tab:heat-hires-times}), but misses the all-times target. The
...
only deployment arguments. Appendix Table~\ref{tab:ladder-main} is the
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_tunability_rank}
\caption{Worst error (evolved times for Burgers and heat) against speed at
...
\nTunePoissonFomLabel, looser than in Table~\ref{tab:headline}). On
Burgers $q$ and $M$ change together ($M\approx4(k+q)$, $M=544$ an
alternative at $q=256$; fixed-$M$ ladder: Table~\ref{tab:ladder-main}).
Star: looser stopping tolerance; grey: rule failed its held-out check.}
\label{fig:tunability}
\end{figure}
...
cases, Figure~\ref{fig:tunability}); on the linear problems the
...
\item Speedups are measured against the named iterative full-order
...
\item Accuracy is bounded by the frozen bank: Burgers development
...
$\nHeatThreeInit\,\%$.
\item The fixed-test-space rank study uses one checkpoint; the
...
\item Quadrature rules are validated on a finite set of reached states,
...
\item Except where marked final or held-out, cohorts are small
...
and the full wave state miss their targets (Table~\ref{tab:failures});
