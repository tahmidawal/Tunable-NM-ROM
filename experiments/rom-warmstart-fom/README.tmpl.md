...
it is as accurate as the ROM, against {{p_baseiters_N512}} for the full solve. (Different
sample populations: the diagnostic runs on {{p_worth_ncases}} cases, the full-solve count
averages {{p_baseiters_ncases}}.) Yet
...
exact to {{p_directerr_N512}} relative error. Taking both operands from one row
(`N = 512`, `tau_FOM = 1e-10`, `rom_tau = {{p_cgdirect_rung_N512}}`): {{p_cgden_N512}} ms for
the direct solve against {{p_cgnum_N512}} ms for CG — **{{p_cgdirect_N512}}x**. Across the
whole ladder the direct solve costs {{p_direct_N32}}–{{p_direct_N512}} ms. On this
...
({{p_best_1em10_N32}}x → {{p_best_1em10_N512}}x at `tau_FOM = 1e-10`). **We did not measure a
...
decode grows from {{p_decode_N32}} to {{p_decode_N512}} ms and the saving falls with tighter
...
- **`err_final` at the tightest tolerance is limited by the reference solution's own
  accuracy.** The reference CG is *requested* at `{{p_ref_tau}}` and *achieves* a true
  relative residual of {{p_ref_res_N128}} at `N = 128`, {{p_ref_res_N256}} at `N = 256` and
  {{p_ref_res_N512}} at `N = 512` — an f64 floor of the FD operator that grows with `N`. The correctness gate is reference-free: every row asserts the
...
- **The best-per-`N` hybrid is post-selected, and these runs cannot bound the resulting
  ...
  speedup is {{p_best_overall}}x, so breaking even would need a ~6.7% improvement, far outside
...
- **Panel/consolidated agreement is complete, and all but one quantity agree**: all eleven
  jobs finished, so {{consistency_checked}} hardware-independent quantities were compared between the fanned-out
...
average {{b_n_traj}} held-out trajectories, but **every Burgers wall clock times trajectory 0
...
function agrees with the archived JSON to {{oc_b_archcheck_N128}}% at `N = 128` — a WALL-TIME
anchor, not a trajectory-state comparison — but the outer loops are not the same code). Old values read from `{{oc_b_old_json}}`:
...
{{p_instr_vs_lib_lo}}–{{p_instr_vs_lib_hi}}% *slower* per solve than `jax.scipy`'s at the same
...
2. On **Burgers**, do not apply a single scalar: the multiplier ranges {{oc_b_mult_min}} to
   ...
   ({{oc_p_mult_min}}–{{oc_p_mult_max}} as per-mesh means, {{oc_p_mult_spread}}% spread; the
   raw spread over every repeated rung is {{oc_p_mult_rungspread}}% but that mixes in
...
> *inside* the eleven reports reads `{{bogus_commit_short}}…`, which is **not an object in
