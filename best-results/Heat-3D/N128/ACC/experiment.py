...
def run_and_benchmark():
    ...
    n_eq_samples   = 16       # training snapshots used for EQ NNLS
    outdir         = Path('/cluster/tufts/paralab/tawal01/NMROM-Apr8/20260423-NEURIPS/Autoresearch/Heat-3D/runs/N128_run18')
    finetune_from  = Path('/cluster/tufts/paralab/tawal01/NMROM-Apr8/20260423-NEURIPS/Autoresearch/Heat-3D/runs/N128_run10/checkpoint_vitcp.pkl')
    kappa_weight   = False    # Fix 10: upweight stiff (low-κ) samples
    consistency_alpha = 0.1   # Fix 12: 2-snapshot temporal consistency loss weight
    seed           = 0
    # ───────────────────────────────────────────────────────────────────────
    ...
    finetune_init = None
    if ckpt_path.exists():
        # ── Checkpoint reuse: load params, skip training ────────────────────
        print(f"[ckpt-skip] Found checkpoint at {ckpt_path} — loading and skipping training")
        with open(ckpt_path, 'rb') as f:
            _ckpt = pickle.load(f)
        params = _ckpt['params']
        training_seconds = 0.0
        # Recompute val loss for reporting
        v_sum = 0.0
        ...
        best_val = v_sum / n_val_data
        print(f"[ckpt-skip] Recomputed val_rel_l2 = {best_val:.4e}")
    else:
        # Fine-tune mode: outdir is fresh but finetune_from points to a prior ckpt → load it as init
        if 'finetune_from' in dir() and finetune_from is not None and Path(finetune_from).exists():
            print(f"[finetune] Initializing params from {finetune_from}")
            with open(finetune_from, 'rb') as f:
                _ckpt = pickle.load(f)
            finetune_init = _ckpt['params']
        if finetune_init is not None:
            params = finetune_init
        ...
        # Fix 10 — κ-weighted training loss to upweight stiff (low-κ) samples.
        # Maps each U_train index to its trajectory's κ via traj_starts, then
        # applies w(κ) = clip((κ_ref/κ)^0.5, 1, 4) with κ_ref=0.05.
        # NM-ROM theory unchanged — this is just a data-distribution reweighting.
        kappa_weight_enabled = bool(kappa_weight)
        if kappa_weight_enabled:
            traj_starts_arr = np.array(data['traj_starts'])
            traj_kappas_arr = np.array(data['traj_kappas'])
            sample_kappas = np.zeros(n_train_data, dtype=np.float32)
            for _i, _s in enumerate(traj_starts_arr):
                _e = int(traj_starts_arr[_i+1]) if _i+1 < len(traj_starts_arr) else n_train_data
                sample_kappas[int(_s):_e] = traj_kappas_arr[_i]
            kappa_ref = 0.05
            sample_weights = np.clip((kappa_ref / sample_kappas) ** 0.5, 1.0, 4.0).astype(np.float32)
            print(f"[κ-weight] mean {sample_weights.mean():.3f} max {sample_weights.max():.3f} min {sample_weights.min():.3f}")
        else:
            sample_weights = np.ones(n_train_data, dtype=np.float32)

        # Fix 12 — 2-snapshot temporal consistency loss. For each batch sample,
        # also load the NEXT snapshot in the same trajectory (u_{t+dt}). Add
        # a regularizer encouraging encode(u_t) ≈ encode(u_{t+dt}) (small change
        # for small dt). This makes the AE manifold smoother in time, helping
        # the latent rollout stay close to FOM trajectory.
        consistency_alpha_val = consistency_alpha if 'consistency_alpha' in dir() else 0.0
        if consistency_alpha_val > 0:
            # Build "is paired" mask: idx j is pairable if j+1 is in same traj.
            traj_starts_list = list(np.array(data['traj_starts'])) + [n_train_data]
            pair_mask = np.ones(n_train_data, dtype=bool)
            for _i in range(len(traj_starts_list) - 1):
                pair_mask[traj_starts_list[_i+1] - 1] = False  # last in each traj is not pairable
            pair_indices_arr = np.where(pair_mask)[0]
            print(f"[consistency] {len(pair_indices_arr)}/{n_train_data} pairable indices, alpha={consistency_alpha_val}")

        @jax.jit
        def train_step(p, opt_st, batch, batch_next, aug_key, weights):
            aug_keys = jax.random.split(aug_key, batch.shape[0])
            batch_aug = jax.vmap(augment_3d)(batch, aug_keys)
            # Apply same augmentation to next-snapshot for consistency
            batch_next_aug = jax.vmap(augment_3d)(batch_next, aug_keys)
            def loss_fn(w):
                preds = jax.vmap(lambda u: model.apply({'params': w}, u, training=False))(batch_aug)
                diffs = batch_aug - preds
                recon = jnp.sum(diffs**2, axis=1) / (jnp.sum(batch_aug**2, axis=1) + REL_EPS)
                if consistency_alpha_val > 0:
                    z_t,    _ = jax.vmap(lambda u: model.apply({'params': w}, u, training=False, method=model.encode))(batch_aug)
                    z_tdt,  _ = jax.vmap(lambda u: model.apply({'params': w}, u, training=False, method=model.encode))(batch_next_aug)
                    z_diff = jnp.sum((z_t - z_tdt)**2, axis=1) / (jnp.sum(z_t**2, axis=1) + REL_EPS)
                    return jnp.sum(weights * recon) / jnp.sum(weights) + consistency_alpha_val * jnp.mean(z_diff)
                return jnp.sum(weights * recon) / jnp.sum(weights)
        ...
        # Fix 5 — periodic ckpt save: walltime kills lose all work otherwise.
        def _save_ckpt(_params):
            _ckpt = {
                'params': _params,
            ...
            tmp = outdir / 'checkpoint_vitcp.pkl.tmp'
            final = outdir / 'checkpoint_vitcp.pkl'
            with open(tmp, 'wb') as f: pickle.dump(_ckpt, f)
            tmp.replace(final)

        for epoch in range(num_epochs + 1):
            jkey, ak = jax.random.split(jkey)
            if consistency_alpha_val > 0:
                idx = np_rng.choice(pair_indices_arr, size=batch_size, replace=False)
            else:
                idx = np_rng.choice(n_train_data, size=batch_size, replace=False)
            wbatch = jnp.asarray(sample_weights[idx])
            batch = jnp.asarray(U_train[idx])
            if consistency_alpha_val > 0:
                batch_next = jnp.asarray(U_train[idx + 1])
            else:
                batch_next = batch  # placeholder, unused
            params, opt_state, loss = train_step(params, opt_state, batch, batch_next, ak, wbatch)
            if epoch % LOG_EVERY == 0:
                ...
                if v_loss < best_val:
                    ...
                    _save_ckpt(best_params)
                    print(f"  [ckpt-save] best_val={best_val:.4e} saved")
    ...
    if eq_mode == 'full':
        ...
        # ── CP-structured EQ construction (avoids OOM at N=128) ─────────────
        # Naive jax.jacfwd(cd)(z_next) builds full (N^3, k) Jacobian = 2.1M*k*4B
        # per pair → tens of GB and XLA fusion hangs. Instead exploit CP
        # decoder structure: u = mask * (h(z) @ V_full + b) + u_g
        # where V_full[r, n] = W_x[r,ix(n)]*W_y[r,iy(n)]*W_z[r,iz(n)] is the
        # CP basis evaluated at every grid point n. So J_D_int = V_int.T @ Jh
        # where Jh = ∂h/∂z is the tiny (rank, k) MLP Jacobian and V_int is the
        # (rank, N_int) CP basis at interior nodes only.
        _int_idx_np = np.where(np.array(mask) > 0)[0]
        # Interior node coordinates
        N2 = N * N
        _ix = _int_idx_np // N2
        _iy = (_int_idx_np // N) % N
        _iz = _int_idx_np % N
        # CP basis at interior — keep as numpy to prevent XLA constant folding
        _Wx = np.array(params['decoder']['W_x'])
        _Wy = np.array(params['decoder']['W_y'])
        _Wz = np.array(params['decoder']['W_z'])
        V_int_np = (_Wx[:, _ix] * _Wy[:, _iy] * _Wz[:, _iz]).astype(np.float32)
        print(f"V_int: {V_int_np.shape}  (rank x N_interior)")

        # Analytic MLP Jacobian (numpy chain rule — no JAX, no JIT)
        _W1 = np.array(params['decoder']['W1']['kernel'])
        _b1 = np.array(params['decoder']['W1']['bias'])
        _W2 = np.array(params['decoder']['W2']['kernel'])
        _b2 = np.array(params['decoder']['W2']['bias'])
        _Wr = np.array(params['decoder']['W_rank']['kernel'])
        _Wd = np.array(params['decoder']['W_direct']['kernel'])
        _b_scalar = float(params['decoder']['bias'])
        ...
        def _h_mlp_np(z_np):
            ...
            a2 = s1 @ _W2 + _b2
            s2 = _swish_np(a2)
            return s2 @ _Wr + z_np @ _Wd  # (rank,)
        def _jh_numpy(z_np):
            a1 = z_np @ _W1 + _b1; s1 = _swish_np(a1)
            a2 = s1 @ _W2 + _b2
            ds1 = _dswish_np(a1); ds2 = _dswish_np(a2)
            # J = Wd.T + (Wr.T * ds2) @ (W2.T * ds1) @ W1.T  shape (rank, k)
            Jh = _Wd.T + (_Wr.T * ds2[None, :]) @ (_W2.T * ds1[None, :]) @ _W1.T
            return Jh.astype(np.float32)

        # JAX eager encode/decode (no JIT) — GPU dispatch, no compilation
        eq_u_next_np = [np.asarray(p[0], dtype=np.float32) for p in eq_pairs]
        eq_u_prev_np = [np.asarray(p[1], dtype=np.float32) for p in eq_pairs]
        eq_kap_np    = [float(p[2]) for p in eq_pairs]
        z_np_all = []
        h_np_all = []
        print("EQ: encoding snapshots with JAX eager (no JIT)...")
        with jax.disable_jit():
            for i, u_flat in enumerate(eq_u_next_np):
                u_jnp = jnp.asarray(u_flat)
                z_i, _ = model.apply({'params': params}, u_jnp,
                                     training=False, method=model.encode)
                z_np = np.asarray(z_i, dtype=np.float32)
                z_np_all.append(z_np)
                h_np_all.append(_h_mlp_np(z_np))
                if i % 8 == 0:
                    print(f"  EQ encode {i+1}/{len(eq_u_next_np)}")

        # Residuals computed entirely in numpy
        mask_np = np.asarray(mask, dtype=np.float32)
        u_g_np  = np.asarray(u_g,  dtype=np.float32)
        dx_f = float(dx); dt_f = float(dt)
        R_int_list = []
        Jh_list    = []
        for i in range(len(eq_pairs)):
            # u_pred at every node from the CP h-vector — only interior matters
            h_i = h_np_all[i]
            # Decode at interior: u_pred_int = h @ V_int + b
            u_pred_int = (h_i @ V_int_np + _b_scalar).astype(np.float32)
            # Build full u_pred (zeros at boundary, since mask kills it then add u_g=0)
            u_pred_full = np.zeros(num_nodes, dtype=np.float32)
            u_pred_full[_int_idx_np] = u_pred_int  # interior nodes
            # Apply Dirichlet: u_pred = mask * u_pred + u_g  (u_g is zeros)
            u_pred_full *= mask_np
            u_pred_full += u_g_np
            # Compute Laplacian via numpy stencil
            u3 = u_pred_full.reshape(N, N, N)
            lap_full = np.zeros_like(u3)
            lap_full[1:-1,1:-1,1:-1] = (
                6*u3[1:-1,1:-1,1:-1]
                - u3[0:-2,1:-1,1:-1] - u3[2:,1:-1,1:-1]
                - u3[1:-1,0:-2,1:-1] - u3[1:-1,2:,1:-1]
                - u3[1:-1,1:-1,0:-2] - u3[1:-1,1:-1,2:]
            ) / dx_f**2
            # boundary entries of K_op_3d carry the boundary itself (identity)
            for face in (0, -1):
                lap_full[face,:,:] = u3[face,:,:]
                lap_full[:,face,:] = u3[:,face,:]
                lap_full[:,:,face] = u3[:,:,face]
            R_full = u_pred_full - eq_u_prev_np[i] + dt_f * eq_kap_np[i] * lap_full.flatten()
            # Only interior contributes to G (boundary is masked out anyway)
            R_int_list.append(R_full[_int_idx_np].astype(np.float32))
            Jh_list.append(_jh_numpy(z_np_all[i]))

        # Build G in single batched BLAS call: G[k_i*n_pairs + i, :] = (Jh_i.T @ V_int) * R_i
        # Stack Jh as (n_pairs*k_dim, rank), then one matmul against V_int_np
        Jh_stack = np.concatenate([J.T for J in Jh_list], axis=0)  # (n_pairs*k_dim, rank)
        JD_stack = Jh_stack @ V_int_np                              # (n_pairs*k_dim, N_int)
        # Replicate R rows: each pair contributes k_dim rows that all multiply by R_i
        R_stack = np.repeat(np.stack(R_int_list, axis=0), k_dim, axis=0)  # (n_pairs*k_dim, N_int)
        G_int = (JD_stack * R_stack).astype(np.float32)
        b_int = np.sum(G_int, axis=1).astype(np.float32)

        print(f"NNLS: G shape {G_int.shape} (interior only)")
        w_int, _ = nnls(G_int, b_int)  # patch_nnls.py caps maxiter
        # Lift weights back to full grid
        w_full = np.zeros(num_nodes, dtype=np.float32)
        w_full[_int_idx_np] = w_int
        eq_indices = np.where(w_full > 1e-10)[0]; eq_weights = w_full[eq_indices]
        print(f"EQ nodes: {len(eq_indices)} / {num_nodes}")
    ...
    def make_heat_latent_solver(p, V_eq_, b_sp, mask_sp_, ug_sp_, eq_w, latent_dim, dx_, dt_):
        ...
        def make_step(kappa_f32):
            ...
            def solve_step(z_init, s_init, u_prev_phys_eq_):
                ...
                res_norm = jnp.sqrt(jnp.dot(eq_w*R_f, R_f))
                # Fix 6 — return EQ-node u at NEW time step (avoids decode_full per step in run_rom).
                # u_new = s * h(z) at EQ nodes (NOT s*fn — fn yields u_prev at convergence).
                # u_st[:,0] in _f_norm is exactly h_at_eq before scaling.
                h_new = _mlp_body(z_f)
                u_new_eq_norm = (mask_sp_ * (h_new @ V_eq_ + b_sp) + ug_sp_).reshape(-1, 7)[:, 0]
    ...
    # Fix 7 — fuse the rollout into a single JIT'd lax.fori_loop with kappa as
    # a TRACED RUNTIME argument so JIT compiles ONCE and is reused across all
    # benchmark trajectories (10 different kappas previously triggered 10 fresh
    # compiles, polluting the ROM timing).
    decode_full_jit = jax.jit(decode_full)

    def _solve_step_rt(z_init, s_init, u_prev_phys_eq_, kappa_f32):
        # Same logic as the closure-based solve_step in make_heat_latent_solver,
        # but with kappa as a runtime argument. NM-ROM theory unchanged.
        dec = params['decoder']
        W1k, b1k = dec['W1']['kernel'], dec['W1']['bias']
        W2k, b2k = dec['W2']['kernel'], dec['W2']['bias']
        Wrk, brk = dec['W_rank']['kernel'], dec['W_rank']['bias']
    ...
    # Fix 11 — periodic AE re-anchor: every REANCHOR_EVERY steps, decode_full(z,s)
    # then re-encode it. This projects the latent state back onto the AE manifold,
    # preventing rollout drift. Theory-clean (KKT-style projection). Cost: roughly
    # one extra encode+decode every K steps.
    REANCHOR_EVERY = 999999  # disabled (>NUM_STEPS) — Fix 11 not active
    ...
    def _rollout_rt(z0, s0, u_cur_eq0, kappa_f32):
        ...
        def _step(i, carry):
            ...
            # Re-anchor: every K steps, decode → re-encode → use refreshed z,s,u_eq
            do_anchor = ((i + 1) % REANCHOR_EVERY == 0) & (i + 1 < NUM_STEPS)
            def _anchor(args):
                z_, s_ = args
                u_full = decode_full_jit(z_, s_)
                z_re, s_re = encode_jit(u_full)
                u_eq_re = u_full[eq_idx_jnp]
                return z_re, s_re, u_eq_re
            def _no_anchor(args):
                z_, s_ = args
                return z_, s_, u_eq_n
            z_n2, s_n2, u_eq_n2 = jax.lax.cond(do_anchor, _anchor, _no_anchor, (z_n, s_n))
            iters_ = iters_.at[i].set(n_iters)
            res_ = res_.at[i].set(res_norm)
            return (z_n2, s_n2, u_eq_n2, iters_, res_)
    ...
    def run_rom(u0_flat, kappa, steps):
        ...
        return snapshots, np.array(iters_arr), np.array(res_arr)
    ...
    fom_times, rom_times, final_errors, gn_avgs = [], [], [], []
    print("\n--- Benchmark ---")
    for i, tp in enumerate(test_params):
        u0 = make_gaussian_ic(tp['centers'], tp['amplitudes'], tp['widths'])
        ...
        final_errors.append(final_err)
        gn_avgs.append(float(np.mean(gn_iters)))
        # Diagnostic: AE reconstruction floor at FOM final state.
        # If RelL2(T) >> ae_recon_err, ROM dynamics is the issue.
        # If RelL2(T) ~= ae_recon_err, AE reconstruction is the floor.
        z_t, s_t = encode(U_fom[-1])
        u_ae = decode_full_jit(z_t, s_t)
        ae_recon_err = float(jnp.linalg.norm(u_ae - U_fom[-1]) / (norms_fom[-1] + 1e-12))
        print(f"  [{i+1:2d}/10] κ={kap:.4f} | FOM {fom_t:.3f}s | ROM {rom_t:.3f}s | "
              f"RelL2(T) {final_err:.3e} | AE-recon(T) {ae_recon_err:.3e} | GN avg {np.mean(gn_iters):.1f}")
    ...
    return dict(
        ...
        gn_iters_mean=float(np.mean(gn_avgs)) if gn_avgs else float(max_iters),
