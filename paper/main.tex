...
batched; the batched fit: Table~\ref{tab:headline-times}). Burgers accurate
quadrature: $^{s}$\,stored rule of job \provEqtopLadderJob{}, not
...
rule of Table~\ref{tab:eqladder}); $^{\ell}$\,deterministic
...
$256^2$, Appendix~\ref{app:cg-current}); $^{d}$\,dense residual. The earlier Burgers model has one setting and
