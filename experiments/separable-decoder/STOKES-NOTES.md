...
**Revision 3.** Two rounds of independent verification by Codex `gpt-5.6-sol`.
...
gates mathematically invalid: the revision-2 S-EXACT *prediction* bound. It is
**retracted in full** (retraction 7) and replaced by **S-BACKERR**, which is
independent by construction. The verifier has now corrected three claims of
mine; every one is carried below, in "Retractions".

The operators and the solver have not changed since revision 1.
...
<!-- generated by stk2d_tables.py from stk2d_fom_gates_nu1_M64.json (commit e4c3231f6bc3) -- do not edit by hand -->
...
| 64 | 8064 | 4096 | 8.0358e-04 | 4.0171e-04 | 8.0360e-04 | 4.0170e-04 | 1.368e-16 | 1.255e-14 | 0.8 |
| 128 | 32512 | 16384 | 2.0082e-04 | 1.0041e-04 | - | - | 4.696e-16 | 2.645e-14 | 7.3 |
| 256 | 130560 | 65536 | 5.0201e-05 | 2.5100e-05 | - | - | 4.962e-16 | 2.005e-13 | 79.2 |
...
| 128 | 9.531800e-04 | 9.531800e-04 | 3.008e-08 | 1.144216e-02 | 1.144216e-02 | 2.578e-07 | 0.9126 | 7.2 |
...
| 64 | 4095 | 4095 | 3969 | 3969 | 3969 | 3969 | 110 |
...
| N | M | control Frobenius (min over p=chi_j) | = 1/sqrt(M) | sqrt(M) x metric | matched-control cosine (min) | solenoidal Frobenius (max) | solenoidal cosine (max) | \|\|D Phi\|\| normalized |
|---|---|---|---|---|---|---|---|---|
| 8 | 49 | 0.142857 | 0.142857 | 1.000000000000 | 1.000000000000 | 4.326e-17 | 3.893e-16 | 6.753e-18 |
| 16 | 64 | 0.125000 | 0.125000 | 1.000000000000 | 1.000000000000 | 7.624e-17 | 4.996e-16 | 3.116e-18 |
| 32 | 64 | 0.125000 | 0.125000 | 1.000000000000 | 1.000000000000 | 6.063e-17 | 5.655e-16 | 1.413e-18 |
| 64 | 64 | 0.125000 | 0.125000 | 1.000000000000 | 1.000000000000 | 4.782e-17 | 2.810e-16 | 6.499e-19 |
| 128 | 64 | 0.125000 | 0.125000 | 1.000000000000 | 1.000000000000 | 7.296e-17 | 5.456e-16 | 3.044e-19 |
| 256 | 64 | 0.125000 | 0.125000 | 1.000000000000 | 1.000000000000 | 1.386e-16 | 1.453e-15 | 1.463e-19 |
...
|---|---|---|
| PRECOND (asserts live / ALLOW_CPU / ladders non-empty) | True / 0 / 2,5 | asserts live, ALLOW_CPU=0, both ladders non-empty |
| S-BACKERR (normalised backward error, all 14 solves) | 1.790e-17 | <= 1e-13 |
...
run: stk2d_fom_gates_nu1_M64.json | commit `e4c3231f6bc3` | host `spark-d69e` | numpy 2.4.4 scipy 1.17.1 | jax backend `gpu` x64=True matmul `highest` | total 214 s
<!-- END GENERATED -->
...
The normalized backward error of every solve in the run,

$$\eta = \frac{\lVert Kx-b\rVert_2}{\lVert K\rVert_F\lVert x\rVert_2+\lVert b\rVert_2}\;\le\;10^{-13},$$

asserted over all **14** solves — both manufactured families and the free-slip
arm — with a worst value of $1.79\times10^{-17}$.

Three properties the retracted prediction gate did not have. Its threshold is
**frozen a priori** from backward stability of the sparse LU,
$O(\sqrt{\mathrm{nnz}}\,u_{\text{mach}})$, and derives from nothing it tests.
It is **direction-independent**: it measures a residual, not an alignment. And
it is **falsifiable and sharp** — perturbing a converged solution by a relative
$10^{-11}$, which is four orders *inside* the S-EXACT field tolerance, raises
$\eta$ to $2.00\times10^{-13}$ and fails the gate.

Note the *un*normalized $\lVert Kx-b\rVert/\lVert b\rVert$ is also recorded
and grows like $h^{-2}$ ($1.13\times10^{-15}$ at $N=8$ to $2.87\times10^{-12}$
at $N=256$) purely because $\lVert K\rVert$ does. That form is a diagnostic;
$\eta$ is the gate.

### PRECOND — the harness's own preconditions, now enforced

Round 2 pointed out that "`complete=true` implies every gate passed" held only
under unstated preconditions. They are now checked:

- **`ALLOW_CPU=0`** and **non-empty `RANK_NS` / `FREESLIP_NS` ladders** are
  asserted;
- **python must not be running with `-O`**, and this one is a `raise`, not an
  `assert` — an assert cannot detect its own disablement. Under `-O` the driver
  refuses to run at all rather than emit a JSON claiming `complete=true` without
  having checked anything.
- `SMOKE=1` relaxes the configuration preconditions for development runs and is
  recorded in the JSON as `smoke: true`, so a smoke run can never be mistaken
  for a certified artifact.

The committed artifact is a `SMOKE=0`, `ALLOW_CPU=0` run on the frozen
configuration.
...
8. **The $0.5/\sqrt{M}$ restatement I proposed for the S3 floor was
   ...
   dimensionless form $\sqrt M\,\times$ (control metric) $\ge0.99$ is now
   gated alongside the $10^{-2}$ floor, and it measures **exactly
   $1.000000000000$** at every mesh. No phase-2 threshold change is needed:
   $M\in\{32,64,128\}$ gives $0.17678$, $0.125$, $0.08839$, all clear.
...
- **S3's constant floor does not scale.** It is kept at $10^{-2}$ and cleared by
  ...
  correct aligned control at $M>10^4$. The dimensionless
  $\sqrt M\,\times$ (control metric) $\ge0.99$ is now gated alongside it and
  is the form that scales. Phase 2's frozen $M\in\{32,64,128\}$ is safely
  clear either way.
...
214 s wall, single local process, no cluster job. The committed JSON is a
`SMOKE=0`, `ALLOW_CPU=0` run on the frozen configuration, `complete: true`. JAX is imported for provenance
