"""Train the Heat-2D FiLM AUTO-DECODER u(x; z) (one latent per (trajectory,
time) snapshot) + POD basis for the linear control.

  data    : hf.build_trajectories(N) from seed (TRAIN = first N_TRAIN traj)
...
Usage: N=64 K_LAT=8 [AD_STEPS=40000] [AD_BATCH=128] [P_SUB=0] [T_SMOOTH=1e-2]
       [LAT_REG=1e-4] [LAT_LR=5e-3] python hlat_train_ad.py <outdir>
"""
...
from hlat_common import mp, F64, log
...
def main():
    ...
    with open(os.path.join(OUTDIR, f"hlat_ad_{tag}.pkl"), "wb") as f:
        pickle.dump(ck, f)
    rep = {k: v for k, v in ck.items() if k not in ("params", "Z_train", "V")}
    rep["sv"] = [float(s) for s in sv[:32]]
    with open(os.path.join(OUTDIR, f"hlat_ad_{tag}_report.json"), "w") as f:
