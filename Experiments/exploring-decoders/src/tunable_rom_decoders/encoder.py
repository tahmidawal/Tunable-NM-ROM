"""ViT encoder (pinned copy from poisson/).

Two outputs are exposed:
  * `encode(u)` -> z in R^latent_dim     (mean-pooled, head-projected)
  * `encode_tokens(u)` -> T in R^(n_tok, embed_dim)   (per-token features)

The token output is post-final-transformer-block, post-LayerNorm — the
cross-attention decoder uses these as keys/values, while the modulated
SIREN decoder uses only `z`.
"""
...
class ViTEncoder(nn.Module):
    ...
    def _trunk(self, u_flat):
        ...
        return tokens  # (num_patches, embed_dim)
