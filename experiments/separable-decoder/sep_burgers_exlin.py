"""EXACT LINEAR TERMS in the weak residual, on the round-4/5 speed protocol.

`sep_speed_r5.py` with the sampled residual replaced by the EXACT-LINEAR one
(exp/2026-08-26-eq-learned; report 2026-08-25-eq-fidelity-ladder.md section 3
item 1).  The separable decoder makes every Phi^T u term exactly computable:

    A = Phi^T G_int                      (M, R), precomputed once offline
    r_w(z) = wt * [ A(h(z) - h(z_n)) + DT*( Phi_q^T N(u)|_nodes
                                            + nu*lam*A h(z) ) ]

Mass, previous-state and Laplacian terms need NO quadrature; only the
advection term Phi^T N(u) (sign-upwind, quadratic in u) stays on the m NNLS
nodes, via the byte-identical G_st/Phi_q closure.  prev_c becomes the
M-vector A h(z_n), cached per step exactly like the old node values were.
With EQ_ADV_ONLY=1 (default) the m nodes are refit on the ADVECTION row
blocks only (exlin_common.eq_fit_burgers_adv) -- the freed budget serves one
term.

GATES (rule change, logged in LAB-LOG 2026-08-26): gate 0 (bit-identity of
the whole sampled residual to make_weak_ops) no longer applies to the exlin
residual BY DESIGN.  It is still asserted for the incumbent-form ops built on
the same node set (code identity), and two new gates cover the change:
  gate L: exlin linear part == full-grid linear part      (exactness, <=1e-12)
  gate A: exlin advection part == incumbent advection part (<=1e-12)
Everything else -- IC encoder+Gram fit, LM rule, stall/budget/extrap
variants, the classical (newton_tol, lin_tol) ladder, matched-accuracy
selection, paired AB/BA, batch sweep -- is byte-identical to sep_speed_r5.py.

--- original round-5 header follows ---
...
EXLIN = int(os.environ.get("EXLIN", "1"))
EQ_ADV_ONLY = int(os.environ.get("EQ_ADV_ONLY", "1"))
...
def main():
    ...
    h_fn = dec.head_fn()
    OUT = f"{OUT_PREFIX}sep_burgers_exlin_{os.environ.get('OUT_TAG', f'N{N}_K{K}_R{R}')}.json"
    ...
    report = dict(config=dict(
        pde="burgers2d", round=5, kind="profile+speed+exlin", N=N, k=K, r=R,
        exlin=bool(EXLIN), eq_adv_only=bool(EQ_ADV_ONLY),
    ...
    for Mi in EQ_MS:
        ...
        if EQ_ADV_ONLY:
            keep, wq_np, eq_info = xc.eq_fit_burgers_adv(
                ...
                4 * Mi, f"exlin N={N} k={K} M={Mi} m={4*Mi} adv-only",
                bc.nnls_capped)
        else:
            keep, wq_np, eq_info = ctol_eq.eq_fit_burgers(
                u_full_int, adv_full, np.asarray(Phi), cand_pos, Z_eq, K,
                4 * Mi, f"sep speed r4 N={N} k={K} M={Mi} m={4*Mi}",
                bc.nnls_capped)
        ...
        # A = Phi^T G_int, the (M, R) exact-linear matrix; Phi kept (jnp) for
        # the gate-L full-grid linear reference, freed after the gates.
        Phi_full_j = jnp.asarray(np.asarray(Phi)) if EXLIN else None
        A_j = (Phi_full_j.T @ G_int) if EXLIN else None
        del Phi
        ...
        if EXLIN:
            # -------- exact-linear sampled ops (see module docstring) -------
            def mk_ex(G_st=G_st, Phi_q=Phi_q, lam_j=lam_j, A=A_j):
                ...
                def full_ex(z):
                    return G_st[:, 0, :] @ h_fn(z)
                return (r_w_ex, rJ_ex, prev_of_ex, full_ex, u_and_N_ex,
                        d_c_ex, parts_ex)
            (r_w_x, rJ_x, prev_x, full_x, uN_x, dc_x, parts_x) = mk_ex()
            ops_ex = bc._finish_ops(rJ_x, r_w_x, prev_x, full_x, m, "lspg")
            ops_ex["M"] = Mi
            ops_ex["tol_scale"] = float(np.sqrt(n_i2))

            # full-grid linear reference for gate L (explicit jit arguments)
            def lin_full_ref(Gb, Ph, z, zp, nu, lam=lam_j):
                w_ = (1.0 + bc.DT * nu * lam) ** (-bc.WEAK_ALPHA)
                u = Gb @ h_fn(z)
                up = Gb @ h_fn(zp)
                return w_ * (Ph.T @ (u - up) + bc.DT * nu * lam * (Ph.T @ u))
            lin_full_j = jax.jit(lin_full_ref)
            parts_x_j = jax.jit(parts_x)
            prev_x_j = jax.jit(prev_x)
            prev_inc_j = jax.jit(prev_f)

            def parts_inc(z, prev_c, nu, lam=lam_j, Phi_q=Phi_q):
                w_ = (1.0 + bc.DT * nu * lam) ** (-bc.WEAK_ALPHA)
                u, Nu = uN_f(z)
                return w_ * bc.DT * (Phi_q.T @ Nu)
            adv_inc_j = jax.jit(parts_inc)
            ...
            for _ in range(5):
                ...
                zp = jnp.asarray(Z_tr[grng.integers(len(Z_tr))])
                nu = float(np.median(nu_test))
                lin_f = lin_full_j(G_int, Phi_full_j, zt, zp, nu)
                _, lin_x, adv_x = parts_x_j(zt, prev_x_j(zp), nu)
                adv_i = adv_inc_j(zt, prev_inc_j(zp), nu)
            ...
            del Phi_full_j
            eq_ops[name] = dict(ops_fast=ops_ex, ops_ref=ops_ref, idx=idx,
                                pos=pos, m=m, r_w=r_w_x, rJ=rJ_x, prev=prev_x,
                                uN=uN_x, dc=dc_x, G_st=G_st, Phi_q=Phi_q,
                                A=A_j)
        else:
