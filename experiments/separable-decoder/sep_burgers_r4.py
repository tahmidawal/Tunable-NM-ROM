...
def main():
    ...
    TAG = (f"N{N}_K{K}_R{R}_h{ARCH.get('h_hidden', 128)}"
           f"x{ARCH.get('h_layers', 2)}")
    OUT = f"{OUT_PREFIX}sep_burgers_r4_{TAG}.json"
    CKPT = f"{OUT_PREFIX}sep_burgers_r4_{TAG}.pkl"
    ...
    params, Z_tr, tinfo = ss.train_autodecoder_v2(
        ...
        snap_norm=bool(SNAP_NORM), w_extra=w_extra, z_polish=Z_POLISH,
        z_polish_lr=Z_POLISH_LR,
        tag=f"burgers r4 N={N} k={K} r={R}", **ARCH)
    report["train"] = tinfo
